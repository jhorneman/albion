...
struct OPM *OPM_update_list[UPDATE_OPMS_MAX];
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Update_button_state
 * FUNCTION  : Update the current button state.
 * FILE      : CONTROL.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 02.09.94 13:42
 * LAST      : 02.09.94 13:42
...
void
Update_button_state(struct BLEV_Event_struct *Event)
{
	UNSHORT Old, New;
...
Add_update_OPM(struct OPM *OPM)
{
	...
	{
		/* Is this slot free ? */
		if (!OPM_update_list[i])
		{
			/* Yes -> Insert new entry */
			OPM_update_list[i] = OPM;
			break;
		}
	}
}
...
Remove_update_OPM(struct OPM *OPM)
{
	...
	for (i=0;i<UPDATE_OPMS_MAX;i++)
	{
		...
		if (OPM_update_list[i] == OPM)
		{
			/* Yes -> Remove entry */
			OPM_update_list[i] = NULL;
			break;
		}
	}
}

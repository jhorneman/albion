...
//	Map_ptr = (struct Map_data *) MEM_Claim_pointer(Map_handle);
	...
	MEM_Free_memory(Blocklist_handle[0]);
	MEM_Free_memory(Icongfx_handle[0]);

/*	if (Map_ptr->ICON_SET_2_NR)
	{
		MEM_Free_memory(Icondata_handle[1]);
		MEM_Free_memory(Blocklist_handle[1]);
...
Display_M2_selection(UNSHORT X, UNSHORT Y)
{
	...
	Map_selected_X = (X + (Scroll_2D.Playfield_X & 0x000F)) / 16 +
	 Mapbuf_X + 1;
	Map_selected_Y = (Y + (Scroll_2D.Playfield_Y & 0x000F)) / 16 +
	 Mapbuf_Y + 1;

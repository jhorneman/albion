 ...
 UNSHORT Tactical_Y, UNSHORT Strength)
{
	...
	if (Fireball_COMOB && Firering_COMOB)
	{
		...
		Nr_steps = Prepare_COMOB_movement(Current_use_magic_data.Casting_participant,
		 Fireball_COMOB, Victim_part->Tactical_X, Victim_part->Tactical_Y, 5);

/*		{
			struct Combat_participant *Source_part;

			Source_part = Victim_part;
			Victim_part = Current_use_magic_data.Casting_participant;

			Nr_steps = Prepare_COMOB_movement(Source_part,
	...
	if (Strength)
	{
		...
		if (Fireball_COMOB && Firering_COMOB)
		{
			...
			{
				/* Yes -> Make the screen red */
				Recolour_palette(0, 192, 0xff, 0, 0, 50);
			}

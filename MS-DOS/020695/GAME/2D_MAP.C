...
MEM_HANDLE Icondata_handle[2];
static MEM_HANDLE Icongfx_handle[2];
MEM_HANDLE Blocklist_handle[2];
...
Right_2D_object(struct Object *Object, union Method_parms *P)
{
	...
	X = ((X2 + 15) & 0xFFF0) - (Scroll_2D.Playfield_X & 0x000F);
	Y = ((Y2 + 15) & 0xFFF0) - (Scroll_2D.Playfield_Y & 0x000F);

	Display_M2_selection(X, Y);

	Map_selected_X = (X + (Scroll_2D.Playfield_X & 0x000F)) / 16 + Mapbuf_X + 1;
	Map_selected_Y = (Y + (Scroll_2D.Playfield_Y & 0x000F)) / 16 + Mapbuf_Y + 1;

	Map_PUM.Title = System_text_ptrs[14];

	Do_PUM(X + 16, Y - 40, &Map_PUM);
}
...
M2_ModInit(void)
{
	...
	/* Load first icon set */
	Handle = Load_subfile(ICON_DATA, (UNSHORT) Map_ptr->ICON_SET_1_NR);
	if (!Handle)
	{
		Error(ERROR_FILE_LOAD);
		Exit_program();
	...
	Icondata_handle[0] = Handle;

	Handle = Load_subfile(ICON_GFX, (UNSHORT) Map_ptr->ICON_SET_1_NR);
	if (!Handle)
	{
		Error(ERROR_FILE_LOAD);
		Exit_program();
	...
	Icongfx_handle[0] = Handle;

	Handle = Load_subfile(BLOCK_LIST, (UNSHORT) Map_ptr->ICON_SET_1_NR);
	if (!Handle)
	{
		Error(ERROR_FILE_LOAD);
		Exit_program();
	...
	Blocklist_handle[0] = Handle;

	/* Load second icon set (if any) */
	if (Map_ptr->ICON_SET_2_NR)
	{
		Handle = Load_subfile(ICON_DATA, (UNSHORT) Map_ptr->ICON_SET_2_NR);
		if (!Handle)
		{
			Error(ERROR_FILE_LOAD);
			Exit_program();
		...
		Icondata_handle[1] = Handle;

		Handle = Load_subfile(ICON_GFX, (UNSHORT) Map_ptr->ICON_SET_2_NR);
		if (!Handle)
		{
			Error(ERROR_FILE_LOAD);
			Exit_program();
		...
		Icongfx_handle[1] = Handle;

		Handle = Load_subfile(BLOCK_LIST, (UNSHORT) Map_ptr->ICON_SET_2_NR);
		if (!Handle)
		{
			Error(ERROR_FILE_LOAD);
			Exit_program();
...
/*	OPM_FillBox(&Status_area_OPM, 181, 3, 80, 40, BLACK);
	xprintf(&Status_area_OPM, 182, 4, "Step : %u:%u", Current_step, Current_substep);
*/
	if (Diagnostic_mode)
	{
...
Exit_Move_M2(void)
{
	...
	Input_handling_flag = TRUE;
}

/*
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Select_M2_Main_loop
...
Select_M2_Main_loop(void)
{
	...
	{
		/* Yes -> Highlight */
		X = ((Mouse_X + 15) & 0xFFF0) - (Scroll_2D.Playfield_X & 0x000F);
		Y = ((Mouse_Y + 15) & 0xFFF0) - (Scroll_2D.Playfield_Y & 0x000F);
 ...
 * LAST      : 02.11.94 15:01
 * INPUTS    : UNSHORT X - X-coordinate (on 16-pixel boundary).
 *             UNSHORT Y - Y-coordinate (on 16-pixel boundary).
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
{
	Put_recoloured_box(&Main_OPM, X + 1, Y , 14, 1, &(Recolour_tables[7][0]));
	Put_recoloured_box(&Main_OPM, X, Y + 1, 16, 14, &(Recolour_tables[7][0]));
	Put_recoloured_box(&Main_OPM, X + 1, Y + 15, 14, 1, &(Recolour_tables[7][0]));
}
...
Init_Select_M2(void)
{
	...
	if (X + W >= MAP_2D_X + Map_2D_width - 16)
	{
		W = MAP_2D_X + Map_2D_width - 16 - X;
	}

	if (Y + H >= MAP_2D_Y + Map_2D_height - 16)
	{
		H = MAP_2D_Y + Map_2D_height -16 - Y;
	}
...
{
	static struct Movement_2D Move = {
		0,
		0, 0,
		0,
		0, 0,
	...
	if (Before_move())
	{
		...
		Move.View_direction = PARTY_DATA.View_direction;
		Move.Travel_mode = PARTY_DATA.Travel_mode;
		Move.State = Active_member_state;

		Result = Try_2D_move(&Move);

		/* Enter move */
		PARTY_DATA.X = Move.X;
		PARTY_DATA.Y = Move.Y;
		PARTY_DATA.View_direction = Move.View_direction;
		Active_member_state = Move.State;
		Move_2D(Move.dX, Move.dY, Move.Flags & UPDATE_AFTER_MOVE);
...
Try_2D_move(struct Movement_2D *Move)
{
	...
	BOOLEAN Result = TRUE, Slide_flag = FALSE;
	SISHORT dX, dY, ddDir, corrX = 0;
	UNSHORT X, Y, Direction;
	UNSHORT cDir, dDir;
...
Special_2D_high_check(UNSHORT X, UNSHORT Y)
{
	...
	{
		/* Yes -> Get icon height */
		Status = Icon_data_ptr[Underlay_nr-2].Flags;
	...
	{
		/* Yes -> Get icon height */
		Status = Icon_data_ptr[Overlay_nr-2].Flags;
...
Special_2D_low_check(UNSHORT X, UNSHORT Y, SISHORT dX, SISHORT dY)
{
	...
	if (Overlay_nr > 1)
	{
		...
		Frame = Icon_data_ptr[Overlay_nr-2].First_frame;
	}
	...
	else
	{
		...
		Pixel = *(Icon_graphics[0] + (256 * Frame) + (15 * 16) + Side);
...
Move_2D(SISHORT dX, SISHORT dY, BOOLEAN Update)
{
	...
	/* Any movement ? */
	if (dX || dY || Update ||
	 Party_path[Party_path_index].View_direction != PARTY_DATA.View_direction)
	{
	...
	else
	{
		...
		if (Update)
		{
			...
			/* Yes -> Get party path index */
			idx = Party_path_index;

			/* Increase path index */
			idx++;
			if (idx >= PARTY_PATH_LENGTH)
				idx -= PARTY_PATH_LENGTH;

			Party_path[idx].X = (PARTY_DATA.X - 1) * 16;
			Party_path[idx].Y = (PARTY_DATA.Y - 1) * 16 + 15;
			Party_path[idx].View_direction = PARTY_DATA.View_direction;
			Party_path[idx].State = Active_member_state;

			/* Store new index */
			Party_path_index = idx;
...
//			_2D_steps_left++;
		...
		else
		{
			 ...
			 STANDING_STATE))
			{
				...
				/* Insert coordinates and new view direction */
				Party_path[idx].X = X;
				Party_path[idx].Y = Y;
				Party_path[idx].View_direction = PARTY_DATA.View_direction;
				Party_path[idx].State = Active_member_state;
...
Store_2D_movement(SISHORT dX, SISHORT dY)
{
	...
	for (i=1;i<=16;i++)
	{
		...
		/* Store coordinate and view direction */
		Party_path[idx].X = (PARTY_DATA.X - 1) * 16 + (dX * i);
		Party_path[idx].Y = (PARTY_DATA.Y - 1) * 16 + 15 + (dY * i);
		Party_path[idx].View_direction = PARTY_DATA.View_direction;
		Party_path[idx].State = Active_member_state;
	}

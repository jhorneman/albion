...
UNCHAR Floor_buffer_fname[] = "Floor buffer";

struct File_type Floor_buffer_ftype = {
	NULL,
	0xFF,
	0,
	MEM_KILL_ALWAYS,
...
UNCHAR Shade_table_fname[] = "Shade table";

struct File_type Shade_table_ftype = {
...
M3_ModInit(void)
{
	...
	I3DM.Shade_table = (UNBYTE *) (((UNLONG) MEM_Get_pointer(Shade_table_handle) + 256) & 0xffffff00);
	...
	for (i=0;i<4;i++)
	{
		I3DM.Shade_offsets[i] = (Shade_offsets[i] * Lab_ptr->Shade_deviation) / 100;
	}
...
Init_3D_display(void)
{
	...
	M3_Object = Add_object(Earth_object, &M3_Class, NULL, Window_3D_X, Window_3D_Y,
	 I3DM.Window_3D_width, I3DM.Window_3D_height);
}
...
Load_3D_textures(struct Object_3D *Object_ptr, struct Wall_3D *Wall_ptr)
{
	...
	Shellsort(Swap_3D_textures, Compare_3D_textures, Nr_of_textures, (UNBYTE *) (UNLONG) 0);
...
Calculate_shade_tables_for_transparent_walls(void)
{
	...
	struct Overlay_3D *Overlay_ptr;
	MEM_HANDLE Handle;
	UNSHORT Target_R, Target_G, Target_B, R, G, B;
	UNSHORT i, j;
	UNBYTE *Ptr;
 ...
 * BUGS      : No known.
 * NOTES     : - This function will also set a pointer to a block of
 *              RLE-data in the interface data structure.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
 ...
 UNSHORT Target_G, UNSHORT Target_B, UNSHORT Number)
{
	...
	Ptr = (UNBYTE*) (((UNLONG) MEM_Claim_pointer(Shade_table_handle) + 256) & 0xffffff00);
...
Initialize_3D_position(void)
{
	...
	Map_to_dungeon(PARTY_DATA.X, PARTY_DATA.Y, &X, &Y);
	...
	if (Result)
	{
		...
		else
		{
			...
			if (Result)
			{
				...
				if (NPC_index != 0xFFFF)
				{
					/* Yes -> Determine distance from party to me */
 ...
 struct Object_group_3D *Object_group, struct Object_3D *Object_base)
{
	...
	for (i=0;i<OBJECTS_PER_GROUP;i++)
	{
		...
		if (Object_nr)
		{
			...
			/* Blocking ? */
			if (Object_ptr->Flags & (1 << (BLOCKED_TRAVELMODES_B + Travel_mode)))
			{
...
void
Map_to_dungeon(UNSHORT Map_X, UNSHORT Map_Y, UNLONG *Dungeon_X_ptr,
...
//	*Dungeon_Y_ptr = (UNLONG)(Map_height - Map_Y - 1) * f + (f / 2);
}
 ...
 *             UNLONG *Map_X_ptr - Pointer to map X-coordinate.
 *             UNLONG *Map_Y_ptr - Pointer to map Y-coordinate.
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
void
Dungeon_to_map(UNLONG Dungeon_X, UNLONG Dungeon_Y, UNSHORT *Map_X_ptr,
 UNSHORT *Map_Y_ptr)
{
	UNLONG f;
	...
	*Map_X_ptr = (UNSHORT) ((Dungeon_X / f) + 1);
	*Map_Y_ptr = (UNSHORT) (Map_height - (Dungeon_Y / f)); // - 1);
}

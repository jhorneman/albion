...
BOOLEAN Input_handling_flag = TRUE;
...
//	if ((Method == DLEFT_METHOD) || (Method == DRIGHT_METHOD))
//		return(FALSE);
...
Update_all_OPMs(void)
{
	...
	for (i=0;i<UPDATE_OPMS_MAX;i++)
	{
		...
		if (OPM && (OPM->status & OPMSTAT_INIT) && (!((OPM->status & OPMSTAT_VIRTUAL) && (OPM->virtualsrc == &Main_OPM))))
		{
			OPM->status |= OPMSTAT_CHANGED;
		}
	}

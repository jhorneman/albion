...
static UNSHORT Spells_per_class[SPELL_CLASSES_MAX] = {
	21, 11, 11, 15, 0, 9, 0
};
...
Spell_handler *Spell_handler_lists[SPELL_CLASSES_MAX] = {
...
static struct Method Spell_window_methods[] = {
	...
	{ EXIT_METHOD, Exit_Spell_window_object },
	{ DRAW_METHOD, Draw_Spell_window_object },
	{ UPDATE_METHOD, Update_help_line },
	{ RIGHT_METHOD, Close_Spell_window_object },
	{ CLOSE_METHOD, Close_Spell_window_object },
...
Do_cast_combat_spell(struct Combat_participant *Part)
{
	...
	Cast_spell_combat_action.Action_value =
	 Part->Target.Magic_target_data.Class_nr;
	Cast_spell_combat_action.Action_extra =
...
Enter_spell_target(void)
{
	...
	UNSHORT Target_item_slot_index = 0xFFFF;
	UNSHORT i;
	...
	/* Write target data */
	Current_use_magic_data.Target_item_slot_index = Target_item_slot_index;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Select_empty_tactical_square
 * FUNCTION  : Select an empty tactical square (combat spell target selection
 *              exception handler).
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
...
UNLONG
Select_empty_tactical_square(UNSHORT Class_nr, UNSHORT Spell_nr,
 UNSHORT *Target_item_slot_index_ptr)
{
	UNLONG Combat_mask;
	UNSHORT Square_index;

	/* Get empty squares in monster area */
	Combat_mask = ((Get_monster_targets() | Get_party_targets()) &
	 COMBAT_MONSTER_MASK) ^ COMBAT_MONSTER_MASK;
...
Handle_spell(void)
{
	...
	else
	{
		...
		if (Char->Spell_points >= Required_SP)
		{
			Char->Spell_points -= Required_SP;
		}
		else
		{
			Char->Spell_points = 0;
 ...
 UNSHORT Incoming_strength)
{
	...
	{
		/* Yes -> Add permanent spell effect */
	...
	if (Class_nr < SPELL_CLASSES_MAX)
	{
		...
		if (Spell_nr && (Spell_nr <= Spells_per_class[Class_nr]))
		{
			...
			if (!(Spell_known(Char_handle, Class_nr, Spell_nr)))
			{
				...
				Char->xSpell_capabilities[Class_nr][Spell_nr] =
...
{
	return(System_text_ptrs[203 + (Class_nr * SPELLS_PER_CLASS) +
	 Spell_nr - 1]);
}
...
{
	struct Spell_window_OID Spell_window_OID;
	...
	if (Counter)
	{
		...
		Spell_window_OID.Char_handle = Char_handle;
		Spell_window_OID.Spell_info = &(Spell_info[0]);
		Spell_window_OID.Selected_class_ptr = &Selected_class;
		Spell_window_OID.Selected_spell_ptr = &Selected_spell;
		Spell_window_OID.Nr_spells = Counter;

		/* Do spell window */
		Push_module(&Window_Mod);

		Spell_window_object = Add_object(0, &Spell_window_Class,
		 (UNBYTE *) &Spell_window_OID, 50, 50, 200, 119);
...
Init_Spell_window_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Spell_window->Nr_spells > MAX_SPELLS_IN_WINDOW)
	{
		...
		Change_object_size(Object->Self, SPELL_WIDTH + 27 + BETWEEN +
		 SCROLL_BAR_WIDTH, (MAX_SPELLS_IN_WINDOW * SPELL_HEIGHT) + 33);
	...
	else
	{
		...
		Change_object_size(Object->Self, SPELL_WIDTH + 27,
		 (Spell_window->Nr_spells * SPELL_HEIGHT) + 33);
	...
	/* Set root OPM */
	Set_root_OPM(&(Spell_window->Window_OPM));
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Exit_Spell_window_object
 * FUNCTION  : Exit method of Spell Window object.
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 04.04.95 11:55
 * LAST      : 04.04.95 11:55
...
void
Exit_Spell_window_object(struct Object *Object, union Method_parms *P)
{
...
Draw_Spell_window_object(struct Object *Object, union Method_parms *P)
{
	...
	Draw_deep_border(OPM, 12, 11, SPELL_WIDTH + 2, H - 22);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Close_Spell_window_object
 * FUNCTION  : Close method of Spell Window object.
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 04.04.95 11:58
 * LAST      : 04.04.95 11:58
 * INPUTS    : struct Object *Object - Pointer to object.
 *             union Method_parms *P - Method parameters.
 * RESULT    : None.
...
void
Close_Spell_window_object(struct Object *Object, union Method_parms *P)
{
...
Help_Spell_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Spell_data->Status == SPELL_TOO_MUCH_SP)
	{
		...
		sprintf(String, System_text_ptrs[427],
		 Get_spell_required_SP(Spell_data->Class_nr, Spell_data->Spell_nr),
		 Calculate_required_LP(Spell_window->Char_handle, Spell_data->Class_nr,
 		 Spell_data->Spell_nr));
	}
	else
	{

...
struct Object_class Automap_Class = {
	...
	if (Q < 9)
	{
		...
		switch (Q)
		{
			...
			case 4:
			{
				...
				Buffer_ptr = (struct Automap_entry *) MEM_Claim_pointer(Automap_buffer_handle);
				...
				if (c)
				{
					...
					if (c == GOTO_POINT_AUTO_ICON)
					{
						...
						if (Nr != 0xFFFF)
						{
							...
							Ptr += Map_text_0_sub_block_offsets[Nr + 1];

							/* Print help line */
							Execute_method(Help_line_object, SET_METHOD, (void *) Ptr);
	...
	for (i=1;i<=Map_height;i++)
	{
		for (j=1;j<=Map_width;j++)
		{
			...
			if (Get_automap_bit(j, i))
			{
				...
				{
					/* Yes -> Insert floor */
					Automap_buffer_ptr->Layer_1 = Square_ptr->Floor_layer + 10000 - 1;
				}

				/* Is there a wall or object group here ? */
				c = Square_ptr->Wall_layer;
				if (c)
				{
					/* Is it a wall ? */
					if (c >= FIRST_WALL)
		...
		for (j=0;j<Nr_entries;j++)
		{
			...
			if (Entry_data[j].First_block_nr != 0xFFFF)
			{
				...
				if (Get_automap_bit(Entry_data[j].X, i))
				{
					...
					Chain_nr = Get_event_chain_nr(Map_ptr, Entry_data[j].First_block_nr);
					if (Chain_nr != 0xFFFF)
					{
...
*/
							...
							{
								/* No -> Insert icon */
								Insert_automap_icon(i, Entry_data[j].X, Icon);
							}
						}
					}
				}

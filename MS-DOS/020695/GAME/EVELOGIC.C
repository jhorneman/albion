...
Handle_default_set(UNSHORT Value)
{
	...
	if (!(Event_action->Flags & FOUND_CHAIN))
	{
		...
		if (i != 0xFFFF)
		{
			...
			/* Prepare it */
			Prepare_event_set_context(Default_event_set_handle, i, Context, NULL);
...
Handle_map_set(UNSHORT Value)
{
	...
	for(i=0;i<NPCS_PER_MAP;i++)
	{
		...
		if (NPC_present(i))
		{
			 ...
			 (VNPCs[i].First_block_nr != 0xFFFF))
			{
				...
				Chain_nr = Get_event_chain_nr((UNBYTE *) Map_ptr, j);
				...
				if (!Read_bit_array(EVENT_SAVE_BIT_ARRAY, Bit_nr))
				{
					...
					if (Block->Byte_3 != Event_action->Action_extra)
					{
						...
						{
							/* No -> Any value ? */
							if (Block->Word_6 == 0xFFFF)
							{
								/* Yes -> Already found a block ? */
								if (Found_block)
								{
									/* Yes -> Is that one more specific than this one ? */
									if ((Found_block->Byte_3 != 0xFF) ||
									 (Found_block->Word_6 != 0xFF))
										continue;
								}
	...
	if (First_block_nr == 0xFFFF)
	{
		...
		for (i=0;i<Nr_entries;i++)
		{
			...
			if ((Entry_data[i].X == X) && (Entry_data[i].Trigger_modes
				& (1 << ACTION_TRIGGER)) && (Entry_data[i].First_block_nr != 0xFFFF))
			{
				/* Yes -> Get event chain index */
	...
	Chain_nr = Get_event_chain_nr((UNBYTE *) Map_ptr, First_block_nr);
...
Handle_dialogue_set(UNSHORT Value)
{
	...
	if (!Character_capable(Dialogue_char_handle))
		return;
	...
	{
		/* Search event set */
		i = Search_event_set(Dialogue_event_set_handles[j], ~(FRIEND_ACTOR_FLAG | ENEMY_ACTOR_FLAG));
		...
		if (i != 0xFFFF)
		{
			...
			/* Prepare it */
			Prepare_event_set_context(Dialogue_char_handle, i, Context,
...
Search_member_sets(UNSHORT Member_nr, UNSHORT Actor_type)
{
	...
	{
		/* Search event set */
		i = Search_event_set(Party_event_set_handles[Member_nr-1][j], Actor_type);
		...
		if (i != 0xFFFF)
		{
			...
			/* Prepare it */
			Prepare_event_set_context(Party_char_handles[Member_nr-1], i, Context,
			 Character_context_validator);
			Context->Text_handle = Party_event_text_handles[Member_nr-1][j];
			Context->Source_data.Set_source.Char_type = PARTY_CHAR_TYPE;
			Context->Source_data.Set_source.Char_index = Member_nr - 1;
 ...
 *             - It will use the action parameters from the current event action.
...
Execute_event_chain(void)
{
	...
	if (i != 0xFFFF)
	{
		...
		for(;;)
		{
			...
			/* Break event chain ? */
			if (Context->Flags & BREAK_EVENT_CHAIN)
				/* Yes */
				break;
 ...
 UNSHORT Trigger_mode, UNSHORT Value)
{
	...
	if (NPC_index == 0xFFFF)
	{
		First_block_nr = Search_triggered_event(X, Y, 0xFFFFFFFF, Trigger_mode);
	}
	else
	{
		First_block_nr = Search_triggered_event(X, Y, ~(1 << NPC_index),
		 Trigger_mode);
	}
	...
	Context->Chain_nr = Get_event_chain_nr(Map_data, First_block_nr);
	MEM_Free_pointer(Map_handle);
 ...
 * INPUTS    : UNSHORT X - Map X-coordinate (0 for global event).
 *             UNSHORT Y - Map Y-coordinate (0 for global event).
 *             UNLONG NPC_list - Bit-list of NPCs that may be included in the
 *              search procedure.
 *             UNSHORT Trigger_mode - Trigger mode.
...
UNSHORT
Search_triggered_event(UNSHORT X, UNSHORT Y, UNLONG NPC_list,
 UNSHORT Trigger_mode)
{
	...
	/* Search NPC events */
	Value = Search_triggered_NPC_event(X, Y, NPC_list, Trigger_mode);
...
UNSHORT
Search_triggered_NPC_event(UNSHORT X, UNSHORT Y, UNLONG NPC_list,
 UNSHORT Trigger_mode)
{
	UNLONG Bit_nr;
	UNSHORT Chain_nr;
	...
	if (NPC_list)
	{
		/* Yes -> Search NPCs for events */
		for(i=0;i<NPCS_PER_MAP;i++)
		{
			/* Anyone there / included in check ? */
			if ((NPC_present(i)) && (NPC_list & (1 << i)))
			{
				/* Yes -> Right coordinates / right trigger mode / has event ? */
				if ((VNPCs[i].Map_X == X) && (VNPCs[i].Map_Y == Y) &&
				 ...
				 (VNPCs[i].First_block_nr != 0xFFFF))
				{
					...
					Chain_nr = Get_event_chain_nr(Map_ptr, First_block_nr);
...
UNSHORT
Get_event_triggers(UNSHORT X, UNSHORT Y, UNLONG NPC_list)
{
	UNSHORT Value;
	...
	/* Check NPC events */
	Value = Get_NPC_event_triggers(X, Y, NPC_list);
...
UNSHORT
Get_NPC_event_triggers(UNSHORT X, UNSHORT Y, UNLONG NPC_list)
{
	struct Event_block *Block;
	UNLONG Bit_nr;
	UNSHORT Value = 0;
 ...
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - The map must be initialized!
 *             - NPCs will also be checked.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
			...
			if (NPC_present(i))
			{
				 ...
				 (VNPCs[i].First_block_nr != 0xFFFF))
				{
					...
					if (!Read_bit_array(EVENT_SAVE_BIT_ARRAY, Bit_nr))
					{
						...
						Context->Text_handle = Map_text_handle;
						Context->Chain_nr =
						 Get_event_first_block_nr(Map_ptr, First_block_nr);
						Context->Chain_block_nr = First_block_nr;
						Context->Block_nr = First_block_nr;
						Context->Max_blocks = Nr_event_blocks;
						Context->Base = Event_data_offset;
			...
			for (j=0;j<Nr_entries;j++)
			{
				 ...
				 (Entry_data[j].First_block_nr != 0xFFFF))
				{
					...
					if (!Read_bit_array(EVENT_SAVE_BIT_ARRAY, Bit_nr))
					{
						...
						Context->Source_data.Map_source.Y = i;
		...
		for (j=0;j<Nr_entries;j++)
		{
			...
			if (Entry_data[j].Trigger_modes & (1 << Trigger_mode))
			{
				...
				if (Catalogue)
				{
					...
					Catalogue[Counter].First_block_nr =
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Character_context_validator
 * FUNCTION  : Determine if a character's event context is still valid.
 * FILE      : EVELOGIC.C
 * AUTHOR    : Jurie Horneman
...
BOOLEAN
Character_context_validator(struct Event_context *Context)
{
	BOOLEAN Result = TRUE;
	UNSHORT Index;

	/* Get character index */
	Index = Context->Source_data.Set_source.Char_index;
	...
	switch (Context->Source_data.Set_source.Char_type)
	{
		...
		{
			/* Anyone there ? */
			if (PARTY_DATA.Member_nrs[Index-1])
			{
				/* Yes -> Capable ? */
				Result = Character_capable(Party_char_handles[Index-1]);
			}
			else
			{
				/* No */
 ...
 * LAST      : 28.04.95 13:43
 * INPUTS    : UNBYTE *Map_data - Pointer to start of map data.
 *             UNSHORT Chain_nr - Event chain number.
...
UNSHORT
Get_event_first_block_nr(UNBYTE *Map_data, UNSHORT Chain_nr)
{
	UNSHORT i, *Ptr;

	Ptr = (UNSHORT *)(Map_data + Event_chain_offset);

	i = (_3D_map) ? EVENTS_PER_3D_MAP : EVENTS_PER_MAP;
 ...
 * LAST      : 30.01.95 12:20
 * INPUTS    : UNBYTE *Map_data - Pointer to start of map data.
 *             UNSHORT First_block_nr - Number of first event block.
...
UNSHORT
Get_event_chain_nr(UNBYTE *Map_data, UNSHORT First_block_nr)
{
	UNSHORT i, j, *Ptr;

	Ptr = (UNSHORT *)(Map_data + Event_chain_offset);

	j = (_3D_map) ? EVENTS_PER_3D_MAP : EVENTS_PER_MAP;
...
{
	struct Map_event_entry *Entry_data, *Output = NULL;
	UNSHORT Nr_entries = 0, i;
	UNBYTE *Ptr;
...
Get_event_chain_nr_at_position(UNBYTE *Map_data, SISHORT X, SISHORT Y)
{
	...
	if (Entry)
		return(Get_event_chain_nr(Map_data, Entry->First_block_nr));
	else
		return(0xFFFF);
}
		...
		case SET_EVENT_SOURCE:
		{
			...
			switch(Context->Source_data.Set_source.Char_type)
			{
				case PARTY_CHAR_TYPE:
				{
					...
					xprintf(&Diag_OPM, 10, 20, "Index : %u",

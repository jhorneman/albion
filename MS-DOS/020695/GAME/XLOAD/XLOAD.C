...
BOOLEAN XLD_Do_load_batch(struct XFile_type *Xftype, UNSHORT Number,
...
MEM_HANDLE XLD_Load_unpacked_subfile(struct XFile_type *Xftype,
 UNSHORT Subfile_nr);

BOOLEAN XLD_Open_library(struct XFile_type *Xftype, UNSHORT Subfile_nr);
...
XLD_Save_file(MEM_HANDLE Handle, struct XFile_type *Xftype)
{
	...
	if (ID>=0)
	{
		...
		Write = DOS_Write(ID, Pointer, (UNLONG) Length);
...
XLD_Open_library(struct XFile_type *Xftype, UNSHORT Subfile_nr)
{
	...
	Filename = Xftype->Filename;
	Length = strlen(Xftype->Filename);
	sprintf(Number, "%u", Subfile_nr / 100);
	*(Filename + Length - 5) = *Number;

...
Update_Party_member_object(struct Object *Object, union Method_parms *P)
{
	...
	if (PARTY_DATA.Member_nrs[Party_member->Number - 1])
	{
		...
		if (Char->Life_points)
		{
			...
			LP_bar_size = max((Char->Life_points * 20) /
			 (Char->Life_points_maximum + Char->Life_points_magic), 1);
		}
		else
		{
			/* No -> No bar */
		...
		if (Char->Spell_points)
		{
			...
			SP_bar_size = max((Char->Spell_points * 20) /
			 (Char->Spell_points_maximum + Char->Spell_points_magic), 1);
		}
		else
		{
			/* No -> No bar */
...
Update_help_line(struct Object *Object, union Method_parms *P)
{
	...
	/* Update children of this object */
...
Draw_Permanent_text_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Permanent_text->Counter)
	{
		...
		Display_processed_text(&Main_OPM, &(Permanent_text->Processed));
		Pop_PA();
	}
}

...
Add_shadow_to_COMOB(struct COMOB *Casting_COMOB)
{
	...
	Behaviour_data = Add_COMOB_behaviour(Shadow_COMOB, Shadow_handler);
	Behaviour_data->Shadow_data.Casting_COMOB = Casting_COMOB;
}
#endif
...
Do_fireball(struct Combat_participant *Part)
{
	...
	for (i=0;i<40;i++)
	{
		...
		for (j=0;j<5;j++)
		{
			...
			COMOB->X_3D = Trailed_COMOB->X_3D;
			COMOB->Y_3D = Trailed_COMOB->Y_3D;
			COMOB->Z_3D = Trailed_COMOB->Z_3D;
			...
			COMOB->Display_width = (Trailed_COMOB->Display_width *
			 (100 - (j * 15))) / 100;
			COMOB->Display_height = (Trailed_COMOB->Display_height *
			 (100 - (j * 15))) / 100;

			COMOB->Graphics_handle = Trailed_COMOB->Graphics_handle;

			COMOB->Nr_frames = Trailed_COMOB->Nr_frames;

//			COMOB->Frame = (Trailed_COMOB->Frame + 1) % COMOB->Nr_frames;

			Behaviour_data = Add_COMOB_behaviour(COMOB, Trailed_COMOB,
			 Trail_handler);

			Behaviour_data->Trail_data.Minimum_distance = 1 * COMOB_DEC_FACTOR;
			Behaviour_data->Trail_data.Maximum_distance = 40 * COMOB_DEC_FACTOR;

...
BOOLEAN Party_could_not_flee = FALSE;
BOOLEAN End_combat_flag = FALSE;

static UNSHORT Combat_palette_nr;

static UNLONG Combat_animation_timer;
static UNSHORT Combat_remaining_ticks;
static UNSHORT Combat_animation_period = 10;
...
MEM_HANDLE Tactical_icons_handle;
...
static struct Combat_event Combat_event_list[6 + MONSTERS_PER_GROUP + 1];
...
Combat_ModInit(void)
{
	struct Event_action Combat_start_action =
	{
		0,	NO_ACTOR_TYPE, 0,
	...
	Combat_background_handle = Load_subfile(COMBAT_BACKGROUND, Combat_background_number);
	...
	/* Load tactical icons */
	Tactical_icons_handle = Load_file(TACTICAL_ICONS);
	if (!Tactical_icons_handle)
	{
		Error(ERROR_FILE_LOAD);
	}
	...
	Init_monster_combat_data(Monster_group_number);
...
Combat_ModExit(void)
{
	...
	MEM_Free_memory(Tactical_icons_handle);
...
Update_Combat_display(void)
{
	...
	if (Fighting)
	{
		...
		if (T >= Combat_animation_timer)
			Combat_remaining_ticks += T - Combat_animation_timer + 1;
		else
			Combat_remaining_ticks += Combat_animation_timer - T + 1;

		Combat_animation_timer = T;
		...
		if (Combat_remaining_ticks > Combat_animation_period)
		{
			/* Yes -> Update animations */
			Update_combat_animations();

			/* Count down */
			Combat_remaining_ticks -= Combat_animation_period;
		}
	...
	else
	{
		...
		Combat_animation_timer = SYSTEM_GetTicks();
...
Create_sorted_combat_event_list(void)
{
	...
	/* Insert environment in combat event list */
	...
	for (i=0;i<6;i++)
	{
		...
		if (PARTY_DATA.Member_nrs[i])
		{
			...
			if (!(Get_conditions(Party_char_handles[i]) & FIGHTING_MASK))
			{
				...
				Party_parts[i].Tactical_icon_nr = PARTY_DATA.Member_nrs[i];

				Party_parts[i].Char_handle = Party_char_handles[i];

				/* Get battle order position */
				Position = PARTY_DATA.Battle_order[i] + (3 * NR_TACTICAL_COLUMNS);
...
{
	static UNSHORT Monster_show_sets[] = {
		2, 1, 1
	};
	...
	for (i=0;i<MONSTERS_PER_GROUP;i++)
	{
		...
		if (Monster_group[i])
		{
			...
			Monster_parts[Index].Show_set_index =
			 Monster_show_sets[Monster_group[i] - 1];
...
Clone_monster_data(MEM_HANDLE Original_data_handle)
{
	...
	/* Enter LP maximum */
	Char->Life_points_maximum = Value;

	/* Randomize LP value */
	Value = (Value * (100 + (rand() % 11) - 5)) / 100;

	/* Clip LP value */
	Value = max(min(Value, 0), Char->Life_points_maximum);

	/* Store new LP value */
	Char->Life_points = Value;

	/* Get current SP value */
	Value = Char->Spell_points;

	/* Enter SP maximum */
	Char->Spell_points_maximum = Value;

	/* Randomize SP value */
	Value = (Value * (100 + (rand() % 11) - 5)) / 100;

	/* Clip SP value */
	Value = max(min(Value, 0), Char->Spell_points_maximum);

	/* Store new SP value */
	Char->Spell_points = Value;
...
Update_monster_animation(struct Combat_participant *Monster_part)
{
	...
	if (Anim_type != NO_COMANIM)
	{
		...
		Monster_part->Shadow_COMOB->Frame = New_frame * 2 + 1;
	}
}
			...
			if (Current)
			{
				...
				if (Current != Part)
				{
					...
					if (Current->Type == Part->Type)
					{
						...
						{
							/* Yes -> Mark target */
							Move_mask |= (1 << Current->Target.Move_target_data.Target_square_index);
						}
					}
				}
			}
 ...
 * LAST      : 11.05.95 20:22
 * INPUTS    : struct Combat_participant *Part - Pointer to monster's
 *              participant data.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - Death is NOT shown!
 *             - The monster's items, gold and food are put in the Apres pool.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
void
Kill_monster(struct Combat_participant *Part)
{
	struct Character_data *Char;
	UNSHORT i;
	...
	Put_gold_in_apres_pool(Char->Nr_gold_coins);
	Put_food_in_apres_pool(Char->Nr_food_rations);

	MEM_Free_pointer(Part->Char_handle);

	/* Destroy monster */
	Destroy_monster(Part);
}

/*
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Destroy_monster
 * FUNCTION  : Destroy monster.
 * FILE      : COMBAT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 11.05.95 20:38
 * LAST      : 11.05.95 20:38
 * INPUTS    : struct Combat_participant *Part - Pointer to monster's
 *              participant data.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - Death is NOT shown!
 *             - The monster's items, gold and food are NOT put in the Apres
 *              pool.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
void
Destroy_monster(struct Combat_participant *Part)
{
	struct Character_data *Char;

...
static struct BBPALETTE Faded_palette;

static struct BBCOLOR Base_colours[64];

/* Fade parameters */
UNSHORT Current_fade_first_colour;
UNSHORT Current_fade_nr_colours;
UNSHORT Current_fade_R;
UNSHORT Current_fade_G;
UNSHORT Current_fade_B;
UNSHORT Current_fade_percentage;
 ...
 * NAME      : Reset_palette_fading
 * FUNCTION  : Reset palette fading.
 * FILE      : COLOURS.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 29.10.95 18:37
 * LAST      : 29.10.95 18:37
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
...
void
Reset_palette_fading(void)
{
	/* Reset fade parameters */
	Current_fade_first_colour	= 0;
	Current_fade_nr_colours		= 0;
	Current_fade_R					= 0;
	Current_fade_G					= 0;
	Current_fade_B					= 0;
...
Init_palette(void)
{
	...
	/* Copy colours to palette and store for later */
	Ptr = MEM_Claim_pointer(Handle);
	for (i=0;i<64;i++)
	{
		Base_colours[i].red				= *Ptr;
		Palette.color[192 + i].red		= *Ptr++;

		Base_colours[i].green			= *Ptr;
		Palette.color[192 + i].green	= *Ptr++;

		Base_colours[i].blue				= *Ptr;
 		Palette.color[192 + i].blue	= *Ptr++;
	}
...
Load_palette(UNSHORT Palette_nr)
{
	...
	/* Copy base colours to palette */
	for (i=0;i<64;i++)
	{
		Palette.color[192 + i].red		= Base_colours[i].red;
		Palette.color[192 + i].green	= Base_colours[i].green;
 		Palette.color[192 + i].blue	= Base_colours[i].blue;
	}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Activate_palette
 * FUNCTION  : Fade the current palette and activate it.
 * FILE      : COLOURS.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 18.10.95 20:09
 * LAST      : 19.10.95 15:37
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - By centralising all palette fading at this spot and never
 *              fading the palette used by the rest of the game, I can ensure
 *              that the program never uses a faded palette.
 *             - This function is called by Switch_screens whenever the
 *              palette was changed and must be updated.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
Activate_palette(void)
{
	...
	{
		/* Yes -> Fade */
		for (i=Current_fade_first_colour;i < Current_fade_first_colour +
		 Current_fade_nr_colours;i++)
		{
			/* Get the current colour */
			R = Faded_palette.color[i].red;
			G = Faded_palette.color[i].green;
			B = Faded_palette.color[i].blue;

			/* Interpolate towards the target colour */
			R = ((R - Current_fade_R) * (100 - Current_fade_percentage)) /
			 100 + Current_fade_R;
			G = ((G - Current_fade_G) * (100 - Current_fade_percentage)) /
			 100 + Current_fade_G;
			B = ((B - Current_fade_B) * (100 - Current_fade_percentage)) /
			 100 + Current_fade_B;

			/* Store the new colour in the current palette */
			Faded_palette.color[i].red		= R;
			Faded_palette.color[i].green	= G;
			Faded_palette.color[i].blue	= B;
		}
	}

	/* "Improvement" in PCLIB32 */
	global_screenport.screenpalptr = &Faded_palette;
 ...
 * FUNCTION  : Protect the cycled colours in the current palette.
 * FILE      : COLOURS.C
 * AUTHOR    : Jurie Horneman
...
void
Protect_colours(void)
{
	/* Clear all colour protections */
 ...
 UNSHORT Target_G, UNSHORT Target_B, UNSHORT Percentage)
{
	...
	Current_fade_first_colour	= Start;
	Current_fade_nr_colours		= Size;
	Current_fade_R 				= Target_R;
	Current_fade_G 				= Target_G;
	Current_fade_B 				= Target_B;
	Current_fade_percentage 	= Percentage;
 ...
 * RESULT    : BOOLEAN : The palette was changed.
 * BUGS      : No known.
 * NOTES     : - It is the responsibility of the calling function to make
 *              sure the palette is updated and faded.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */

...
UNLONG Restore_Item_list_object(struct Object *Object, union Method_parms *P);
...
static struct Method Item_list_methods[] = {
	...
	{ RESTORE_METHOD,	Restore_Item_list_object },
...
Init_Item_list_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Scroll bar needed ? */
	if ((Item_list->Slots_width * Item_list->Slots_height) <
	 Item_list->Nr_items)
	{
		...
		(
			Object->Self,
			(Item_list->Slots_width * ITEM_SLOT_OUTER_WIDTH) + BETWEEN +
			 SCROLL_BAR_WIDTH - ITEM_SLOT_BETWEEN_X + 2,
		...
		/* Add object */
		Item_list->Scroll_bar_object = Add_object
		(
			Object->Self,
			&Scroll_bar_Class,
			(UNBYTE *) &Scroll_bar_OID,
			Object->Rect.width - BETWEEN - 2,
			0,
			SCROLL_BAR_WIDTH,
			Object->Rect.height - 1
		);
	}
	else
	{
		...
		(
			Object->Self,
			(Item_list->Slots_width * ITEM_SLOT_OUTER_WIDTH) -
			 ITEM_SLOT_BETWEEN_X + 2,
			(Item_list->Slots_height * ITEM_SLOT_OUTER_HEIGHT) + 2
		);
	...
	for (i=0;i<Item_list->Slots_height;i++)
	{
		for (j=0;j<Item_list->Slots_width;j++)
		{
			...
			(
				Object->Self,
				Item_list->Item_slot_class_ptr,
				(UNBYTE *) &Item_slot_OID,
				j * ITEM_SLOT_OUTER_WIDTH + 1,
				i * ITEM_SLOT_OUTER_HEIGHT + 1,
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Restore_Item_list_object
 * FUNCTION  : Restore method of Item list object.
 * FILE      : ITEMLIST.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 26.09.95 21:06
 * LAST      : 27.09.95 11:38
 * INPUTS    : struct Object *Object - Pointer to object.
 *             union Method_parms *P - Method parameters.
 * RESULT    : UNLONG : 0.
...
UNLONG
Restore_Item_list_object(struct Object *Object, union Method_parms *P)
{
	struct Item_list_object *Item_list;
	struct BBRECT *Rect;
	struct BBRECT Old;
	...
	/* Install clip area */
	memcpy(&Old, &(Current_OPM->clip), sizeof(struct BBRECT));
	memcpy(&(Current_OPM->clip), Rect, sizeof(struct BBRECT));
	...
	/* Restore clip area */
	memcpy(&(Current_OPM->clip), &Old, sizeof(struct BBRECT));
 ...
 * RESULT    : UNLONG : 0.
 * BUGS      : No known.
 * NOTES     : - This function works for virtually any variation of the
 *              Item slot object, as it does not directly call any other
 *              functions.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
		...
		if (Item_data->Nr_frames > 1)
		{
			/* Yes -> Time to animate ? */
			{
				...
				if (Feedback_object == Object->Self)
				{
					...
					Execute_method(Object->Self, FEEDBACK_METHOD, P);
				}
				else
				{
					...
					if (Highlighted_object == Object->Self)
					{
						...
						Execute_method(Object->Self, HIGHLIGHT_METHOD, P);
					}
					else
					{
						/* No -> Re-draw item */
						Execute_method(Object->Self, DRAW_METHOD, P);
					}
				}
			}
		}
...
Pop_up_Item_slot_object(struct Object *Object, union Method_parms *P)
{
	...
	struct Item_slot_object *Item_slot;
	struct Item_packet Packet;
	struct Item_descriptor *Item_selection_data;
	UNSHORT Index;
	...
	if (Item_list->Menu)
	{
		...
		if (!Packet_empty(&Packet))
		{
			...
			/* Set item data */
			Item_selection_data = (struct Item_descriptor *) Item_list->Menu->Data;
			Item_selection_data->Slots_handle		= Item_list->Slots_handle;
			Item_selection_data->Slots_offset		= Item_list->Slots_offset;
			Item_selection_data->Item_slot_index	= Index;
...
Help_Item_slot_object(struct Object *Object, union Method_parms *P)
{
	...
	((Current_drag_drop_data.Data_ID == BODY_ITEM_DD_DATA_ID) ||
	 ...
	 (Current_drag_drop_data.Data_ID == APRES_ITEM_DD_DATA_ID)))
	{
		...
		if (Packet_empty(&Packet))
		{
			...
			_bprintf(String, 100, System_text_ptrs[517], Item_name);
		...
		else
		{
			...
			else
			{
				...
				/* Make help line string */
				_bprintf(String, 100, System_text_ptrs[518], Item_name2, Item_name);
	...
	else
	{
		...
		if (!Packet_empty(&Packet))
		{
			...
			/* Make help line string */
			_bprintf(String, 100, System_text_ptrs[22], Item_name);
 ...
 *             - This function can handle item lists with scroll bars.
 * #FUNCTION HEADER END#
 */
...
Find_item_slot_object(UNSHORT Item_list_object, UNSHORT Number)
{
	struct Item_list_object *Item_list;
	struct Item_slot_object *Item_slot;
	UNSHORT Item_slot_number;
	UNSHORT Handle;

	/* Get item list data */
	Item_list = (struct Item_list_object *) Get_object_data(Item_list_object);

	/* Calculate item slot number */
	Item_slot_number = Number;
	if (Item_list->Scroll_bar_object)
	{
		Item_slot_number -= (UNSHORT) Execute_method(Item_list->Scroll_bar_object,
		 GET_METHOD, NULL);
	}
	...
	while (Handle)
	{
		...
		if (Item_slot->Number == Item_slot_number)
		{
			/* Yes -> Exit */
			return Item_slot;
		}
 ...
 UNSHORT Frame)
{
	...
	if (Packet->Quantity > 1)
	{
		...
		/* Is the quantity 255 ? */
		if (Packet->Quantity == 255)
		{
			/* Yes -> Then this must be a merchant slot with infinite items */
		...
		{
			/* No -> Print quantity */
			_bprintf(Number, 4, "%3u", Packet->Quantity);
			Print_centered_string
			(
				OPM,
				X,

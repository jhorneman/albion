 ...
 * NAME      : Select_party_member_item
 * FUNCTION  : Select an item from a party member's inventory.
 * FILE      : ITEMSEL.C
...
UNSHORT
Select_party_member_item(UNSHORT Member_index, UNCHAR *Text,
 Item_evaluator Evaluator)
{
	...
	for (i=0;i<ITEMS_ON_BODY;i++)
	{
		...
		if ((i != TAIL-1) ||
		 (Get_race(Party_char_handles[Member_index - 1]) == ISKAI_RACE))
		{
			...
			if (!Packet_empty(&(Char->Body_items[i])))
			{
				...
				memcpy
				(
					(UNBYTE *) &Slots[Counter],
					(UNBYTE *) &(Char->Body_items[i]),
...
Draw_ItemSel_object(struct Object *Object, union Method_parms *P)
{
	...
	if (ItemSel_window->Text)
	{
		...
		PA.Area.left	= 14;
		PA.Area.top		= Object->Rect.height - 40;
		PA.Area.width	= 185;
		PA.Area.height	= 28;

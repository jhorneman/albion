...
#define MODIFICATIONS_PER_LIST	(1000)
	...
	*First_list_handle_ptr = Handle;
	...
	if (Nr_modifications % MODIFICATIONS_PER_LIST)
	{
		...
		(
			...
			Data,
			(Nr_modifications % MODIFICATIONS_PER_LIST) *
 ...
 UNSHORT Nr_modifications)
{
	...
	{
		Index = i % MODIFICATIONS_PER_LIST;

		/* In the right map ? */
		if (List->Modifications[Index].Map_nr == Map_nr)
		{
			/* Yes -> Make a modification */
			Do_change_icon
			(
				(UNSHORT) List->Modifications[Index].X,
				(UNSHORT) List->Modifications[Index].Y,
				(UNSHORT) List->Modifications[Index].Type,
				(UNSHORT) List->Modifications[Index].Sub_type,
				(UNSHORT) List->Modifications[Index].Value
			);
		}
...
Clone_modifications(UNSHORT Source_map_nr, UNSHORT Target_map_nr)
{
	...
	for (i=0;i<Nr_permanent_modifications;i++)
	{
		...
		/* In the source map ? */
		if (List->Modifications[Index].Map_nr == Source_map_nr)
		{
			/* Yes -> Make same modification in target map */
			Add_permanent_modification
			(
				List->Modifications[Index].X,
				List->Modifications[Index].Y,
				List->Modifications[Index].Type,
				List->Modifications[Index].Sub_type,
				List->Modifications[Index].Value,
				Target_map_nr
			);
		}
 ...
 UNSHORT Sub_type, UNSHORT Value, UNSHORT Map_nr)
{
	...
	/* Search for and remove this modification */
	Check_for_double_permanent_modification
	(
		X,
		Y,
		Type,
	...
	/* Find the last modification list */
	for (i=0;i<(Nr_permanent_modifications / MODIFICATIONS_PER_LIST);i++)
	{
		Handle2 = List->Next_list;
		MEM_Free_pointer(Handle);
		Handle = Handle2;
	...
	Index = Nr_permanent_modifications % MODIFICATIONS_PER_LIST;

	List->Modifications[Index].X			= X;
	List->Modifications[Index].Y			= Y;
	List->Modifications[Index].Type		= Type;
	List->Modifications[Index].Sub_type	= Sub_type;
	List->Modifications[Index].Value		= Value;
	List->Modifications[Index].Map_nr	= Map_nr;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Check_for_double_permanent_modification
 * FUNCTION  : Check if a modification already exists in the permanent list,
 *              and remove it if this is the case.
 * FILE      : ICCHANGE.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 13.11.95 15:56
 * LAST      : 13.11.95 15:56
 * INPUTS    : UNSHORT X - X-coordinate.
 *             UNSHORT Y - Y-coordinate.
 *             UNSHORT Type - Modification type.
...
void
Check_for_double_permanent_modification(UNSHORT X, UNSHORT Y, UNSHORT Type,
 UNSHORT Sub_type, UNSHORT Map_nr)
{
	struct Modification_list *List;
	MEM_HANDLE Handle;
	MEM_HANDLE Handle2;
	...
	for (i=0;i<Nr_permanent_modifications;i++)
	{
		...
		if ((List->Modifications[Index].X		== X) &&
		 (List->Modifications[Index].Y			== Y) &&
		 (List->Modifications[Index].Type		== Type) &&
		 (List->Modifications[Index].Sub_type	== Sub_type) &&
		 (List->Modifications[Index].Map_nr		== Map_nr))
 ...
 UNSHORT Sub_type, UNSHORT Value, UNSHORT Map_nr)
{
	...
	/* Search for and remove this modification */
	Check_for_double_temporary_modification
	(
		X,
		Y,
		Type,
	...
	/* Find the last modification list */
	for (i=0;i<(Nr_temporary_modifications / MODIFICATIONS_PER_LIST);i++)
	{
		Handle2 = List->Next_list;
		MEM_Free_pointer(Handle);
		Handle = Handle2;
	...
	/* Insert new modification */
	Index = Nr_temporary_modifications % MODIFICATIONS_PER_LIST;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Check_for_double_temporary_modification
 * FUNCTION  : Check if a modification already exists in the temporary list,
 *              and remove it if this is the case.
 * FILE      : ICCHANGE.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 13.11.95 16:10
 * LAST      : 13.11.95 16:10
 * INPUTS    : UNSHORT X - X-coordinate.
 *             UNSHORT Y - Y-coordinate.
 *             UNSHORT Type - Modification type.
...
void
Check_for_double_temporary_modification(UNSHORT X, UNSHORT Y, UNSHORT Type,
 UNSHORT Sub_type, UNSHORT Map_nr)
{
	struct Modification_list *List;
	MEM_HANDLE Handle;

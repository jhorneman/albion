...
#define MAX_SUBFILES_PER_BATCH	(500)
...
#define XLDERR_TOO_MANY_FILES_IN_BATCH	(15)

#define XLDERR_MAX							(15)
...
static UNLONG XLD_Subfile_lengths[MAX_SUBFILES];
static UNLONG XLD_Subfile_length;
static UNLONG XLD_Subfile_offset;
static UNLONG XLD_Batch_offset;

static struct Batch_load_data Batch_data_buffer[MAX_SUBFILES_PER_BATCH];
...
UNLONG
XLD_Get_subfile_length(struct XFile_type *Xftype, UNSHORT Subfile_nr)
{
	...
	/* Get subfile length */
	Subfile_length = XLD_Subfile_lengths[Subsubfile_nr];
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : XLD_Open_subfile
 * FUNCTION  : Open a subfile from a library.
 * FILE      : XLOAD.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 12.09.95 10:51
 * LAST      : 15.09.95 17:19
 * INPUTS    : struct XFile_type *Xftype - Pointer to XLD file type.
 *             UNSHORT Subfile_nr - Subfile number (1...).
...
BOOLEAN
XLD_Open_subfile(struct XFile_type *Xftype, UNSHORT Subfile_nr,
 SISHORT *File_handle_ptr, UNLONG *File_base_offset_ptr)
{
	...
	/* Calculate seek offset to subfile
	  (this calculation is slightly different from the usual one) */
	XLD_Subfile_offset = sizeof(struct XLD_Library) +
	 (XLD_Library.Nr_subfiles * 4);
	for (i=0;i<Subsubfile_nr;i++)
	{
		XLD_Subfile_offset += XLD_Subfile_lengths[i];
	}

	/* Clear flag (this file must be closed manually) */
	XLD_Library_open = FALSE;

	/* Store return values */
	*File_handle_ptr			= XLD_Library_ID;
	*File_base_offset_ptr	= XLD_Subfile_offset;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : XLD_Close_subfile
 * FUNCTION  : Close a subfile from a library.
 * FILE      : XLOAD.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 12.09.95 10:59
 * LAST      : 15.09.95 17:19
...
void
XLD_Close_subfile(SISHORT File_handle)
{
	/* Close the file */
 ...
 UNSHORT *Subfile_list, MEM_HANDLE *Handle_list)
{
	...
	if (Number == 1)
	{
		...
		else
		{
			...
			if (*Handle_list == NULL)
			{
				/* No -> In full batch mode ? */
				if (XLD_Full_batch_mode)
				{
					/* Yes -> Tell calling function */
					Result = FALSE;
				}
			}
		}
	...
	/* Too many subfiles in batch ? */
	if (Number > MAX_SUBFILES_PER_BATCH)
	{
		/* Yes -> Report error */
		XLD_Error(XLDERR_TOO_MANY_FILES_IN_BATCH, Xftype, 0);
...
XLD_Open_library(struct XFile_type *Xftype, UNSHORT Subfile_nr)
{
	...
	/* Select subfile group */
	_bprintf(Number, 2, "%u", Subfile_nr / 100);
	*(Xftype->Filename + strlen(Xftype->Filename) - 5) = *Number;
...
XLD_Print_error(UNCHAR *buffer, UNBYTE *data)
{
	...
	if (Error->Subfile_nr)
	{
		...
		(
			...
			XLD_Error_strings[Error_code],
			Error->Xftype->Ftype.Name,
			Error->Subfile_nr
		);
	}
	else

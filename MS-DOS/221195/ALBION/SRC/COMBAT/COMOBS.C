...
#define COMOB_IS_LEGAL(a) COMOB_is_legal(a, __LINE__, __FILE__)

#else

#define COMOB_IS_LEGAL(a) COMOB_is_legal(a, 0, NULL)
...
BOOLEAN COMOB_is_legal(struct COMOB *COMOB, int Line_nr, char *Filename);
...
static MEM_HANDLE Trans_table_handle;

struct COMOB COMOB_table[MAX_COMOBS];
 ...
 * LAST      : 21.10.95 21:44
 * INPUTS    : UNSHORT Palette_nr - Combat palette index.
 * RESULT    : BOOLEAN : COMOB system was initialized successfully.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
Init_COMOB_system(UNSHORT Palette_nr)
{
	...
	{
		Star_gfx_handles[i] = NULL;
	}

	/* Clear spark handles */
	for (i=0;i<NR_COMBAT_SPARKS;i++)
	{
		Spark_gfx_handles[i] = NULL;
	}

	/* Clear COMOB table */
	BASEMEM_FillMemByte
	(
		(UNBYTE *) &COMOB_table[0],
		MAX_COMOBS * sizeof(struct COMOB),
		0
	);
	...
	BASEMEM_FillMemByte
	(
		(UNBYTE *) &COMOB_behaviour_list[0],
		MAX_COMOB_BEHAVIOURS * sizeof(struct COMOB_behaviour),
		0
	);

	/* Load stars */
	Result = Load_full_batch
	(
		COMBAT_GFX,
		NR_COMBAT_STARS,
		&(Star_gfx_batch[0]),
	...
	/* Load transparency tables for current palette */
	Trans_table_handle = Load_subfile(TRANS_TABLE, Palette_nr);
	if (!Trans_table_handle)
	{
		Error(ERROR_FILE_LOAD);
		return FALSE;
	}
...
Exit_COMOB_system(void)
{
	...
	for (i=0;i<MAX_COMOBS;i++)
	{
		...
		{
			/* Free graphics ? */
			if (COMOB_table[i].User_flags & COMOB_FREE_GFX_ON_DELETE)
			{
				/* Yes -> Any graphics loaded ? */
				if (COMOB_table[i].Graphics_handle)
				{
					/* Yes -> Free it */
					MEM_Free_memory(COMOB_table[i].Graphics_handle);
				}
			}

			/* Free special table ? */
			if (COMOB_table[i].User_flags & COMOB_FREE_SPECIAL_ON_DELETE)
			{
				/* Yes -> Any special table ? */
				if (COMOB_table[i].Special_handle)
				{
					/* Yes -> Free it */
					MEM_Free_memory(COMOB_table[i].Special_handle);
				}
			}
		}
 	}
	...
	/* Free transparency table */
	MEM_Free_memory(Trans_table_handle);
}
 ...
 *             - Although NULL pointers are illegal, they will be accepted
 *              and handled correctly by this function.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
Delete_COMOB(struct COMOB *COMOB)
{
	...
	/* Exit if NULL (and don't complain) */
	if (!COMOB)
		return;

	/* Exit if illegal */
	if (!COMOB_IS_LEGAL(COMOB))
		return;
	...
	/* Free special table ? */
	if (COMOB->User_flags & COMOB_FREE_SPECIAL_ON_DELETE)
	{
		/* Yes -> Any special table ? */
		if (COMOB->Special_handle)
		{
			/* Yes -> Free it */
			MEM_Free_memory(COMOB->Special_handle);

			/* Clear handle */
			COMOB->Special_handle = NULL;
		}
	}
...
Duplicate_COMOB(struct COMOB *Source_COMOB)
{
	...
	/* Legal input ? */
	if (!COMOB_IS_LEGAL(Source_COMOB))
	{
		/* No -> Return NULL */
		return NULL;
	}
 ...
 COMOB_behaviour_handler Handler)
{
	...
	/* Is the first COMOB legal ? */
	if (!COMOB_IS_LEGAL(First_COMOB))
	{
		/* No -> Exit */
		return NULL;
	}
...
Swap_COMOBs(UNLONG A, UNLONG B, UNBYTE *Data)
{
	...
	T = COMOB_sort_list[A];
	COMOB_sort_list[A] = COMOB_sort_list[B];
	COMOB_sort_list[B] = T;
}
...
{
	return(COMOB_sort_list[A]->Z_3D < COMOB_sort_list[B]->Z_3D);
}
...
Update_COMOBs(void)
{
	...
	for (i=0;i<Nr_COMOBs;i++)
	{
		...
		if (COMOB_IS_LEGAL(COMOB))
		{
			/* Yes -> Clear updated flag */
			COMOB->System_flags &= ~COMOB_UPDATED;
		}
	}
	...
	for (i=0;i<Nr_COMOBs;i++)
	{
		...
		if (COMOB_IS_LEGAL(COMOB))
		{
			...
			{
				/* Yes -> Already updated ? */
				if (COMOB->System_flags & COMOB_UPDATED)
				{
					continue;
				}
	...
	for (i=0;i<MAX_COMOB_BEHAVIOURS;i++)
	{
		...
		{
			/* Yes -> Is the first COMOB legal ? */
			if (COMOB_IS_LEGAL(COMOB_behaviour_list[i].First_COMOB))
			{
				...
				if (COMOB_behaviour_list[i].First_COMOB->System_flags & COMOB_ACTIVE)
				{
					...
					{
						/* Yes -> Is it active ? */
						if (!(COMOB_behaviour_list[i].Second_COMOB->System_flags & COMOB_ACTIVE))
							continue;
					}
...
Draw_COMOB(struct COMOB *COMOB)
{
	...
	/* Is the COMOB hidden ? */
	if (!(COMOB->System_flags & COMOB_HIDDEN))
	{
		/* No -> Get 3D coordinates */
		...
		if (Z_3D >= MINIMUM_Z)
		{
			...
			/* Is pixel or big pixel ? */
			if ((Draw_mode == PIXEL_COMOB_DRAWMODE) ||
			 (Draw_mode == BIG_PIXEL_COMOB_DRAWMODE))
			{
				/* Yes -> Act depending on draw mode */
				switch (Draw_mode)
				{
					/* Single pixel */
					case PIXEL_COMOB_DRAWMODE:
					{
						OPM_SetPixel
						(
							&Main_OPM,
			...
			{
				/* No -> Get graphics header */
				Gfx = (struct Gfx_header *) MEM_Claim_pointer(COMOB->Graphics_handle);

				/* Legal animation frame ? */
				if (COMOB->Frame >= Gfx->Nr_frames)
				{
					/* No -> Report error */
					Error(ERROR_ILLEGAL_COMOB_FRAME);

					/* Set frame to harmless value */
					COMOB->Frame = 0;
				}
				...
				#if FALSE
				/* Get X-mirroring flag */
				if (COMOB->User_flags & COMOB_X_MIRROR)
					X_mirror_flag = TRUE;
				else
					X_mirror_flag = FALSE;
				#endif
				...
				switch (Draw_mode)
				{
					...
					case COLOURING_COMOB_DRAWMODE:
					{
						...
						Put_zoomed_recoloured_silhouette
						(
							&Main_OPM,
							X_2D,
							Y_2D,
					...
					{
						Table = MEM_Claim_pointer(Trans_table_handle);

						Put_zoomed_transparent_block
						(
							&Main_OPM,
							X_2D,
							Y_2D,
						...
						MEM_Free_pointer(Trans_table_handle);
						break;
					}
					/* Luminant */
					case LUMINANCE_COMOB_DRAWMODE:
					{
						Table = MEM_Claim_pointer(Trans_table_handle) +
						 LUMINANCE_TABLE_OFFSET;
					...
					/* Transparent, luminant */
					case TRANSLUMINANCE_COMOB_DRAWMODE:
					{
						Table = MEM_Claim_pointer(Trans_table_handle) +
						 TRANSLUMINANCE_TABLE_OFFSET;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_COMOB_graphics_ptr
 * FUNCTION  : Get pointer to COMOB source graphics.
 * FILE      : COMOBS.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 17.10.95 19:52
 * LAST      : 17.10.95 19:52
 * INPUTS    : struct COMOB *COMOB - Pointer to COMOB data.
 *             UNSHORT *Width_ptr - Pointer to width.
 *             UNSHORT *Height_ptr - Pointer to height.
 * RESULT    : UNBYTE * : Pointer to graphics of current frame.
 * BUGS      : No known.
 * NOTES     : - The COMOB's graphics handle must be freed after use!!!
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
UNBYTE *
Get_COMOB_graphics_ptr(struct COMOB *COMOB)
{
	struct Gfx_header *Gfx;
	UNSHORT i;
	UNBYTE *Ptr = NULL;
 ...
 SILONG *Top_3D_ptr, SILONG *Right_3D_ptr, SILONG *Bottom_3D_ptr)
{
	...
	{
		/* No -> Set safe coordinates */
		*Left_3D_ptr	= 0;
		*Top_3D_ptr		= 0;
		*Right_3D_ptr	= COMOB_DEC_FACTOR;
		*Bottom_3D_ptr	= -COMOB_DEC_FACTOR;
	}
}
...
Trail_handler(struct COMOB_behaviour *Behaviour)
{
	...
	/* Are legal COMOBs ? */
	if (COMOB_IS_LEGAL(Trailing_COMOB) &&
	 COMOB_IS_LEGAL(Trailed_COMOB))
	{
		/* Yes -> Calculate vector */
		...
		Length = (SILONG) sqrt(((double) dX_3D * (double) dX_3D) +
		 ((double) dY_3D * (double) dY_3D) + ((double) dZ_3D * (double) dZ_3D));
...
Shadow_handler(struct COMOB_behaviour *Behaviour)
{
	...
	/* Are legal COMOBs ? */
	if (COMOB_IS_LEGAL(Casting_COMOB) &&
	 COMOB_IS_LEGAL(Shadow_COMOB))
	{
		/* Yes -> Copy casting COMOB's coordinates to shadow COMOB */
...
Fairy_handler(struct COMOB_behaviour *Behaviour)
{
	...
	if (First_combat_update)
	{
		...
		/* Is a legal COMOB ? */
		if (COMOB_IS_LEGAL(Fairy_COMOB))
		{
			/* Yes -> Generate dust particles */
			for (i=0;i<2;i++)
			{
				/* Create COMOB */
				...
				Dust_COMOB->Special_handle = Trans_table_handle;
				Dust_COMOB->Special_offset = TRANSLUMINANCE_TABLE_OFFSET;
...
Dust_handler(struct COMOB_behaviour *Behaviour)
{
	...
	/*					New_COMOB->dY_3D = max(Dust_COMOB->dY_3D + COMOB_DEC_FACTOR,
						...
						New_COMOB->Special_handle = Trans_table_handle;
						New_COMOB->Special_offset = TRANSLUMINANCE_TABLE_OFFSET;
...
void
Animcontrol_handler(struct COMOB_behaviour *Behaviour)
{
	struct COMOB *COMOB;
...
Add_trail_to_COMOB(struct COMOB *Trailed_COMOB, UNSHORT Nr_trailing_COMOBs)
{
	...
	/* Trailed COMOB is illegal / number of COMOBs in the trail is zero ? */
	if (!COMOB_IS_LEGAL(Trailed_COMOB) || !Nr_trailing_COMOBs)
	{
		/* Yes -> Exit */
		return;
	}
 ...
 UNSHORT Halo_width, UNSHORT Halo_lifespan)
{
	...
	/* Source COMOB is illegal / number of COMOBs in the halo is zero /
	 halo width is zero ? */
	if (!COMOB_IS_LEGAL(Source_COMOB) || !Nr_halo_COMOBs || !Halo_width)
	{
		/* Yes -> Exit */
		return;
	}
	...
	for (i=0;i<Nr_halo_COMOBs;i++)
	{
		...
		if (!i)
		{
			...
			Halo_COMOB->Special_handle = Trans_table_handle;
			Halo_COMOB->Special_offset = TRANSLUMINANCE_TABLE_OFFSET;
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : COMOB_is_legal
 * FUNCTION  : Check if a COMOB is legal.
 * FILE      : COMOB.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 14.10.95 15:50
 * LAST      : 26.10.95 19:29
 * INPUTS    : struct COMOB *COMOB - Pointer to COMOB.
 *             int Line_nr - Line number of caller.
 *             char *Filename - Filename of caller.
 * RESULT    : BOOLEAN : COMOB is legal.
 * BUGS      : No known.
 * NOTES     : - If the COMOB is NULL it will be considered illegal.
 * SEE ALSO  : BBMEM.H
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
COMOB_is_legal(struct COMOB *COMOB, int Line_nr, char *Filename)
{
	/* Check if COMOB lies within COMOB array */
	if (((UNBYTE *) COMOB) < ((UNBYTE *) &COMOB_table[0]))
	{
		Report_error(ERROR_ILLEGAL_COMOB, Line_nr, Filename);
		return FALSE;
	}

	if (((UNBYTE *) COMOB) >= ((UNBYTE *) &COMOB_table[MAX_COMOBS]))
	{
		Report_error(ERROR_ILLEGAL_COMOB, Line_nr, Filename);
		return FALSE;
	}

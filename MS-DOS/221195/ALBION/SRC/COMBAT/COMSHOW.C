...
struct Projectile_data {
	UNSHORT To_LR_gfx_nr;						/* Towards the party */
	UNSHORT To_RL_gfx_nr;
	UNSHORT Fro_LR_gfx_nr;						/* From the party */
	UNSHORT Fro_RL_gfx_nr;
 ...
 union Combat_show_parms *P)
{
	...
	if (COMOB)
	{
		...
		COMOB->Display_width		= P->Damage_parms.Damage * 4 + 200;
 ...
 union Combat_show_parms *P)
{
	...
	Part->Graphics_handle = Load_subfile
	(
		MONSTER_GFX,
		(UNSHORT) Char->Monster_graphics_nr
	);
	...
	Tactical_icon_nr = MAX_PARTY_CHARS + (UNSHORT) Char->Monster_graphics_nr;
 ...
 union Combat_show_parms *P)
{
	...
	/* Get number of frames in walking animation */
	Nr_frames = Get_combat_animation_length(Part, WALK_ANIM);
 ...
 union Combat_show_parms *P)
{
	...
	Wait_4_half_combat_animation(Part);
...
Standard_monster_show_death(struct Combat_participant *Part,
 union Combat_show_parms *P)
{
	static const UNSHORT Explosion_Y_offsets[15] = {
 ...
 union Combat_show_parms *P)
{
	...
	struct Character_data *Char;
	UNSHORT Nr_frames;
	UNSHORT Corpse_frame;

	/* Duplicate monster COMOB */
	COMOB = Duplicate_COMOB(Part->Main_COMOB);

	/* Add shadow to duplicate */
	Add_shadow_to_COMOB(COMOB);

	/* Delete monster COMOB */
	Delete_COMOB(Part->Main_COMOB);

	/* Replace with duplicate */
	Part->Main_COMOB = COMOB;
	...
	/* Get corpse frame */
	Corpse_frame = (UNSHORT) Char->Anim_sequences[DIE_COMANIM][Nr_frames - 1] * 2;
	...
	/* Drop monster */
	while (Part->Main_COMOB->Y_3D > (-40 * COMOB_DEC_FACTOR))
	{
		/* Implement gravity */
		Part->Main_COMOB->dY_3D -= 16;
		Part->Main_COMOB->Y_3D += Part->Main_COMOB->dY_3D;

		/* Has the death animation ended ? */
		if (Part->Anim_type == NO_COMANIM)
		{
			/* Yes -> Set corpse frame */
			Part->Main_COMOB->Frame = Corpse_frame;
		}
	...
	/* Clear vector */
	Part->Main_COMOB->dY_3D	= 0;
	Part->Main_COMOB->Y_3D	= -40 * COMOB_DEC_FACTOR;
 ...
 UNSHORT Target_X, UNSHORT Target_Y, UNSHORT Weapon_slot_index)
{
	...
	if (COMOB)
	{
		...
		))
		{
			...
			if (COMOB->dZ_3D >= 0)
			{
				/* To -> Get projectile graphics number */
				Projectile_gfx_nr = Projectile_table[Projectile_type].To_RL_gfx_nr;
			}
			else
			{
				/* Fro -> Get projectile graphics number */
				Projectile_gfx_nr = Projectile_table[Projectile_type].Fro_RL_gfx_nr;
			}
		}
		else
		{
			...
			{
				/* To -> Get projectile graphics number */
				Projectile_gfx_nr = Projectile_table[Projectile_type].To_LR_gfx_nr;
			}
			else
			{
				/* Fro -> Get projectile graphics number */
				Projectile_gfx_nr = Projectile_table[Projectile_type].Fro_LR_gfx_nr;
			}
		}

		/* Load projectile graphics */
		Projectile_gfx_handle = Load_subfile(COMBAT_GFX, Projectile_gfx_nr);
 ...
 *             SILONG Target_Y_3D - Target 3D Y-coordinate.
 *             SILONG Target_Z_3D - Target 3D Z-coordinate.
 * RESULT    : BOOLEAN : TRUE : COMOB moves from right to left, FALSE : COMOB
 ...
 SILONG *Y_3D_ptr, SILONG *Z_3D_ptr)
{
	...
	else
	{
		...
		X_3D += (Target_width * COMOB_DEC_FACTOR) / 2;
		Y_3D -= (Target_height * COMOB_DEC_FACTOR) / 4;

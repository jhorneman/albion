...
void Check_party_member_magic_combat_action_targets(UNSHORT Member_index);
 ...
 * FUNCTION  : Custom keys method of Tactical window object.
 * FILE      : TACTICAL.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 20.10.95 20:27
 * LAST      : 20.10.95 20:27
 * INPUTS    : struct Object *Object - Pointer to object.
 *             union Method_parms *P - Method parameters.
...
Left_Tactical_square_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Tactical_select_mode)
	{
		...
		if (Tactical_select_mask & (1L << (Square->Y * NR_TACTICAL_COLUMNS +
		 Square->X)))
		{
			/* Yes -> Really clicked ? */
...
Help_Tactical_square_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Part)
	{
		...
		else
		{
			...
			/* Is party member OR monster LP are shown OR cheat mode ? */
			if ((Part->Type == PARTY_PART_TYPE) || Show_monster_LP_flag ||
			 Cheat_mode)
			{
				/* Yes -> Is party member / casting a spell ? */
				if ((Part->Type == PARTY_PART_TYPE) &&
				 (Part->Current_action == CAST_SPELL_COMACT))
				{
					/* Yes -> Build help line string with spell name */
					_bprintf
					(
						Help_string,
						200,
				...
				{
					/* No -> Build normal help line string */
					_bprintf
					(
						Help_string,
						200,
			...
			/* Is this a party member / any action / not in selection mode ? */
			if ((Part->Type == PARTY_PART_TYPE) &&
			 (Part->Current_action != NO_COMACT) &&
			 !Tactical_select_mode)
			{
				/* Yes -> Show targets */
				Highlight_part = Part;
		...
		switch (PUM_part->Type)
		{
			...
			case PARTY_PART_TYPE:
			{
				...
				else
				{
					...
					/* Does this character have magical items ? */
					if (!Character_has_magical_items(PUM_part->Char_handle))
					{
						/* No -> Cannot use magic items */
						PUMES[4].Flags |= PUME_ABSENT;
					}
				...
				if (Front_rows_empty)
				{
					/* Yes -> Has advanced already ? */
					if (Party_has_advanced)
					{
						/* Yes -> Block advance */
						PUMES[7].Flags |= PUME_BLOCKED;

						/* Explain */
						PUMES[7].Blocked_message_nr = 755;
					}
				}
				else
				{
...
PUM_Advance_tactical_square(UNLONG Data)
{
	...
	for (i=0;i<8;i++)
	{
		...
		{
			/* Anything there ? */
			if (COMOB_table[j].System_flags & COMOB_PRESENT)
			{
				/* Yes -> Advance it */
				COMOB_table[j].Z_3D -= COMBAT_SQUARE_DEPTH / 8;
			}
		}
	...
	/* Indicate the party has advanced */
	Party_has_advanced = TRUE;
}
	...
	if (T & (1L << 4))
	{
		...
		for (i=0;i<NR_TACTICAL_ROWS;i++)
		{
			...
			for (j=0;j<NR_TACTICAL_COLUMNS;j++)
			{
				...
				if (Tactical_select_mask & (1L << (i * NR_TACTICAL_COLUMNS + j)))
				{
					/* Yes -> Indicate */
					OPM_Box
					(
	...
	else
	{
		...
		for (i=0;i<NR_TACTICAL_ROWS;i++)
		{
			...
			for (j=0;j<NR_TACTICAL_COLUMNS;j++)
			{
				...
				if (Tactical_select_mask2 & (1L << (i * NR_TACTICAL_COLUMNS + j)))
				{
					/* Yes -> Indicate */
					OPM_Box
					(
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Check_party_combat_actions
 * FUNCTION  : Check the validity of the combat actions of all party members.
 * FILE      : TACTICAL.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 19:59
 * LAST      : 16.10.95 19:59
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
...
void
Check_party_combat_actions(void)
{
	UNSHORT i;
	...
	for (i=0;i<6;i++)
	{
		...
		{
			/* Yes -> Check */
			Check_party_member_combat_action(i + 1);
		}
	}
}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Check_party_member_combat_action
 * FUNCTION  : Check the validity of the combat actions of a party member.
 * FILE      : TACTICAL.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 19:08
 * LAST      : 19.10.95 23:42
...
void
Check_party_member_combat_action(UNSHORT Member_index)
{
	struct Combat_participant *Part;
	...
	/* Exit if this member doesn't exist */
	if (!Member_present(Member_index))
		return;

	/* Get participant data */
	Part = &Party_parts[Member_index - 1];

	/* Act depending on selected action */
	switch (Part->Current_action)
	{
		/* Move */
		case MOVE_COMACT:
		{
			...
			/* Get target square coordinates */
			Target_X = Target_square_index % NR_TACTICAL_COLUMNS;
			Target_Y = Target_square_index / NR_TACTICAL_COLUMNS;

			/* Is empty ? */
			if (Combat_matrix[Target_Y][Target_X].Part)
			{
				/* No -> Clear action */
				Part->Current_action = NO_COMACT;
			}
		...
		{
			/* Find defender */
			Target_X = Part->Target.Attack_target_data.Target_square_index %
			 NR_TACTICAL_COLUMNS;
			Target_Y = Part->Target.Attack_target_data.Target_square_index /
			 NR_TACTICAL_COLUMNS;
			...
			if (Defender_part)
			{
				...
				else
				{
					...
					{
						/* Yes -> Search the participant's body for long-range weapons */
						Weapon_slot_index = Search_body_for_item_type
						(
							Part->Char_handle,
							LONG_RANGE_IT
		...
		{
			/* Can cast magic ? */
			if (Get_conditions(Part->Char_handle) & MAGIC_MASK)
			{
				/* No -> Clear action */
				Part->Current_action = NO_COMACT;
			}
			else
			{
				...
				/* Get LP and SP required for selected spell */
				...
				{
					/* No -> Too much SP required ? */
					if (Required_SP && ((SP / Required_SP) < 1))
					{
						/* Yes -> Clear action */
						Part->Current_action = NO_COMACT;
					}
					...
					{
						/* No -> Check spell target */
						Check_party_member_magic_combat_action_targets(Member_index);
					}
				}
			}
			break;
		...
		{
			/* Can use magic item ? */
			if (Get_conditions(Part->Char_handle) & CONTROL_MASK)
			{
				/* No -> Clear action */
				Part->Current_action = NO_COMACT;
			}
			else
			{
				...
				/* Get packet where the magic item is supposed to be */
				Packet = &(Char->Body_items[Part->Target.Magic_target_data.Source_item_slot_index - 1]);
				...
				else
				{
					...
					if (Packet->Charges)
					{
						...
						{
							/* Yes -> Is this still the same spell ? */
							if (((UNSHORT) Item_data->Spell_class == Current_use_magic_data.Class_nr) &&
							 ((UNSHORT) Item_data->Spell_nr == Current_use_magic_data.Spell_nr))
							{
								/* Yes -> Check spell target */
								Check_party_member_magic_combat_action_targets(Member_index);
							}
							else
							{
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Check_party_member_magic_combat_action_targets
 * FUNCTION  : Check if the target of a party member's magical combat action
 *              is still valid.
 * FILE      : TACTICAL.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 19:43
 * LAST      : 16.10.95 19:43
 * INPUTS    : UNSHORT Member_index - Party member index (1...6).
 * RESULT    : None.
 * BUGS      : No known.
...
void
Check_party_member_magic_combat_action_targets(UNSHORT Member_index)
{
	struct Combat_participant *Casting_part;
	struct Combat_participant *Part;
	UNLONG Combat_target_mask;
	UNSHORT i, j;
	...
	/* Get participant data */
	Casting_part = &Party_parts[Member_index - 1];

	/* Get combat target mask */
	Combat_target_mask = Casting_part->Target.Magic_target_data.Combat_target_mask;

	/* Does this spell have any targets ? */
	if (Combat_target_mask)
	{
		/* Yes -> Check combat matrix */
		for (i=0;i<NR_TACTICAL_ROWS;i++)
		{
			for (j=0;j<NR_TACTICAL_COLUMNS;j++)
			{
				...
				if (Combat_target_mask & (1L << (i * NR_TACTICAL_COLUMNS + j)))
				{
					...
					/* Act depending on spell target mode */
					switch (Casting_part->Target.Magic_target_data.Combat_target_mode)
					{
						/* A participant */
						case PART_TARGMODE:
						{
							...
							if (!Part)
							{
								/* No -> Remove from target mask */
								Combat_target_mask &= ~(1L << (i * NR_TACTICAL_COLUMNS + j));
							}
							break;
						}
						/* Friend */
		...
		{
			/* No -> Clear action */
			Casting_part->Current_action = NO_COMACT;
		}
	}
}

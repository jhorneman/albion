...
#define C0_SPELL_6_DAMAGE		(12)
#define C0_SPELL_7_DAMAGE		(18)
#define C0_SPELL_8_DAMAGE		(27)
#define C0_SPELL_14_DAMAGE		(24)
#define C0_SPELL_20_DAMAGE		(120)
...
void show_samenkorn(UNSHORT Strength,SILONG zx,SILONG zy,SILONG zz)
{
	...
	Add_halo_to_COMOB_2(korn_COMOB,3,size,steps,0);
...
{
	struct COMOB *vp_COMOB = NULL;
	struct COMOB *hp_COMOB = NULL;
	SILONG zx,zy,zz;
	...
	/* Pflanzenteil hinten laden */
	hp_handle = Load_subfile(COMBAT_GFX, 35);
	if (!hp_handle){
		ERROR_PopError();
		return;
	}
	...
	if (Strength)
	{
		/* No -> Pflanze erscheint und wÑchst am Monster hoch */
...
{
	/* Show effect */
	Party_magic_effect(Victim_part, Strength);
...
//	UNSHORT Temp_strength;
	...
	for (j=0;j<C0_5_COMBAT_FIELDS;j++)
	{
		monsterhalo[j] = FALSE;
	...
	do
	{
		...
		for (i=0;i<Current_nr_target_parts;i++){
			...
			if(Victim_part->Main_COMOB->Z_3D>(wand_COMOB->Z_3D-500)
				...
				/* Halo zu Monster */
				if(!monsterhalo[i]){
					clist=Add_halo_to_COMOB_2(Victim_part->Main_COMOB, 3, 10, 0,CONNECT_POSITION);
					chalos[i][0]=clist[0];
					chalos[i][1]=clist[1];
					chalos[i][2]=clist[2];
					monsterhalo[i]=TRUE;
...
Do_C0_Spell_6(struct Combat_participant *Victim_part, UNSHORT Tactical_X,
 UNSHORT Tactical_Y, UNSHORT Strength)
{
	MEM_HANDLE Recolour_table_handle = NULL;
	UNBYTE *Recolour_table_ptr;
	SILONG zx,zy,zz;
	SILONG x,y,z,s,t,u;
	SILONG eisnr;
	struct COMOB *blitz_COMOB = NULL;
	struct COMOB *shape_COMOB = NULL;
//	struct COMOB *COMOB = NULL;
	SILONG left,right,top,bottom;
	SILONG nreis;
	SILONG nreissize;
	...
	Calculate_recolouring_table(Recolour_table_ptr, 0, 256, 160,160,255, 50);
			...
			for(u=0;u<2;u++)
			{
				for(s=0;s<Nr_combat_updates;s++)
				{
					for(t=0;t<nreis;t++)
					{
						...
						{
							bool=Test_COMOB_mask(Victim_part->Main_COMOB,x,y);
						}
						else
						{
							if(rndmm(1,12)>3)
...
leavelop1c06:
		...
		/* Blitz blitzt auf */
		blitz_COMOB=Gen_COMOB(x,y,z-5, 0, 10, blitz_handle, GC_FIRE);
		if(blitz_COMOB==NULL){
			ERROR_PopError();
			return;
		}
		...
		/* SizeII verhalten */
		behave = Add_COMOB_behaviour(blitz_COMOB,NULL, size_II_handler);
		if(!behave)
		{
			ERROR_PopError();
			return;
		...
		if (Victim_part->Type == MONSTER_PART_TYPE){
			...
			shape_COMOB=Duplicate_COMOB(Victim_part->Main_COMOB);
			if(shape_COMOB==NULL){
				ERROR_PopError();
				return;
			}
			shape_COMOB->User_flags|=COMOB_FREE_SPECIAL_ON_DELETE;
			...
			behave = Add_COMOB_behaviour(shape_COMOB,Victim_part->Main_COMOB, connect_handler);
			if(!behave){
				ERROR_PopError();
				return;
			}
			...
			behave->Data.Just_data.Data[1] = shape_COMOB->X_3D-Victim_part->Main_COMOB->X_3D;
			behave->Data.Just_data.Data[2] = shape_COMOB->Y_3D-Victim_part->Main_COMOB->Y_3D; /* verschieben, so das Halo von der Mitte des Objects aus berechnet wird */
			behave->Data.Just_data.Data[3] = shape_COMOB->Z_3D-Victim_part->Main_COMOB->Z_3D;
...
//		Play_sound(211);
		for(t=0;t<eisnr;t++){
			...
			behave->Data.Bounce_data.Gravity = (SISHORT) rndmm(10,20);
			behave->Data.Bounce_data.Bounce = (UNSHORT) rndmm(10,20);
		...
		(
	 	...
	 	/* Do damage to victim */
		Do_combat_damage(Victim_part, max((Strength * C0_SPELL_8_DAMAGE) /
		 100, 1));
	}
}
...
//			Play_sound(211);
			for(t=0;t<eisnr;t++){
				...
				behave->Data.Just_data_w.Data[0]=rndmm(-80,-40); /* jeden Tick um 1% */
				behave->Data.Just_data_w.Data[1]=behave->Data.Just_data_w.Data[0];
				behave->Data.Just_data_w.Data[2]=2; /* bis 25 % */
				behave->Data.Just_data_w.Data[3]=2;
			...
			(
	 		...
	 		/* Do damage to victim */
			Do_combat_damage(Victim_part, max((Strength * C0_SPELL_7_DAMAGE) /
			 100, 1));

			shape_COMOB = NULL;
			Recolour_table_handle = NULL;
		}
	}
...
//	struct COMOB_behaviour *behave_flash;
	...
	/* Korn explodiert */
	Gen_Sparks(x,5*COMOB_DEC_FACTOR,z,100/*amount*/,rndmm(50,100)/*speed*/,rndmm(25,35)/*life*/,rndmm(30,40)/*size*/,GEN_SPARK_TYP_BLUE);
	...
	if (Strength)
	{
		...
		do{
			...
			Recolour_palette(0,192,C0_10_FLASHRED,C0_10_FLASHGREEN,C0_10_FLASHBLUE,pal);
		...
		do{
			...
			if(s<0){
				if(Combat_display_detail_level>=DEFAULT_COMBAT_DISPLAY_DETAIL_LEVEL){
					Gen_Sparks(rndmm(left,right),0,z,30/*amount*/,100/*maxspeed*/,40/*life*/,30/*size*/,GEN_SPARK_TYP_BLUE);
					Gen_Sparks(rndmm(left,right),0,z,30/*amount*/,100/*maxspeed*/,40/*life*/,30/*size*/,GEN_SPARK_TYP_BLUE);
				}
				s+=10;
			}
		...
		Recolour_palette(0,192,C0_10_FLASHRED,C0_10_FLASHGREEN,C0_10_FLASHBLUE,0);
...
//	struct COMOB_behaviour *behave_flash;
	...
	for (j=0;j<C0_11_COMBAT_FIELDS;j++)
	{
		beam_COMOB[j] = NULL;
	}
	...
	if(nof_beams>0){
		...
		for(t=0;t<nof_beams;t++){
			...
			/* Set display parameters */
			beam_COMOB[t]->Draw_mode = TRANSLUMINANCE_COMOB_DRAWMODE;
...
//	struct COMOB_behaviour *behave_flash;
	...
	for (j=0;j<C0_12_COMBAT_FIELDS;j++)
	{
		beam_COMOB[j] = NULL;
	}
 ...
 UNSHORT Tactical_Y, UNSHORT Strength)
{
	...
	struct COMOB_behaviour *fairy_behave = NULL;
	struct COMOB_behaviour *osc_behave = NULL;
	struct COMOB_behaviour *behave = NULL;
	struct COMOB *COMOB = NULL;
	struct COMOB *fairy_COMOB = NULL;
	SILONG x,y,z,fx,fy,fz,w,h,t,upd;
	SILONG time,nextfairy,nexthalo,faries;
	// SILONG col;
	SILONG left,right,top,bottom;
	SILONG hw[3];
	...
	if (Strength)
	{
		/* No -> noch kurze Zeit warten wegen Sparks */
		update_n_times(16);

		fairy_COMOB = Gen_COMOB
		(
			x,
			top,
			z,
		...
		fairy_behave = Add_COMOB_behaviour
		(
			fairy_COMOB,
			NULL,
			Fairy_II_handler
		...
		osc_behave = Add_COMOB_behaviour
		(
			fairy_COMOB,
			NULL,
			Oscillate_handler
		...
		/* 0 passiert gar nichts, entspricht 360 Grad / x,
		   1-x = Periode einer SinusWelle wird x-mal unterteilt ) */
		osc_behave->Data.Oscillate_data.Period = 90;
		...
		do
		{
			...
			{
				if (time>(C0_13_MAXTIME-25)&&time<C0_13_MAXTIME)
				{
 ...
 UNSHORT Strength, struct COMOB *Trap_COMOB)
{
	//SILONG i;
	//UNBYTE *Ptr;
	SILONG x,y,tx,ty,tz,t,fv,frame,tframe,frames;
	// SILONG z, u;
	BOOLEAN leave;
	...
	struct COMOB *ast_COMOB = NULL;
	struct COMOB *source_COMOB = NULL;
	struct COMOB *scoms[C0_14_MAXTRAILS];
	MEM_HANDLE ast_handle = NULL;

	UNSHORT j;

	for (j=0;j<C0_14_MAXTRAILS;j++)
	{
		scoms[j] = NULL;
	}
	...
	for(t=0;t<anz_trails;t++){
		...
		scoms[t]->Draw_mode = TRANSLUMINANCE_COMOB_DRAWMODE;

 		source_COMOB=scoms[t];

	}
	frames = (SILONG) Get_nr_frames(ast_COMOB->Graphics_handle);
	...
	if (Strength)
	{
		...
		Do_combat_damage(Victim_part, max((Strength * C0_SPELL_14_DAMAGE) /
		 100, 1));
	...
	if (Is_trap(Tactical_X, Tactical_Y))
	{
		...
		do
		{
			...
			if(nextstar<0)
			{
				...
				/* SizeII verhalten */
				behave = Add_COMOB_behaviour(COMOB,NULL, size_II_handler);
				if(!behave){
					ERROR_PopError();
					return;
				}
...
//	BOOLEAN leave;
	...
	/* Grî·e des Pilzes */
	nrpilzsize= ((SILONG)max_victim_width*(SILONG)max_victim_height)*100/35000;
	...
	{
		/* No -> Calculate damage */
		Damage = max((Strength * C0_SPELL_20_DAMAGE) / 100, 1);
...
//			Play_sound(222);
			...
			/* Monster mit Pilzen zukleistern */
			pilznr=0;
			y=bottom;
			for(;;){
				...
				for(t=0;;){
					...
					for(u=0;u<3;u++){
						...
						for(s=0;s<Nr_combat_updates;s++){
							...
							pilz_COMOB[t]->Lifespan= (UNSHORT) rndmm(5,50);
				...
				for(t=0;;){
					...
					for(u=0;u<3;u++){
						...
						for(s=0;s<Nr_combat_updates;s++){
							...
							pilz_COMOB[t]->Lifespan= (UNSHORT) rndmm(5,80);

...
void ChangeDirAndDriveTo (UNCHAR *String)
{
	...
	if (Drive[0]>='A' && Drive[0]<='Z')
	{
      ...
      //Thus we have to re-check it (stoopid, isn't it ?)
		if (rc != Drive[0]-'A'+1)
		{
   ...
   //Nicht einfach nur ins Top wechseln ?
	if (strcmp (Dir, "\\") != 0)
	{
...
void CopyOneFile (UNCHAR *Source, UNCHAR *Target)
{
   ...
   OutFileHandle = open (Target, O_TRUNC|O_CREAT|O_WRONLY|O_BINARY,S_IWRITE);
	...
	while (1)
	{
      //Echte Fehler werden durch die harderror-Geschichte abgefangen:

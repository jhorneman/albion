	...
	do
	{
		...
		for (c=0; c<20; c++)
			...
			if (BrowseScript.Line[c+CursorY])
			{
				...
				if (strlen((char *) BrowseScript.Line[c+CursorY])>38)
					(BrowseScript.Line[c+CursorY])[38]=0;

				// Cast to UNCHAR * added - JH
				PrintTextInFrame ((UNCHAR *) BrowseScript.Line[c+CursorY],
										BUTTON_COLOR_TEXT,
										&SetupOpm,
				 						RESOLUTION_X/2-160+7+1,
...
void LoadBrowseFile (SCRIPT *Script, UNCHAR *Filename)
{
	...
	// Cast changed from UNCHAR** to SIBYTE ** - JH
	...
	for (i=0; i<ScriptFilelength; i++)
	{
		...
		if (Script->ScriptMem[i] == 13)
		{
			...
			// Brackets around strlen expression added - JH
			while ( (strlen((char *) Script->Line[Script->AnzLines-1]) > 0) &&
				  	( (Script->Line[Script->AnzLines-1])[strlen((char *) Script->Line[Script->AnzLines-1])-1] == 10 ||
				    	(Script->Line[Script->AnzLines-1])[strlen((char *) Script->Line[Script->AnzLines-1])-1] == 13 ||
				    	(Script->Line[Script->AnzLines-1])[strlen((char *) Script->Line[Script->AnzLines-1])-1] == 26))
			{
				//String um ein Zeichen kÅrzer machen:
				// Cast to char* added - JH
		 		(Script->Line[Script->AnzLines])[strlen((char *) Script->Line[Script->AnzLines-1])-1] = 0;
			}

...
void main (SILONG argc, SIBYTE *argv[])
{
	...
	switch (argc)
	{
		...
		case 2:
			// Cast to char* added - JH
			if (stricmp ((char *) argv[1], "/VGA")==0)
			{
				EnforceVGA=TRUE;
				ScriptFilename = "INSTALL.SCR";
			}
			// Cast to char* added - JH
			else ScriptFilename = (char *) argv[1];
			break;
		...
		case 3:
			...
			if (stricmp ((char *) argv[1], "/VGA")==0)
			{
				EnforceVGA=TRUE;
				// Cast to char* added - JH
				ScriptFilename = (char *) argv[2];
			}
			else
			{
				// Cast to char* added - JH
				if (stricmp ((char *) argv[2], "/VGA")==0)
				{
					EnforceVGA=TRUE;
					// Cast to char* added - JH
					ScriptFilename = (char *) argv[1];
				}
				else

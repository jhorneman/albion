...
void RefreshFixedPalette (void)
{
	...
	SISHORT Colour_index;
	UNSHORT Index;

	Index = 0;
	for (;;)
	{
		/* Get colour index from colour list */
		Colour_index = DefaultPal[Index].Color;

		/* Colour index -1 means end of table */
		if (Colour_index == -1)
			break;
		...
		(
			Colour_index,
			DefaultPal[Index].r * 255 / 100,
			DefaultPal[Index].g * 255 / 100,
			DefaultPal[Index].b * 255 / 100
		);

		/* Next entry in colour list */
		Index++;
	}
...
BOOL PrintTextInFrame (UNCHAR *Text, SILONG FontColor, struct OPM *pOpm, SILONG x1, SILONG y1, SILONG x2, SILONG y2)
{
	...
	for (c=0; c<l+1; c++)
	{
		...
		else
		{
			...
			// Added DeppVar calculation - JH
			if (x != x1 && WasWithinWord==FALSE)
				DeppVar = 1;
			else
				DeppVar = 0;

			if (PixelStringLength (Buffer)+PixelCharLength('-')*DeppVar > x2-x-1)
			{
			...
			for (d=0; d< (SILONG) strlen(Buffer); d++)
...
void ScaleCopyOpm (struct OPM *pSourceOpm, struct OPM *pTargetOpm)
{
	...
	for (ty=0; ty<pTargetOpm->height; ty++)
	{
		...
		{
			// Changed shift right by 10 to division by 1024 - JH
			pTarget[tx]=pSource[sx / 1024];
			sx+=addSX;
		}
	}
}

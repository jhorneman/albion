...
/*** Put masked Block simulieren */
static void Put_masked_block(struct OPM *opm, SILONG x, SILONG y, SILONG width, SILONG height, UNBYTE *gfxptr)
{
	struct OPM gfxopm;

	/* temporÑres OPM anlegen */
	if ( !OPM_New( width, height, 1, &gfxopm, gfxptr ) ){
		return;
	}

	/* OPM transparent */
	gfxopm.status|=OPMSTAT_TRANSPARENT;
	gfxopm.transcol=0;

	/* OPM zeichnen */
	OPM_CopyOPMOPM( &gfxopm, opm,0,0,width,height,x,y);
 ...
 UNBYTE *Graphics_ptr)
{
	...
	if (Source_width && Source_height && Target_width && Target_height)
	{
		...
		{
			/* Yes -> Is the target OPM too big ? */
			if ((OPM->width <= 800) && (OPM->height <= 600))
			{
				/* No -> Set clipping area */
				...
				/* Zoom block */
				_ZOOM_BLOCK(Graphics_ptr ,OPM->data, Source_width, Source_width,
				 Source_height, OPM->nextypos, Target_width, Target_height, X, Y,-1,ZOOMMODE_NORMAL);
...
//		{
			...
			if ((OPM->width <= 800) && (OPM->height <= 600))
			{
				...
				/* Zoom block */
				_ZOOM_BLOCK(Graphics_ptr ,OPM->data, Source_width, Source_width,
				 Source_height, OPM->nextypos, Target_width, Target_height, X, Y,Colour,ZOOMMODE_SILHOUETTE);
...
//		{
			...
			if ((OPM->width <= 800) && (OPM->height <= 600))
			{
				...
				/* Zoom block */
				_ZOOM_BLOCK(Graphics_ptr ,OPM->data, Source_width, Source_width,
				 Source_height, OPM->nextypos, Target_width, Target_height, X, Y,(SILONG)Recolouring_table,ZOOMMODE_RECOLOUR_BACK);
 ...
 *             SISHORT Target_height - Target height of block.
 *             UNBYTE *Graphics_ptr - Pointer to graphics.
 *             UNBYTE *Recolouring_table - Pointer to recolouring table.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - The core drawing function :
 *
...
//		{
			...
			if ((OPM->width <= 800) && (OPM->height <= 600))
			{
				...
				/* Zoom block */
				_ZOOM_BLOCK(Graphics_ptr ,OPM->data, Source_width, Source_width,
				 Source_height, OPM->nextypos, Target_width, Target_height, X, Y,(SILONG)Recolouring_table,ZOOMMODE_RECOLOUR);
...
//		{
			...
			if ((OPM->width <= 800) && (OPM->height <= 600))
			{
				...
				/* Zoom block */
				_ZOOM_BLOCK(Graphics_ptr ,OPM->data, Source_width, Source_width,
				 Source_height, OPM->nextypos, Target_width, Target_height, X, Y,(SILONG)Recolouring_table,ZOOMMODE_TRANSPARENT);

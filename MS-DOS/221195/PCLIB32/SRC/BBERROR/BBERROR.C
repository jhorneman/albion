...
/* (Size has been doubled because of a weak point in the BBERROR design:
	 the error output functions cannot assume the buffer given to them can
	 actually contain BBERROR_OUTSTRING_SIZE characters when the library
	 name is printed - JH) */

UNCHAR BBERROR_MessageString[ BBERROR_OUTSTRINGSIZE * 2];

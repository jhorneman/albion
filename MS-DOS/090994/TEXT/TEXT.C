...
{
	/* Pop all text styles currently on the stack */
	while (Textstyle_stack_index)
		Pop_textstyle();

	/* Terminate the last text style (which cannot be popped), if any */
	if (Textstyle_stack[0])
		Exit_textstyle();

	/* Push the default text style on the stack */
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Init_textstyle
 * FUNCTION  : Initialize the text style at the top of the textstyle stack.
 * FILE      : TEXT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 25.08.94 10:58
 * LAST      : 25.08.94 10:58
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
...
Init_textstyle(void)
{
	...
	if (Textstyle->Paper == 0xFFFF)
	{
		...
		Textstyle_background_handles[Textstyle_stack_index] =
		 MEM_Allocate_memory(PA->Area.width * PA->Area.height);
	}
}
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Exit_textstyle
 * FUNCTION  : Terminate the text style at the top of the textstyle stack.
 * FILE      : TEXT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 25.08.94 10:58
 * LAST      : 25.08.94 10:58
...
Exit_textstyle(void)
{
	...
	if (Textstyle->Paper == 0xFFFF)
	{
		/* Yes -> Destroy background buffer */
		MEM_Free_memory(Textstyle_background_handles[Textstyle_stack_index]);
	}
}
...
Reset_PA_stack(void)
{
	/* Pop all PA's currently on the stack */
	while (PA_stack_index)
		Pop_PA();

	/* Terminate the last PA (which cannot be popped), if any */
	if (PA_stack[0])
		Exit_PA(PA_stack[0]);

	/* Push the default PA on the stack */
	Init_PA(&Default_PA);
...
{
	Push_textstyle(&(PA->PA_textstyle));

...
void Divide_EP(UNLONG Added_experience_points);
void Clear_combat_conditions(void);
...
Enter_Combat(UNSHORT Monster_group_nr, UNSHORT Combat_background_nr)
{
	...
	switch (Combat_status)
	{
		case COMBAT_PARTY_WON:
		{
			...
			if (!Result)
			{
				/* No -> Is the current active member OK ? */
				Result = Activate_party_member(PARTY_DATA.Active_member);
 ...
 * LAST      : 16.08.95 21:10
 * INPUTS    : UNLONG Added_experience_points - Added experience points.
 * RESULT    : None.
 * BUGS      : No known.
...
void
Divide_EP(UNLONG Added_experience_points)
{
	...
	/* Count members eligible for experience points */
	Nr_members = 0;
	for (i=0;i<6;i++)
	{
		/* Anyone here ? */
	...
	if (Nr_members)
	{
		...
		EP_per_member = max(1, (Added_experience_points / Nr_members));

		/* Tell the player */
		sprintf(String, System_text_ptrs[552], EP_per_member);
		...
		for (i=0;i<6;i++)
		{
			...
			if (PARTY_DATA.Member_nrs[i])
			{
				...
				{
					/* Yes -> Experience */
					Set_EP(Party_char_handles[i], EP_per_member +
						Get_EP(Party_char_handles[i]));
				}
			}
		}
	}
...
Combat_ModInit(void)
{
	...
	Combat_palette_nr = Combat_palettes[Combat_background_number - 1];

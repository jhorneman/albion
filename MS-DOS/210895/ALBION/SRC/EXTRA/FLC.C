 ...
 * START    : 28-11-1994
 * PROJECT  : FLC animation recording and playback
 * NOTES    :
 * VERSION  : 1.0
 * SEE ALSO : FLC.H
 ************/
...
/* Flic recording support functions */
static UNBYTE *FLC_Create_COLOR_256_chunk(UNBYTE *Buffer);
static UNBYTE *FLC_Create_BYTE_RUN_chunk(UNBYTE *Buffer, UNBYTE *Frame);

static UNBYTE *FLC_Write_run_packet(UNBYTE *Ptr, UNBYTE Pixel,
 UNSHORT Length);
static UNBYTE *FLC_Write_unpacked_packet(UNBYTE *Ptr, UNBYTE *Run_ptr,
 UNSHORT Length);

static UNBYTE *FLC_Create_DELTA_FLC_chunk(UNBYTE *Buffer, UNBYTE *Frame,
 UNBYTE *Previous);

static UNSHORT FLC_Scan_line_for_delta_packets(UNSHORT *Old, UNSHORT *New,
 struct FLC_delta Output[]);
static UNBYTE *FLC_Write_delta_packet(UNBYTE *Buffer,
 struct FLC_delta *Packet);
static UNBYTE *FLC_Write_run_delta_packet(UNBYTE *Ptr, UNSHORT Skip_count,
 UNSHORT Word, UNSHORT Length);
static UNBYTE *FLC_Write_unpacked_delta_packet(UNBYTE *Ptr,
 UNSHORT Skip_count, UNSHORT *Run_ptr, UNSHORT Length);

/* Flic playback support functions */
static BOOLEAN FLC_Decode_frame_chunk(UNBYTE *Frame_chunk_ptr);

static void FLC_Decode_COLOR_256_chunk(UNBYTE *Ptr);
static void FLC_Decode_DELTA_FLC_chunk(UNBYTE *Ptr);
static void FLC_Decode_COLOR_64_chunk(UNBYTE *Ptr);
static void FLC_Decode_DELTA_FLI_chunk(UNBYTE *Ptr);
static void FLC_Decode_BLACK_chunk(UNBYTE *Ptr);
static void FLC_Decode_BYTE_RUN_chunk(UNBYTE *Ptr);
static void FLC_Decode_LITERAL_chunk(UNBYTE *Ptr);

/* Error handling functions */
static void FLC_Error(UNSHORT Error_code);
static void FLC_Print_error(UNCHAR *buffer, UNBYTE *data);
 ...
 *              per frame (20 frames per second).
 * SEE ALSO  : FLC.H
 * #FUNCTION HEADER END#
 */
...
FLC_Playback_flic_frame(void)
{
	...
	if (FLC_Playing_back_flic)
	{
		...
		FLC_Decode_frame_chunk(FLC_O->Input_buffer + Offset);
...
BOOLEAN
FLC_Decode_frame_chunk(UNBYTE *Frame_chunk_ptr)
{
	struct Flic_chunk_header *Chunk;
	...
	for (i=0;i<Height;i++)
	{
		...
		do
		{
			...
			switch ((Special_word & 0xC000) >> 14)
			{
				...
				{
					/* Get colour */
					Colour1 = Special_word & 0x00FF;

					/* Is transparent colour ? */
					if (Colour1 != FLC_O->Trans_colour_index)
					{
						/* No -> Set last pixel */
						OPM_SetPixel(FLC_O->Output_frame, FLC_O->Playback_X +
						 FLC_O->Width - 1, FLC_O->Playback_Y + Y, Colour1);
					}
			...
			do
			{
				...
				else
				{
					...
					while (Counter--)
					{
						...
						/* Is transparent colour ? */
						if (Colour2 != FLC_O->Trans_colour_index)
						{
							/* No -> Set pixel */
							OPM_SetPixel(FLC_O->Output_frame, FLC_O->Playback_X + X,
			...
			do
			{
				...
				if (Counter >= 0)
				{
					...
					while (Counter--)
					{
						...
						/* Is transparent colour ? */
						if (Colour != FLC_O->Trans_colour_index)
						{
							/* No -> Set pixel */
							OPM_SetPixel(FLC_O->Output_frame, FLC_O->Playback_X + X,
...
{
	/* Is the black colour also the transparent colour ? */
	if (FLC_O->Black_colour_index != FLC_O->Trans_colour_index)
	{
...
FLC_Decode_LITERAL_chunk(UNBYTE *Ptr)
{
	...
	/* Is there a transparent colour ? */
	if (FLC_O->Trans_colour_index)
	{
		/* Yes -> Get target address */
		Target = (FLC_O->Output_frame->data + (FLC_O->Playback_Y *
		 FLC_O->Output_frame->nextypos) + FLC_O->Playback_X);

		/* Copy literal image, transparent */
		OPM_CopyBlockTrans(Ptr, Target, (SILONG) 0,
		 (SILONG) (FLC_O->Output_frame->nextypos - FLC_O->Output_frame->width),
		 FLC_O->Width, FLC_O->Height, FLC_O->Trans_colour_index);
	}
	else
	{

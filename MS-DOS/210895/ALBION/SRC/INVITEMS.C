 ...
 * FUNCTION  : Drop method of Body and Backpack item slot AND Body item
 *              list object.
 * FILE      : INVITEMS.C
 * AUTHOR    : Jurie Horneman
		...
		case DRINK_IT:
		{
			...
			else
			{
				...
				if (!(Item_data->Class_use & (1 << Get_class(Active_char_handle))))
				{
					/* No -> Disable */
					PUMES[2].Flags |= PUME_BLOCKED;
					PUMES[2].Blocked_message_nr = 185;
				}
			}
			break;
		}
		...
		case SPECIAL_IT:
		{
			...
			else
			{
				...
				else
				{
					/* No -> Disable */
					PUMES[6].Flags |= PUME_BLOCKED;
					PUMES[6].Blocked_message_nr = 185;
				}
			}
			break;
		}
...
PUM_Learn_spell_inventory_item(UNLONG Data)
{
	...
	else
	{
		...
		Set_spell_strength(Active_char_handle, Item_data->Spell_class,
		 Item_data->Spell_nr - 1, 50 * 100);

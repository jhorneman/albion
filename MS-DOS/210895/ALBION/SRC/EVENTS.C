	...
	switch (Event->Byte_1)
	{
		...
		case PARTY_SPOKEN_TEXT_TYPE:
		{
			...
			if (Member_index != 0xFFFF)
			{
				...
				{
					/* Yes -> Do text window */
					Do_text_file_window_with_symbol(Text_handle, Block_nr, 34, 37,
					 Small_portrait_handles[Member_index - 1], 0);
				}
			}
			break;
		}
		...
		case ACTIVE_SPOKEN_TEXT_TYPE:
		{
			...
			if (Character_capable(Active_char_handle))
			{
				/* Yes -> Do text window */
				Do_text_file_window_with_symbol(Text_handle, Block_nr, 34, 37,
				 Small_portrait_handles[PARTY_DATA.Active_member - 1], 0);
			}
			break;
		}
 ...
 struct Event_block *Event)
{
	...
	switch (Event->Byte_1)
	{
		...
		case LP_DC_TYPE:
		{
			...
			/* Calculate new LP */
			New_LP = (UNSHORT) Modify_amount((UNLONG) Old_LP, (UNLONG) 32767,
	...
	switch (Event->Byte_1)
	{
		...
		case MEMBER_CONSCIOUS_QM_TYPE:
		{
			...
			if (Member_index != 0xFFFF)
			{
				...
				if (!(Get_conditions(Party_char_handles[Member_index]) &
				 DEAD_MASK))
				{
					Result = TRUE;
				}
 ...
 UNSHORT Number, UNSHORT Modify_mode)
{
	...
	else
	{
		...
		while (Counter)
		{
			...
			/* Insert attempted amount in add packet */
			Add_packet.Quantity = Attempted_amount;

...
static MEM_HANDLE Script_handle;
static MEM_HANDLE Script_line_data_handle;
...
Execute_script(UNSHORT Script_index)
{
	...
	else
	{
		...
		Script_handle = Load_subfile(SCRIPT, Script_index);
		if (!Script_handle)
		{
...
Script_ModInit(void)
{
	...
	Script_line_data_handle = MEM_Allocate_memory(MAX_SCRIPT_LINES *
	 sizeof(struct Script_line_data));
	MEM_Clear_memory(Script_line_data_handle);

	/* Analyse script */
	Nr_lines_in_script = Analyse_script(Script_handle,
	...
	Script_start_ptr = (UNCHAR *) MEM_Claim_pointer(Script_handle);
	...
	for (i=0;i<Nr_lines_in_script;i++)
	{
		...
		if (Script_line_data_ptr[i].Line_type == COMMAND_SCRIPT_LINE)
		{
			...
			if (Command_index != 0xFFFF)
			{
				...
				if (Abort_script_flag || Quit_program)
				{
					/* Yes */
					break;
				}
	...
	MEM_Free_memory(Script_line_data_handle);

	/* Destroy script */
	MEM_Kill_memory(Script_handle);
...
Party_member_text_script_command_handler(UNCHAR *Script_ptr)
{
	...
	if (Member_index != 0xFFFF)
	{
		...
		{
			/* Yes -> Do text window */
			Do_text_file_window_with_symbol(Text_handle, (UNSHORT) Block_nr,
			 34, 37, Small_portrait_handles[Member_index - 1], 0);
		}
	}
...
NPC_text_script_command_handler(UNCHAR *Script_ptr)
{
	...
	/* Do text window */
	Do_text_file_window_with_symbol(Text_handle, (UNSHORT) Block_nr, 34, 37,
	 NPC_portrait_handle, 0);
...
Active_member_text_script_command_handler(UNCHAR *Script_ptr)
{
	...
	if (Character_capable(Active_char_handle))
	{
		/* Yes -> Do text window */
		Do_text_file_window_with_symbol(Text_handle, (UNSHORT) Block_nr,
		 34, 37,	Small_portrait_handles[PARTY_DATA.Active_member - 1], 0);
	}
...
{
	struct Script_error_data Error;

	/* Initialize error structure */
	Error.Code			= Error_code;
	Error.Line_nr		= Current_script_line_nr;
	Error.Script_nr	= Current_script_nr;

	/* Push error on the error stack */
	ERROR_PushError(Script_print_error, Script_library_name,
	 sizeof(struct Script_error_data), (UNBYTE *) &Error);
}
...
Script_print_error(UNCHAR *buffer, UNBYTE *data)
{
	...
	Error = (struct Script_error_data *) data;
	...
	if (Error_code > SCRIPTERR_MAX)
 	...
 	/* Print error */
	sprintf((char *)buffer, Script_error_strings[Error_code], Error->Line_nr,
	 Error->Script_nr);
}
 ...
 UNSHORT Play_mode)
{
	...
	/* Load flic */
	ANIM_Flic_handle = Load_subfile(FLIC, Flic_nr);
	if (!ANIM_Flic_handle)
	{
		Error(ERROR_FILE_LOAD);
		Exit_program();
...
Play_animation_frames(UNSHORT Nr_frames)
{
	...
	for (i=0;i<Nr_frames;i++)
	{
		...
		/* Wait */
		while (SYSTEM_GetTicks() < T + ANIM_dT);
	}

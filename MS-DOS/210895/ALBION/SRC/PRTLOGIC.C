...
Init_party_data(UNSHORT Load_game_nr, BOOLEAN No_load_flag)
{
	...
	if (Load_game_nr)
	{
		Load_game_state(Load_game_nr, No_load_flag);
	}
	else
	{
...
Set_party_gold(UNLONG New_party_gold)
{
	...
	if (Delta_gold)
	{
		...
		if (Delta_gold > 0)
		{
			...
			while (Nr_members && Delta_gold > 0)
			{
				/* Check all party members */
				Members_left = Nr_members;
				for (i=0;i<6;i++)
				{
					/* Anyone there ? */
					if (PARTY_DATA.Member_nrs[i])

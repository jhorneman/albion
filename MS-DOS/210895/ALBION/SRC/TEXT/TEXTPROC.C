...
static UNBYTE Processing_buffer[LINE_LENGTH_MAX];
	...
	while (Text_ptr)
	{
		while (TRUE)
		{
			...
			if (Char == COMSTART_CHAR)
			{
				...
				UNSHORT i, j;
				UNCHAR *Start_of_command, Command[4], *Add_ptr;
				BOOLEAN Flag;
...
Find_wrap_position(UNCHAR *Text)
{
	...
	while (TRUE)
	{
		...
		TEXT_Current_X += (UNSHORT)(Current_font->Width_table)[Char2];
...
Write_processed_line(UNCHAR *End, struct Processed_text *Processed)
{
	...
	*End = Char;
	...
	if (!Process_text_dry_run_flag)
	{
		...
		/* Create line info */
		memcpy(&Info, &Current_line_info, sizeof(struct Line_info));
		memcpy(&(Info.Style), &This_line_style, sizeof(struct Textstyle));

		/* Copy line info */
		memcpy(Ptr, &Info, sizeof(struct Line_info));

...
UNSHORT Processed_text_index; 	/* Index in current processed TPB */
UNSHORT Raw_text_index;				/* Index in current raw TPB */
UNSHORT Processing_buffer_index;	/* Index in processing buffer */

UNBYTE Processing_buffer[LINE_LENGTH_MAX];

MEM_HANDLE First_raw_TPB_handle;
MEM_HANDLE Current_raw_TPB_handle;
MEM_HANDLE Current_processed_TPB_handle;
...
{
	struct TPB *Current_raw_TPB, *Current_processed_TPB;
	UNCHAR *Text_ptr, Char, *Raw_ptr, *End_of_RTPB;
	UNCHAR *Wrap_ptr, *Space_ptr;
	...
	First_raw_TPB_handle = MEM_Allocate_memory(TPB_SIZE);
	...
	while (Text_ptr)
	{
		while (TRUE)
		{
			...
			if (Char == COMSTART_CHAR)
			{
				...
				UNSHORT i;
				UNCHAR *Start_of_command, Command[4];
				BOOLEAN Flag;
				...
				while (Command_list->Command_handler)
				{
					...
					{
						/* Yes -> Execute */
						(Command_list->Command_handler)(Text_ptr);
	...
	*Raw_ptr = 0;
	...
	Current_processed_TPB_handle = MEM_Allocate_memory(TPB_SIZE);
...
Write_processed_line(UNCHAR *End, struct Processed_text *Processed)
{
	...
	*End = Char;
	...
	if ((Processed_text_index + Size) >= TPB_SIZE-1)
	{
		...
		/* Link to previous processed TPB */
		Ptr = MEM_Claim_pointer(Current_processed_TPB_handle);
	...
	/* Copy line info */
	memcpy(Ptr, &Current_line_info, sizeof(struct Line_info));
	...
	/* Copy actual text */
	strncpy(Ptr, Start, Current_line_info.String_length);
...
Analyze_line(UNCHAR *Text, struct Processed_text *Processed)
{
	struct Fontstyle *Current_font;
	struct Kerning_pair *Table;
	UNSHORT Height_under_base,Max_between_height;
	UNSHORT w,Size;
	UNBYTE Previous_character, Char2;
	UNCHAR Char, *Start;

	/* Reset line info */
	memcpy(&(Current_line_info.Style), &Current_text_style,
	 sizeof(struct Textstyle));
	...
	*(Text+Size) = 0;
	...
	while (TRUE)
	{
		...
		/* Skip character width in pixels */
		w = (UNSHORT)(Current_font->Width_table)[Char2]
	...
	Current_line_info.Width = Current_X;
...
Read_raw_line(void)
{
	...
	if (Processing_buffer_index)
	{
		...
		memmove(Target, Target + Processing_buffer_index,
		 LINE_LENGTH_MAX - Processing_buffer_index);

		/* Less characters need be read */
		Size = Processing_buffer_index;

		/* The target is further back */
		Target += LINE_LENGTH_MAX - Processing_buffer_index;

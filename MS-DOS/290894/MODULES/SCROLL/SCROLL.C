 ...
 UNSHORT Width, UNSHORT Height)
{
	...
	/* Build virtual OPM */
	OPM_CreateVirtualOPM(&(Scroll->Base_OPM), &Viewport, 0, 0, Width, Height);
	OPM_SetVirtualPosition(&Viewport, Buffer_X, Buffer_Y);

	/* Display virtual OPM */
	DSA_CopyOPMToScreen(Screen, &Viewport, Screen_X, Screen_Y,
	 DSA_CMOPMTS_ALWAYS);
 ...
 UNSHORT Width, UNSHORT Height)
{
	...
	/* Display virtual OPM */
	OPM_CopyOPMOPM(&Viewport, OPM, 0, 0, Width, Height, Screen_X, Screen_Y);

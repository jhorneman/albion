...
MEM_Init_memory(UNSHORT Nr_requests, struct Memory_request *Request_list)
{
	...
	for (i=0;i<min(Nr_requests, MEMORY_AREAS_MAX);i++)
	{
		...
		/* Allocate it */
		Start = (UNLONG) BASEMEM_Alloc(Size, Request_list[i].Base_memory_type);
	...
	if (Success)
	{
		...
		MEM_Workspaces[0].Nr_of_areas			= Nr_requests;
		MEM_Workspaces[0].Parent_workspace	= 0;

		/* Copy flag */
		MEM_Debugging2 = MEM_Debugging;
...
MEM_Exit_memory(void)
{
	...
	{
		/* Yes -> Make old handles invalid */
		MEM_Reset_memory();

		/* Destroy the first workspace */
		Workspace = &MEM_Workspaces[0];
		...
		for(i=0;i<Workspace->Nr_of_areas;i++)
		{
			...
			/* Clear memory area data */
			Workspace->Areas[i].Size = 0;
		}
...
MEM_Init_area(struct Memory_entry *Area)
{
	...
	if (Entry)
	{
		...
		if (MEM_Debugging2)
		{
			/* Yes -> Fill with debugging fill pattern */
			BASEMEM_FillMemLong(Entry->Start, Entry->Size, MEM_FILLER_1);
		}
	}
}
...
MEM_Error(UNSHORT Error_code)
{
	...
	/* Initialize memory error structure */
	Error.Code = Error_code;

	/* Push error on the error stack */
	ERROR_PushError(MEM_Print_error, MEM_Library_name,
	 sizeof(struct Memory_error), (UNBYTE *) &Error);
}

/*
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : MEM_Critical_error
...
void
MEM_Critical_error(UNSHORT Error_code)
{
	/* Report error */
...
MEM_Print_error(UNCHAR *buffer, UNBYTE *data)
{
	...
	/* Print error */
	sprintf((char *) buffer, "%s", MEM_Error_strings[i]);
}
...
BOOLEAN
MEM_Handle_is_legal(MEM_HANDLE Handle)
{
	/* Check if handle lies within handle array */
	if (((UNBYTE *) Handle) < (UNBYTE *) &MEM_Handles[0])
	{
		MEM_Critical_error(MEMERR_ILLEGAL_HANDLE);
		return FALSE;
	}
	...
	if (Handle)
	{
		...
		if (MEM_Handle_is_legal(Handle))
		{
			...
			if (Entry)
			{
				...
				if (MEM_Debugging2)
				{
					/* Yes -> Skip debugging fill pattern */
					Ptr += DEBUGGING_PAD_SIZE;
				}
			}
			else
			{
...
MEM_Destroy_memory_handle(struct Memory_entry *Target)
{
	...
	if (Handle)
	{
		/* Yes -> Has it been claimed ? */
		if (!Handle->Claim_counter)
		{
			/* No -> Remove reference to handle */
			Target->BLOCK_HANDLE = NULL;

			/* Clear memory handle */
			BASEMEM_FillMemByte((UNBYTE *) Handle,
			 sizeof(struct Memory_handle), 0);
		}
	}
}
...
MEM_Get_block_size(MEM_HANDLE Handle)
{
	...
	if (Handle)
	{
		...
		if (MEM_Handle_is_legal(Handle))
		{
			...
			if (MEM_Debugging2)
			{
				/* Yes -> Subtract size of debugging buffers */
...
MEM_Free_memory(MEM_HANDLE Handle)
{
	...
	if (Handle)
	{
		...
		if (MEM_Handle_is_legal(Handle))
		{
			...
			/* Is the handle allocated and unclaimed ? */

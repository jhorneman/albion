...
Perform_action(struct Event_action *New)
{
	...
	/* Copy input data */
	memcpy((UNBYTE *) Event_action, (UNBYTE *) New,
...
Handle_acting_member_set(UNSHORT Value)
{
	...
	if (Event_action->Actor_type == PARTY_ACTOR_TYPE)
	{
		...
		if (Member_present(Member_nr))
		{
			...
			{
				/* Yes -> Search event sets of actinh member */
...
Execute_event_chain(void)
{
	...
	if (i != 0xFFFF)
	{
		...
		for(;;)
		{
			...
			/* Break event chain / exit program / game over ? */
			if ((Context->Flags & BREAK_EVENT_CHAIN) || Quit_program_flag ||
			 Game_over_flag)
			{
				/* Yes */
				break;

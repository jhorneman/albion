...
//	shapes=calc_strength(Strength,C0_Spell_4_MIN_SHAPES,C0_Spell_4_MAX_SHAPES);
	...
	if (Victim_part->Type == MONSTER_PART_TYPE)
	{
		...
		for(t=0;t<shapes;t++){
			...
			shape_COMOB[t]->Draw_mode = TRANSLUMINANCE_COMOB_DRAWMODE;
	...
	Add_temporary_effect(Victim_part, HURRY_TEMP_EFFECT, Strength,
	 10, NULL);
}
...
leavelop1c06:
		...
		if (Victim_part->Type == MONSTER_PART_TYPE){
			...
			if(!(behave = Add_COMOB_behaviour(shape_COMOB,Victim_part->Main_COMOB, connect_handler))){
				ERROR_PopError();
				return;
			}
...
//		Play_sound(211);
		...
		/* Add temporary effect */
		Add_temporary_effect(Victim_part, FROZEN_TEMP_EFFECT, Strength,
		 10, shape_COMOB);
	}
}
 ...
 UNSHORT Tactical_Y, UNSHORT Strength)
{
	...
	if (Strength)
	{
		...
		/* Add temporary effect */
		Add_temporary_effect(Victim_part, BLINDED_TEMP_EFFECT, Strength,
		 10, NULL);

...
static BOOLEAN Changing_mouse_pointer_flag = FALSE;
...
/* Mouse pointer file type */
static UNCHAR Mouse_pointer_fname[] = "Mouse pointer";

static struct File_type Mouse_pointer_ftype = {
	NULL,
	0xFF,
	0,
	MEM_KILL_ALWAYS, // | MEM_LOCK,
	Mouse_pointer_fname
};
...
{
	/* Play sound effect */
//	Play_sound_effect(103, 50, 60, 0, 0);
...
{
	/* Avoid endless loops */
	if (Changing_mouse_pointer_flag)
		return;
 ...
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - This function will set the flag Changing_mouse_pointer_flag
 *              to avoid endless loops caused by BBMEM pushing a new mouse
 *              pointer by way of the lengthy operation callback functions.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
Init_mouse_pointer(struct Mouse_pointer *New)
{
	...
	/* Set flag to avoid endless loops */
	Changing_mouse_pointer_flag = TRUE;

	/* Calculate size of sprite image */
	Size = New->Width * New->Height;

	/* Allocate memory for new mouse GTO */
	/* (this call might cause an endless loop if there was no safety
	   mechanism) */
	Handle = MEM_Do_allocate(sizeof(struct GTO) + Size + 4, (UNLONG) 0,
	 &Mouse_pointer_ftype);
	...
	/* Clear flag */
	Changing_mouse_pointer_flag = FALSE;
}
...
Mouse_on(void)
{
	...
	if (Mouse_counter)
	{
		...
		{
			/* Yes */
			GTO = (struct GTO *) MEM_Claim_pointer(Mouse_GTO_handle);
			SYSTEM_ShowMousePtr(GTO);
		}
	}
}
...
Switch_screens(void)
{
	...
	if (BBERROR_StackEntries != Error_stack_entries)
	{
		...
		if (BBERROR_StackEntries > Error_stack_entries)
		{
			...
			/* Toggle colour */
			Palette.color[0].red		^= 0xFF;
			Palette.color[0].green	^= 0xFF;
			Palette.color[0].blue	^= 0xFF;

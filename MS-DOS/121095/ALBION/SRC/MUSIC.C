...
#define MUSICERR_FILE_LOAD		(2)

#define MUSICERR_MAX				(2)
...
/* Global volumes */
UNSHORT Global_MIDI_volume;
UNSHORT Global_digital_volume;
...
static UNCHAR *Music_error_strings[MUSICERR_MAX + 1] = {
	"Illegal error code.",
	"AIL error : %s",
	 ...
	 (Sound_state & SOUND_FX_ON))
	{
		...
		for (i=0;i<MAX_NORMAL_SOUND_EFFECTS;i++)
		{
			...
			if (Sound_effect_table[i].State != SOUND_EFFECT_FREE)
			{
				...
				Stop_sound_effect(&(Sound_effect_table[i]));
			}
		}
...
void
Set_MIDI_volume(UNSHORT New_volume)
{
	/* Sound system initialized ? */
	if (Sound_state & SOUNDSYSTEM_INITIALIZED)
	{
		/* Yes -> Set MIDI volume */
		Global_MIDI_volume = New_volume;
...
Get_digital_volume(void)
{
	...
	if (Sound_state & SOUNDSYSTEM_INITIALIZED)
	{
		/* Yes -> Get digital volume */
		Volume = Global_digital_volume;
	}
	else
	{
		/* No -> Return zero */
...
void
Set_digital_volume(UNSHORT New_volume)
{
	/* Sound system initialized ? */
	if (Sound_state & SOUNDSYSTEM_INITIALIZED)
	{
		/* Yes -> Set digital volume */
		Global_digital_volume = New_volume;

		/* Set master digital volume */
		AIL_set_digital_master_volume(Digital_driver, (LONG) New_volume);
	}
}
	...
	if (Sound_state & SOUNDSYSTEM_INITIALIZED)
	{
		...
		if (Sound_state & MIDI_ON)
		{
			...
			if ((Sound_state & DIGI_ON) && (Ambient_song.State != SONG_FREE))
			{
				...
				/* Unlock wave library */
				Ptr = MEM_Get_pointer(Ambient_wavelib_handle);
				Length = MEM_Get_block_size(Ambient_wavelib_handle);
...
Play_ambient_song(UNSHORT Song_nr)
{
	 ...
	 (Sound_state & DIGI_ON))
	{
		...
		if (Song_nr)
		{
			...
			{
				/* No -> Unlock song */
				Ptr = MEM_Get_pointer(Ambient_song.Memory_handle);
				Length = MEM_Get_block_size(Ambient_song.Memory_handle);
				BASEMEM_Unlock_region(Ptr, Length);

				/* Remove song */
				MEM_Free_pointer(Ambient_song.Memory_handle);
				MEM_Free_memory(Ambient_song.Memory_handle);
			...
			/* Get wave library length */
			Length = MEM_Get_block_size(Ambient_wavelib_handle);

			/* Lock wave library memory */
			BASEMEM_Lock_region(Ptr, Length);

			/* Create wave synthesizer */
			Ambient_wave_synth = AIL_create_wave_synthesizer
			(
				Digital_driver,
				XMIDI_driver,
				(void *) Ptr,
...
void
Initialize_located_effects(UNLONG Listening_range)
{
	MEM_HANDLE Handle;
	UNLONG Length;
	UNSHORT i;
			...
			for (i=0;i<MAX_LOCATED_SOUND_EFFECTS;i++)
			{
				...
				if (Located_sound_effect_table[i].Effect.State != SOUND_EFFECT_FREE)
				{
					...
					Handle = Load_subfile(DSAMPLE,
					 Located_sound_effect_table[i].Effect.Effect_nr);
					...
					if (Handle)
					{
						...
						/* Store data */
						Located_sound_effect_table[i].Effect.Memory_handle	= Handle;
						Located_sound_effect_table[i].Effect.Length 			= Length;
					}
					else
					{
						/* No -> Out of memory ? */
...
void
Load_located_effects(UNBYTE *Ptr)
{
	UNSHORT i;
	...
	memcpy
	(
		(UNBYTE *) &(Located_sound_effect_table[0]),
		Ptr,
	...
	for (i=0;i<MAX_LOCATED_SOUND_EFFECTS;i++)
	{
		...
		Located_sound_effect_table[i].Effect.Memory_handle	= NULL;

		/* Clear sample length in case the samples have changed */
		Located_sound_effect_table[i].Effect.Length = 0;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_located_effect_table_address
...
UNBYTE *
Get_located_effect_table_address(void)
{
	return ((UNBYTE *) &(Located_sound_effect_table[0]));
}

/*
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_located_effect_table_length
...
UNLONG
Get_located_effect_table_length(void)
{
...
void
Remove_located_effect(UNSHORT Effect_index)
{
	...
	if (Located_sound_effect_table[Effect_index].Effect.State ==
	 SOUND_EFFECT_FREE)
		return;

	/* Delete sound effect */
	Delete_sound_effect(&(Located_sound_effect_table[Effect_index].Effect));
}

/*
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Remove_located_effects_at
...
void
Remove_located_effects_at(SILONG Effect_X, SILONG Effect_Y)
{
	UNSHORT i;
	...
	for (i=0;i<MAX_LOCATED_SOUND_EFFECTS;i++)
	{
		...
		if ((Located_sound_effect_table[i].Effect.State != SOUND_EFFECT_FREE) &&
		 (Located_sound_effect_table[i].Effect_X == Effect_X) &&
		 (Located_sound_effect_table[i].Effect_Y == Effect_Y))
		{
			/* Yes -> Delete it */
			Delete_sound_effect(&(Located_sound_effect_table[i].Effect));
		}
...
void
Set_effect_position(UNSHORT Effect_index, SILONG Effect_X, SILONG Effect_Y)
{
	/* Exit if effect index is illegal */
	if (Effect_index >= MAX_LOCATED_SOUND_EFFECTS)
		return;
	...
	Located_sound_effect_table[Effect_index].Effect_X = Effect_X;
	Located_sound_effect_table[Effect_index].Effect_Y = Effect_Y;
}
...
End_song(struct Song *Song)
{
	...
	if (Song->State != SONG_FREE)
	{
		...
		/* Unlock song */
		Ptr = MEM_Get_pointer(Song->Memory_handle);
		Length = MEM_Get_block_size(Song->Memory_handle);
		BASEMEM_Unlock_region(Ptr, Length);
...
{
	static UNSHORT Located_sound_effects_restart_counter = 0;
	...
	Rough_effect_listening_range = (Effect_listening_range * Effect_listening_range);

	/* Update restart counter */
	if (Located_sound_effects_restart_counter < 20)
		Located_sound_effects_restart_counter++;
	else
		Located_sound_effects_restart_counter = 0;
	...
	for (i=0;i<MAX_LOCATED_SOUND_EFFECTS;i++)
	{
		...
		if (Located_sound_effect_table[i].Effect.State == SOUND_EFFECT_STOPPED)
		{
			...
			if (Located_sound_effect_table[i].Probability < 100)
			{
				/* No -> Should this sound effect be started again ? */
				if (Located_sound_effects_restart_counter ||
				 ((rand() % 100) > Located_sound_effect_table[i].Probability))
				{
					/* No -> Skip */
					continue;
				}
...
Start_sound_effect(struct Sound_effect *Effect)
{
	...
	if (Channel_index != 0xFFFF)
	{
		...
		/* Get sample length */
		Length = MEM_Get_block_size(Effect->Memory_handle);
		...
		(
			Channel->Sample_handle,
			(Effect->Volume * SOUND_FX_VOLUME) / 100
		);
		AIL_set_sample_playback_rate
		(
			Channel->Sample_handle, (LONG) Effect->Frequency
		);
		AIL_set_sample_pan
		(
			Channel->Sample_handle,
...
Stop_digital_channel(UNSHORT Channel_index)
{
	 ...
	 (Digital_channels[Channel_index].State == DIGI_CHANNEL_PLAYING))
	{
		...
		/* Unlock sound effect */
		Ptr = MEM_Get_pointer(Digital_channels[Channel_index].Effect_ptr->Memory_handle);
		Length = MEM_Get_block_size(Digital_channels[Channel_index].Effect_ptr->Memory_handle);
		BASEMEM_Unlock_region(Ptr, Length);

		/* Free pointer */
		MEM_Free_pointer(Digital_channels[Channel_index].Effect_ptr->Memory_handle);

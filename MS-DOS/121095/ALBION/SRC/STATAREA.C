...
#define PERMANENT_TEXT_MINIMUM	(PERMANENT_TEXT_DURATION - 10)
		...
		if (Member_present(i + 1))
		{
			...
			if ((Party_member->Damage) && (Party_member->Damage_display_timer))
			{
				...
				if (Party_member->Damage > 0)
				{
					...
					sprintf(Number, "%u", Party_member->Damage);
				...
				else
				{
					...
					/* Build damage string */
					sprintf(Number, "%u", 0 - Party_member->Damage);
...
Set_Permanent_text_object(struct Object *Object, union Method_parms *P)
{
	...
	if (In_Combat)
	{
		/* Yes -> While a new text is still on the screen */

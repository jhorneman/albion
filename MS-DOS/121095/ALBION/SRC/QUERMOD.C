 ...
 struct Event_block *Event)
{
	...
	switch (Event->Byte_1)
	{
		...
		case ATTRIBUTE_DC_TYPE:
		{
			...
			{
				/* No -> Modify */
				Attr->Normal = (SISHORT) Modify_amount_from_maximum
				(
					(SILONG) Attr->Normal,
					(SILONG) Attr->Maximum,
					Modify_mode,
		...
		case SKILL_DC_TYPE:
		{
			...
			{
				/* No -> Modify */
				Skill->Normal = (SISHORT) Modify_amount_from_maximum
				(
					(SILONG) Skill->Normal,
					(SILONG) Skill->Maximum,
					Modify_mode,
					(SILONG) Number
				);
			}
		...
		case GOLD_DC_TYPE:
		{
			...
			{
				/* Yes -> Modify */
				Char->Char_gold = (SISHORT) Modify_amount
				(
					(SILONG) Char->Char_gold,
					32767,
					Modify_mode,
		...
		case FOOD_DC_TYPE:
		{
			...
			{
				/* Yes -> Modify */
				Char->Char_food = (SISHORT) Modify_amount
				(
					(SILONG) Char->Char_food,
					32767,
					Modify_mode,
 ...
 UNSHORT Number, UNSHORT Modify_mode)
{
	...
	Old_amount = (SILONG) Count_items_in_backpack(Char_handle, Item_index);

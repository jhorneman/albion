...
/* HDOB handler data */
union HDOB_handler_data {
	struct Drop_HDOB_data Drop_data;
	struct Move_HDOB_data Move_data;
};

/* prototypes */
void Drop_HDOB_handler(UNSHORT HDOB_nr);
void Move_HDOB_handler(UNSHORT HDOB_nr);

/* global variables */
static UNSHORT HDOB_Hide_counter;

static struct HDOB HDOBs[MAX_HDOBS];
static union HDOB_handler_data HDOB_handler_data[MAX_HDOBS];
...
Add_HDOB(struct HDOB *New)
{
	...
	for (i=0;i<MAX_HDOBS;i++)
	{
		...
		if (!(HDOBs[i].Flags & HDOB_ON))
		{
			...
			/* Copy HDOB data */
			BASEMEM_CopyMem((UNBYTE *) New,
			 (UNBYTE *) Free, sizeof(struct HDOB));
			...
			Free->Background_handle =
			 MEM_Allocate_memory(New->Width * New->Height);
...
Draw_HDOBs(struct BBPOINT *Mouse)
{
	...
	for (i=0;i<MAX_HDOBS;i++)
	{
		...
		if (Current->Flags & HDOB_ON)
		{
			...
			Get_block(Current_OPM, X, Y, Current->Width, Current->Height,
			 Background_ptr);
			...
			switch (Current->Draw_mode)
			{
				case HDOB_MASK:
				{
					Put_masked_block(Current_OPM, X, Y, Current->Width,
					 Current->Height, Graphics_ptr);
					break;
				}
				case HDOB_BLOCK:
				{
					Put_unmasked_block(Current_OPM, X, Y, Current->Width,
					 Current->Height, Graphics_ptr);
					break;
				}
				case HDOB_SILHOUETTE:
				{
					Put_silhouette(Current_OPM, X, Y, Current->Width,
					 Current->Height, Graphics_ptr, (UNBYTE) Current->Draw_data);
					break;
				}
				case HDOB_BOX:
				{
					OPM_FillBox(Current_OPM, X, Y, Current->Width,
					 Current->Height, (UNBYTE) Current->Draw_data);
					break;
				}
				case HDOB_RECOLOUR:
				{
					Put_recoloured_block(Current_OPM, X, Y, Current->Width,
					 Current->Height, Graphics_ptr, (UNBYTE *) Current->Draw_data);
					break;
				}
				case HDOB_RECOLBOX:
				{
					Put_recoloured_box(Current_OPM, X, Y, Current->Width,
					 Current->Height, (UNBYTE *) Current->Draw_data);
					break;
				}
			}

			MEM_Free_pointer(Current->Graphics_handle);
...
Erase_HDOBs(struct BBPOINT *Mouse)
{
	...
	for (i=MAX_HDOBS-1;i>=0;i--)
	{
		...
		if (Current->Flags & HDOB_ON)
		{
			...
			/* Restore background */
			Background_ptr = MEM_Claim_pointer(Current->Background_handle);
			Put_unmasked_block(Current_OPM, X, Y, Current->Width, Current->Height,
			 Background_ptr);
			MEM_Free_pointer(Current->Background_handle);
		}
	}
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : HDOB_is_ready
 * FUNCTION  : Check if a HDOB is ready.
 * FILE      : HDOB.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 06.10.95 13:43
 * LAST      : 06.10.95 13:43
 * INPUTS    : UNSHORT HDOB_nr - HDOB index.
 * RESULT    : BOOLEAN : Ready.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
HDOB_is_ready(UNSHORT HDOB_nr)
{
	BOOLEAN Ready = TRUE;

	/* Anything there ? */
	if (HDOBs[HDOB_nr].Flags & HDOB_ON)
	{
		/* Yes -> Is it ready ? */
		if (HDOBs[HDOB_nr].Flags & HDOB_NOT_READY)
		{
			/* No */
			Ready = FALSE;
		}
...
Drop_HDOB(UNSHORT HDOB_nr)
{
	...
	{
		/* Yes -> Initialize drop */
		Init_HDOB_drop(HDOB_nr);

		/* Wait */
		while (!HDOB_is_ready(HDOB_nr))
		{
			Update_screen();
		}
	}
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Init_HDOB_drop
 * FUNCTION  : Initialize HDOB dropping.
 * FILE      : HDOB.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 05.10.95 22:06
 * LAST      : 06.10.95 13:25
 * INPUTS    : UNSHORT HDOB_nr - HDOB index.
 * RESULT    : None.
 * BUGS      : No known.
...
void
Init_HDOB_drop(UNSHORT HDOB_nr)
{
	...
	/* Get HDOB handler data */
	Data = &(HDOB_handler_data[HDOB_nr].Drop_data);
...
void
Drop_HDOB_handler(UNSHORT HDOB_nr)
{
	struct Drop_HDOB_data *Drop_data;
	struct HDOB *HDOB;
	SISHORT wX, wY;
	...
	/* Get drop data */
	Drop_data = &(HDOB_handler_data[HDOB_nr].Drop_data);
	...
	/* Off screen ? */
	if ((wX > (0 - HDOB->Width)) && (wX < Current_OPM->width) &&
	 (wY < Current_OPM->height))
	{
...
Move_HDOB(UNSHORT HDOB_nr, SISHORT Target_X, SISHORT Target_Y, UNSHORT Speed)
{
	...
	/* Get HDOB handler data */
	Data = &(HDOB_handler_data[HDOB_nr].Move_data);
	...
	if (HDOB->Flags & HDOB_ON)
	{
		...
		while (HDOB->Flags & HDOB_NOT_READY);
	}
}
...
void
Move_HDOB_handler(UNSHORT HDOB_nr)
{
	struct Move_HDOB_data *Move_data;
	struct HDOB *HDOB;
	SILONG dX;
	...
	/* Get movement data */
	Move_data = &(HDOB_handler_data[HDOB_nr].Move_data);

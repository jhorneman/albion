...
UNSHORT Global_monster_damage;
UNSHORT Global_monster_protection;
	...
	else
	{
		...
		for (i=0;i<Nr_moves;i++)
		{
			...
			else
			{
				...
				/* Count up */
				P.Move_parms.Nr_moves++;
			}
		}

		/* Any real moves ? */
		if (P.Move_parms.Nr_moves > 0)
		{
			/* Yes -> Show movement */
			...
			else
			{
				 ...
				 (Probe_attribute(Defender_part->Char_handle, DEXTERITY)))
				{
					...
					Break_combat_items(Attacker_part->Char_handle,
					 (1 << Current_weapon_slot_index), (1 << CLOSE_RANGE_IT));
 ...
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - This function is also called by the Flee combat spell.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
	...
	if (Reverse_combat_magic_flag)
	{
		...
		if (Part)
		{
			...
			Part->Target.Magic_target_data.Combat_target_mask =
			 1 << (Victim_part->Tactical_Y * NR_TACTICAL_COLUMNS +

...
void Insert_party_member_in_combat_matrix(struct Combat_participant *Part);
...
Enter_Combat(UNSHORT Monster_group_nr, UNSHORT Combat_background_nr)
{
	...
	static UNSHORT Night_combat_background_table[MAX_COMBAT_BACKGROUNDS] = {
		 0,  0, 17,  0,  0,
		 0,  0,  0,  0,  0,
		 0, 18,  0, 13, 19,
		 0,  0,  0,  0
	...
	{
		/* Night -> Is there a night version of this background ? */
	...
	if (!Restart_game_flag)
	{
		...
		switch (Combat_status)
		{
			...
			case COMBAT_PARTY_FLED:
			{
				...
				/* Try to reset the original active member */
				Result = Activate_party_member(Previous_active_member);

				/* Success ? */
				if (!Result)
				{
					/* No -> Can the active member still be active ? */
					...
					else
					{
						...
						else
						{
							...
							if (Part->Flags & PART_UNCONTROL)
							{
								...
								Insert_party_member_in_combat_matrix(Part);
							}
						}
					}
				}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Insert_party_member_in_combat_matrix
 * FUNCTION  : Insert a party member in the combat matrix.
 * FILE      : COMBAT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 09.10.95 21:39
 * LAST      : 09.10.95 21:39
...
void
Insert_party_member_in_combat_matrix(struct Combat_participant *Part)
{
	UNSHORT X, Y;

	/* Check bottom rows of combat matrix */
	for (Y=4;Y>=3;Y--)
	{
		for (X=0;X<6;X++)
		{
			...
			{
				/* No -> Insert party member in matrix */
				Combat_matrix[Y][X].Part = Part;

				/* Insert coordinates in participant data */
				Part->Tactical_X = X;
				Part->Tactical_Y = Y;
			}
		}
		...
		switch(Effect_type)
		{
			...
			case FROZEN_TEMP_EFFECT:
			{
				...
				Clear_condition(Part->Char_handle, LAMED);
		 		...
		 		/* Do damage */
				Do_combat_damage(Part,
				 max((Part->Temp_effect_data[Effect_type].Strength * 20) / 100, 1));

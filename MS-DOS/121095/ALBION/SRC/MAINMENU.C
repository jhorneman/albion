...
void Update_MIDI_volume(struct Number_bar_object *Number_bar);
void Update_digital_volume(struct Number_bar_object *Number_bar);
...
Main_menu_ModExit(void)
{
	...
	/* Restart / in combat ? */
	if (Restart_game_flag && In_Combat)
	{
		/* Yes -> Pop combat module */
		Pop_module();
	}
}
...
Main_menu_DisExit(void)
{
	...
	/* Does the party display object exist ? */
...
Init_Options_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Get MIDI volume */
	Main_menu_MIDI_volume = (SILONG) Get_MIDI_volume();
	...
	Number_bar_OID.Update		= Update_MIDI_volume;
	...
	/* Get digital volume */
	Main_menu_digital_volume = (SILONG) Get_digital_volume();

	/* Build digital volume bar OID */
	Number_bar_OID.Value_ptr	= &Main_menu_digital_volume;
	Number_bar_OID.Minimum		= 0;
	Number_bar_OID.Maximum		= 127;
	Number_bar_OID.Update		= Update_digital_volume;
	Number_bar_OID.Print			= NULL;
...
Exit_Options_object(struct Object *Object, union Method_parms *P)
{
	...
	sprintf(Number, "%u", Main_menu_MIDI_volume);
	Write_INI_variable
	(
		"ALBION",
		"MIDI_VOLUME",
	...
	sprintf(Number, "%u", Main_menu_digital_volume);
	Write_INI_variable
	(
		"ALBION",
		"DIGI_VOLUME",
	...
	sprintf(Number, "%u", Main_menu_3D_window_size);
	Write_INI_variable
	(
		"ALBION",
		"3D_WINDOW_SIZE",
	...
	Window_3D_size = Main_menu_3D_window_size;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Update_MIDI_volume
 * FUNCTION  : Update the MIDI volume (number bar function).
 * FILE      : MAINMENU.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.09.95 17:05
 * LAST      : 22.09.95 17:05
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
...
void
Update_MIDI_volume(struct Number_bar_object *Number_bar)
{
	Set_MIDI_volume((UNSHORT) Main_menu_MIDI_volume);
}

/*
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Update_digital_volume
 * FUNCTION  : Update the digital volume (number bar function).
 * FILE      : MAINMENU.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.09.95 17:06
 * LAST      : 22.09.95 17:06
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
...
void
Update_digital_volume(struct Number_bar_object *Number_bar)
{
	Set_digital_volume((UNSHORT) Main_menu_digital_volume);
}

...
/*
#define DIALOGUE_BACKGROUND_X			(0)
#define DIALOGUE_BACKGROUND_Y			(0)
#define DIALOGUE_BACKGROUND_WIDTH	(360)
#define DIALOGUE_BACKGROUND_HEIGHT	(Panel_Y)
*/
...
Dialogue_ModInit(void)
{
	...
	Dialogue_background_handle = MEM_Allocate_memory(DIALOGUE_BACKGROUND_WIDTH *
...
Handle_ask_word_default_MC_entry(void)
{
	...
	Default_dialogue_action.Action_extra	= 0;
...
Handle_give_item_default_MC_entry(void)
{
	...
	/* Select an item */
	Item_slot_index = Select_character_item
	(
		Active_char_handle,
...
Init_dialogue_log(UNBYTE *Data)
{
	...
	Handle = MEM_Do_allocate(sizeof(struct Dialogue_log_list), 0,
	...
	for (i=0;i<Nr_dialogue_log_entries / LOG_ENTRIES_PER_LIST;i++)
	{
		...
		Handle2 = MEM_Do_allocate(sizeof(struct Dialogue_log_list),
		 (UNLONG) i + 1, &Dialogue_log_list_ftype);
...
Prepare_dialogue_log_for_saving(void)
{
	...
	*((UNSHORT *) Ptr) = Nr_dialogue_log_entries;
	...
	{
		/* Yes -> Copy these as well */
		memcpy(Ptr, &(List->Entries[0]), (Nr_dialogue_log_entries %
		 LOG_ENTRIES_PER_LIST) * sizeof(struct Dialogue_log_entry));
	}

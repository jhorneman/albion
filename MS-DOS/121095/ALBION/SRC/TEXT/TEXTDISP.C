...
static UNSHORT Space_table[MAX_LINE_LENGTH];
...
Print_string(struct OPM *OPM, SISHORT X, SISHORT Y, UNCHAR *String)
{
	...
	while (TEXT_Current_X < OPM->width)
	{
 ...
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Print_right_string
 * FUNCTION  : Print a normal string, right-justified.
 * FILE      : TEXTDISP.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 09.09.95 18:45
 * LAST      : 09.09.95 18:45
 * INPUTS    : struct OPM *OPM - Pointer to target OPM.
 *             SISHORT X - Left X-coordinate in OPM.
 *             SISHORT Y - Top Y-coordinate in OPM.
 *             UNSHORT Width - Width in which the string must be justified.
 *             UNCHAR *String - Pointer to string.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - Do not use this function to display processed strings.
...
void
Print_right_string(struct OPM *OPM, SISHORT X, SISHORT Y, UNSHORT Width,
 UNCHAR *String)
{
	UNSHORT W;
...
Print_justified_string(struct OPM *OPM, SISHORT X, SISHORT Y, UNCHAR *String)
{
	...
	while (TEXT_Current_X < OPM->width)
	{
		...
		{
			/* Yes -> Skip pixels */
			TEXT_Current_X += Space_table[Space_index];
			Space_index++;

...
Datachange_event(void)
{
	...
	switch (Event->Byte_3)
	{
		...
		case SINGLE_MEMBER_DC:
		{
			...
			{
				/* Yes -> Change party member's data */
				Do_datachange(Party_char_handles[Member_index - 1], Event);
			}
		...
		case LOCATED_EFFECT_SOUND_TYPE:
		{
			...
			if (Context->Source == MAP_EVENT_SOURCE)
			{
				...
				if (Number)
				{
					...
					Add_located_effect(Number,
					 (UNSHORT) Event->Byte_3,
					 (UNSHORT) Event->Byte_4,
					 (UNSHORT) Event->Byte_5,
					 Event->Word_6,
					 (SILONG) X * Map_square_size,
					 (SILONG) Y * Map_square_size);
				}
				else
				{
					/* Remove -> Remove all sound effects at this position */
					Remove_located_effects_at((SILONG) X * Map_square_size,
					 (SILONG) Y * Map_square_size);
				}
			}

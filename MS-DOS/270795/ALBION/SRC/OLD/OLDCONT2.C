...
struct OPM *OPM_update_list[ROOTS_MAX][UPDATE_OPMS_MAX];
...
UNLONG Recording_buffer_size;
UNLONG Recording_buffer_index;
...
struct Mouse_pointer *Mouse_stack[MOUSE_POINTERS_MAX];
UNSHORT Mouse_stack_index = 0;

struct BBRECT *MA_stack[MA_MAX];
UNSHORT MA_stack_index = 0;
...
Start_recording(void)
{
	...
	Recording_buffer_handle = MEM_Allocate_memory(10000 * sizeof(struct Input_record));
...
{
	struct Input_record *Ptr;
	struct BBPOINT Mouse;
	UNSHORT Old, New;

	/* Get timer */
	Ticks = SYSTEM_GetTicks();

	/* Playing back an input recording ? */
	if (Playback)
	{
		/* Yes -> Are there still events in the recorded buffer ? */
		if (Recording_buffer_index < Recording_buffer_size)
		{
			/* Yes -> Playback */
			Ptr = (struct Input_record *) MEM_Claim_pointer(Recording_buffer_handle)
			 + Recording_buffer_index;

			if (Ptr->Ticks <= Ticks)
			{
				/* Get event */
				memcpy(Event, &(Ptr->Event), sizeof(struct Input_record));
	...
	else
	{
		...
		if (Recording)
		{
			/* Yes -> Is there still room in the recording buffer ? */
			if (Recording_buffer_index < Recording_buffer_size)
			{
				/* Yes -> Record */
				Ptr = (struct Input_record *) MEM_Claim_pointer(Recording_buffer_handle)
				...
				/* Put event */
				memcpy(&(Ptr->Event), Event, sizeof(struct Input_record));
				...
				if (Recording_buffer_index == Recording_buffer_size)
				{
					static UNCHAR Filename[] = "TEST.REC";
					UNBYTE *Source;

					Stop_recording();

					Source = MEM_Claim_pointer(Recording_buffer_handle);
					DOS_WriteFile(Filename, Source,
					 Recording_buffer_size * sizeof(struct Input_record));
					MEM_Free_pointer(Recording_buffer_handle);
					MEM_Free_memory(Recording_buffer_handle);
				}
			}
...
Wait_4_user(void)
{
	...
	while (TRUE)
	{
		...
		/* Exit if user clicked or pressed a key */
		if ((Event.sl_eventtype == BLEV_KEYDOWN)
		 || (Event.sl_eventtype == BLEV_MOUSELDOWN)
		 || (Event.sl_eventtype == BLEV_MOUSERDOWN))
			break;
 ...
 *              method, their children won't be checked.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
 ...
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - This function is needed for object methods.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
Init_mouse_pointer(struct Mouse_pointer *New)
{
	...
	UNLONG Size;
	UNBYTE *Ptr;
	static UNCHAR Info_chunk[] = "INFO", Body_chunk[] = "BODY", End_chunk[] = "ENDE";
	...
	Write_chunk_name(&(GTO->infochunk), Info_chunk);
	GTO->name[0] = 0;
	GTO->transcol = 0;
	GTO->plane = 0xFF;
	GTO->xoffset = 0 - New->X;
	...
	Write_chunk_name(&(GTO->bodychunk), Body_chunk);
...
{
	SYSTEMVAR_mouseclipx0 = New->left;
	SYSTEMVAR_mouseclipy0 = New->top;
	SYSTEMVAR_mouseclipx1 = New->left + New->width - 1;
	SYSTEMVAR_mouseclipy1 = New->top + New->height - 1;
}
...
Show_OPMs(void)
{
	...
	if (Main_OPM.status & OPMSTAT_CHANGED)
	{
		...
		for (i=0;i<UPDATE_OPMS_MAX;i++)
		{
			...
			{
				if ((OPM->status & OPMSTAT_VIRTUAL) && (OPM->virtualsrc == &Main_OPM))
	...
	else
	{
		...
		{
			/* No -> Copy all the OPMs to the screen (if changed) */
			for (i=0;i<UPDATE_OPMS_MAX;i++)
			{
				OPM = OPM_update_list[Root_stack_index][i];

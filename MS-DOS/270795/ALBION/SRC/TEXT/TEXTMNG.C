 ...
 UNSHORT Display_flags)
{
	...
	if (Processed->Text_height > (SISHORT) PA->Area.height)
	{
		...
		while ((Processed->Text_height - Current_text_Y) >
		 (SISHORT) (2 * PA->Area.height))
		{
		...
		for (;;)
		{
			...
			if (Current_text_Y >= Processed->Text_height -
			 (SISHORT) PA->Area.height)
			{
...
Erase_print_area(struct OPM *OPM)
{
	...
	if (Textstyle->Paper == 0xFFFF)
	{
		...
		OPM_CopyOPMOPM(&(PA_data->Background_OPM), OPM, 0, 0, PA->Area.width,
		 PA->Area.height, X + PA->Area.left, Y + PA->Area.top);
	}
	else
	{
		/* No -> Draw paper box */
		OPM_FillBox(OPM, X + PA->Area.left, Y + PA->Area.top, PA->Area.width,
...
Init_print_area(struct OPM *OPM)
{
	...
	if (Textstyle->Paper == 0xFFFF)
	{
		...
		PA_data->Background_handle = MEM_Allocate_memory(PA->Area.width
		 * PA->Area.height);

		/* Create background OPM */
		Ptr = MEM_Claim_pointer(PA_data->Background_handle);
		OPM_New(PA->Area.width, PA->Area.height, 1, &(PA_data->Background_OPM),
		 Ptr);
		...
		/* Save background */
		OPM_CopyOPMOPM(OPM, &(PA_data->Background_OPM), X + PA->Area.left,
		 Y + PA->Area.top, PA->Area.width, PA->Area.height, 0, 0);
	}
}

 ...
 MEM_HANDLE Graphics_handle, UNLONG Graphics_offset)
{
	...
	/* Do text window */
	Do_text_window_with_symbol(Text_ptr, Width, Height, Graphics_handle,
	 Graphics_offset);
...
Do_text_window(UNBYTE *Text_ptr)
{
	...
	/* Adjust PA */
	PA.Area.top = Window_Y + ((Window_height - 20) - Processed.Text_height) /
	 2 + 10;
	...
	Put_recoloured_box(&Main_OPM, TEXT_WINDOW_X + 7, Window_Y + 7,
	 TEXT_WINDOW_WIDTH - 14, Window_height - 14, &(Recolour_tables[0][0]));
	Draw_window_border(&Main_OPM, TEXT_WINDOW_X, Window_Y,
	 TEXT_WINDOW_WIDTH, Window_height);

	/* Print text */
	Display_processed_text(&Main_OPM, &Processed, WAIT_TEXT_DISPLAY_FLAG);
 ...
 MEM_HANDLE Graphics_handle, UNLONG Graphics_offset)
{
	...
	Draw_deep_box(&Main_OPM, TEXT_WINDOW_X + 9 + 3, Symbol_Y - 1, Width + 2,
	 Height + 2);
	...
	/* Draw symbol */
	Put_masked_block(&Main_OPM, TEXT_WINDOW_X + 10 + 3, Symbol_Y, Width,
	 Height, Ptr);

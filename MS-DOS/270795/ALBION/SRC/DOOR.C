...
UNLONG Highlight_Lock_object(struct Object *Object, union Method_parms *P);
...
//	{ HIGHLIGHT_METHOD, Highlight_Lock_object },
 ...
 UNSHORT Door_index, BOOLEAN Has_trap)
{
	...
	/* Is this door open ? */
	if (Read_bit_array(DOOR_BIT_ARRAY, Current_door_index) ||
...
Door_DisInit(void)
{
	...
	Door_object = Add_object(Earth_object, &Door_Class, NULL,
	...
	/* Draw */
	Execute_method(Door_object, DRAW_METHOD, NULL);
...
Draw_Lock_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Draw lock symbol */
	Put_masked_block(Current_OPM, Object->X + 6, Object->Y + 5, 22, 22,
	 &(Lock_symbol[0]));
...
UNLONG
Highlight_Lock_object(struct Object *Object, union Method_parms *P)
{
	/* Erase object */
	Restore_background(Object->Rect.left, Object->Rect.top, Object->Rect.width,
	 Object->Rect.height);
...
PUM_Pick_lock(UNLONG Data)
{
	...
	else
	{
		...
		{
			/* No -> Probe lock-picking skills of active character */
			if (Probe_skill(Active_char_handle, LOCK_PICKING))
			{
				/* Success -> "Picked the lock!" */
				Set_permanent_message_nr(522);

	...
	for (i=0;i<Nr_NPCs;i++)
	{
		...
		if (New_NPC_data_flag)
		{
			...
			if (NPC_ptr[i].New_NPC_data.Number)
			{
				...
				VNPCs[i].NPC_type			= (UNSHORT) NPC_ptr[i].New_NPC_data.Flags & NEW_NPC_TYPE;
	...
	for (i=0;i<NPCS_PER_MAP;i++)
	{
		...
		if (VNPCs[i].Number)
		{
			...
			if (Handle)
			{
				...
				VNPCs[i].Data._2D_NPC_data.NPC_object.Width	= Gfx->Width;
				VNPCs[i].Data._2D_NPC_data.NPC_object.Height	= Gfx->Height;

				MEM_Free_pointer(Handle);

				/* Set object icon height */
				VNPCs[i].Data._2D_NPC_data.NPC_object.Level = 0;

				/* Set graphics handle */
				VNPCs[i].Data._2D_NPC_data.NPC_object.Graphics_handle = Handle;
			}
			else
			{
				/* No -> Disable NPC */
...
Exit_2D_NPCs(void)
{
	...
	for(i=0;i<NPCS_PER_MAP;i++)
	{
		...
		if (VNPCs[i].Number)
		{
			...
			MEM_Free_memory(VNPCs[i].Data._2D_NPC_data.NPC_object.Graphics_handle);
		}
	}
}
	...
	for(j=0;j<NPCS_PER_MAP;j++)
	{
		...
		if (NPC_present(j))
		{
			 ...
			 (VNPCs[j].Next_X || VNPCs[j].Next_Y))
			{
				...
				VNPCs[j].Source_X = Current_X + ((SILONG) dX *
				 (SILONG) Current_substep) / SUBSTEPS_PER_STEP;
				VNPCs[j].Source_Y = Current_Y + ((SILONG) dY *
				 (SILONG) Current_substep) / SUBSTEPS_PER_STEP;
			}
			else
			{
...
Do_path_NPC_2D(UNSHORT NPC_index)
{
	...
	if ((Current_X || Current_Y) && (Next_X || Next_Y))
	{
		...
		if (Big_guy)
		{
			...
			Current_state = (Get_location_status(Current_X, Current_Y,
			...
			/* Set state */
			VNPCs[NPC_index].Data._2D_NPC_data.Move.State = Next_state;
		}
	}
}
...
Do_random_NPC_2D(UNSHORT NPC_index)
{
	...
	if (VNPCs[NPC_index].Rnd_path_length)
	{
		...
		VNPCs[NPC_index].Data._2D_NPC_data.Move.X				= VNPCs[NPC_index].Map_X;
		VNPCs[NPC_index].Data._2D_NPC_data.Move.Y				= VNPCs[NPC_index].Map_Y;
		...
		VNPCs[NPC_index].Map_X = VNPCs[NPC_index].Data._2D_NPC_data.Move.X;
		VNPCs[NPC_index].Map_Y = VNPCs[NPC_index].Data._2D_NPC_data.Move.Y;
...
Do_random_NPC_3D(UNSHORT NPC_index)
{
	...
	if (VNPCs[NPC_index].Rnd_path_length)
	{
		...
		if (!Choose_next_path)
		{
			VNPCs[NPC_index].Next_X = New_X;
			VNPCs[NPC_index].Next_Y = New_Y;
		}
	}
...
UNSHORT
Search_3D_NPC(UNSHORT X, UNSHORT Y, UNSHORT NPC_index)
{
	UNSHORT Found_NPC = 0xFFFF;
	UNSHORT i;
...
Turn_towards_NPC(UNSHORT NPC_index)
{
	...
	{
		/* 2D -> Calculate vector */
		dX = VNPCs[NPC_index].Map_X - PARTY_DATA.X;
		dY = VNPCs[NPC_index].Map_Y - PARTY_DATA.Y;

		/* Get new view direction */
		if (abs(dX) <= abs(dY))
		{
			tVD = Turn_view_directions[0][sgn(dY) + 1][sgn(dX) + 1];
		}
		else
		{
			tVD = Turn_view_directions[1][sgn(dY) + 1][sgn(dX) + 1];
		}

		/* Is there a need for rotation ? */
		if (tVD != PARTY_DATA.View_direction)
		{
			/* Yes -> Is the angle 180 degrees ? */
			if (abs((tVD + 2) - (PARTY_DATA.View_direction + 2)) == 2)
			{
				/* Yes -> Show extra rotation step */
				if ((tVD == NORTH) || (tVD == SOUTH))
				{
					if (dX > 0)

...
Handle_mouse_event(struct BLEV_Event_struct *Event)
{
	...
	Search_mouse_event_object(Method1, Root_stack[Root_stack_index]);
}
...
Search_mouse_method(UNSHORT Method, struct Object *Object)
{
	...
	if (Has_method(Object->Self, CUSTOMMOUSE_METHOD))
	{
		...
		/* Yes -> Get mouse event list */
		Mev_list = (struct Mev *) Execute_method(Object->Self, CUSTOMMOUSE_METHOD, 0L);

		/* Handle and exit if successful */
		if (Check_Mev_list(Mev_list))
		{
			BLEV_ClearAllEvents();
			return (TRUE);
		}
...
Handle_key_event(struct BLEV_Event_struct *Event)
{
	...
	while (Handle)
	{
		...
		if (Has_method(Handle,CUSTOMKEY_METHOD))
		{
			...
			/* Yes -> Get key event list */
			Kev_list = (struct Kev *) Execute_method(Handle, CUSTOMKEY_METHOD, 0L);

			/* Handle and exit if successful */
			if (Check_Kev_list(Event, Kev_list))
				return;
		}
		...
		if (Method1)
			...
			{
				/* Yes -> Execute */
				Execute_method(Handle, Method1, (union Method_parms *) &Parameter);
				return;
			}
 ...
 UNBYTE *OID)
{
	...
	Handle = Root_stack[Root_stack_index];
	if (!Handle)
	{
		/* Yes -> Bind (first object) */
		Root_stack[Root_stack_index] = Self;
	}
	else
	{
		/* No -> Add to layer 1 ? */
...
Pop_root(void)
{
	...
	if (Root_stack_index)
	{
		...
		if (Root_stack[Root_stack_index])
			Delete_object(Root_stack[Root_stack_index]);
...
Push_module(struct Module *Module)
{
	...
	if (Module_stack_index < OBJECTS_MAX-1)
	{
		  ...
		  module's entry is used. */
		{
			...
			Old_ptr = (UNLONG *) &(Old->DisUpd_function);
			New_ptr = (UNLONG *) &(New->DisUpd_function);
 			Mod_ptr = (UNLONG *) &(Module->DisUpd_function);
		...
		else
		{
			...
			/* No -> Increase local pop semaphore */
			Counter = Pop_counter;
			Counter++;
			Pop_counter = Counter;

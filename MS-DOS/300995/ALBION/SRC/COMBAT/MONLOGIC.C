...
struct Combat_participant *Select_strongest_target(struct Combat_participant *Part);
...
Get_monster_tactic(struct Combat_participant *Part)
{
	...
	/* Get tactic index */
	Tactic_index = Part->Tactic_index;
...
Select_monster_action(struct Combat_participant *Part)
{
	...
	/* Decide between offensive and defensive action */
	if (Tactic_ptr->Decider(Part))
	{
...
{
	static UNSHORT Capability_table[16] = {
...
Try_magical_attack(struct Combat_participant *Part)
{
	...
	if (Part->Target_part)
	{
		...
		if (Possible_spell_targets & ONE_ENEMY_TARGET)
		{
			...
			/* Select target */
			Use_magic_data_ptr->Combat_target_mask	= 1 <<
		...
		else
		{
			...
			if (Possible_spell_targets & ROW_ENEMIES_TARGET)
			{
				...
				{
					Target_mask |= 1 << ((NR_TACTICAL_COLUMNS * Selected_row) + i);
				}
	...
	{
		/* No -> Can cast on entire party ? */
		if (Possible_spell_targets & ALL_ENEMIES_TARGET)
		{
			/* Yes -> Choose spell */
			Selected_spell = Choose_monster_spell
			(
		...
		else
		{
			...
			if (Possible_spell_targets & ROW_ENEMIES_TARGET)
			{
				...
				if (Count_enemies_in_row(Part, 4) > Count_enemies_in_row(Part, 5))
					Selected_row = 4;
				else
					Selected_row = 5;
			...
			else
			{
				...
				Use_magic_data_ptr->Combat_target_mask	= (1 << Select_random_enemy(Part));
			}
		}
	}
 ...
 UNSHORT Weapon_item_slot_index)
{
	...
	if (Possible_targets)
	{
		...
		if (Part->Target_part)
		{
			...
			if (Possible_targets & (1 << Tactic_index))
			{
				/* Yes -> Attack it */
	...
	while (Step_index < Nr_steps)
	{
		...
		else
		{
			...
			if (Movement_range_mask &
			 (1 << (Tactic_index + NR_TACTICAL_COLUMNS)))
			{
				/* Yes -> Move down */
				Y++;
			}
		...
		/* No turning back! */
		Movement_range_mask &= ~(1 << Tactic_index);
...
Get_targets_within_movement_range(struct Combat_participant *Part)
{
	...
	for (Y=0;Y<NR_TACTICAL_ROWS;Y++)
	{
		for (X=0;X<NR_TACTICAL_COLUMNS;X++)
		{
			...
			if (Movement_range_mask & (1 << Tactic_index))
			{
 ...
 UNLONG Movement_mask)
{
	...
	if (Tactical_X > 0)
	{
		...
		if (Movement_mask & (1 << (Tactical_index - 1)))
		{
...
Default_decider(struct Combat_participant *Part)
{
	...
	Global_danger = (Remaining_monsters * 100) / Nr_monsters;

	/* Calculate local danger */
	Local_danger = (Get_LP(Part->Char_handle) * 100) /
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Select_strongest_target
 * FUNCTION  : Select the strongest target.
 * FILE      : MONLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 21.09.95 10:13
 * LAST      : 21.09.95 10:13
 *	INPUTS    : struct Combat_participant *Part - Pointer to participant data.
...
Select_strongest_target(struct Combat_participant *Part)
{
	struct Combat_participant *Current_part;
	struct Combat_participant *Target_part;
	UNSHORT Strength;
	UNSHORT Max_strength;
	UNSHORT i, j;
		...
		for (j=0;j<NR_TACTICAL_COLUMNS;j++)
		{
			...
			if (Current_part)
			{
				...
				if (Current_part->Type != Part->Type)
				{
					...
					/* Stronger than current maximum ? */
					if (Strength > Max_strength)
					{
						/* Yes -> This is the new maximum */

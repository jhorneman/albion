...
Init_COMOB_system(void)
{
	...
	/* Load stars */
	Result = Load_full_batch(COMBAT_GFX, NR_COMBAT_STARS, &(Star_gfx_batch[0]),
	 &(Star_gfx_handles[0]));
	if (!Result)
	{
	...
	/* Load sparks */
	Result = Load_full_batch(COMBAT_GFX, NR_COMBAT_SPARKS, &(Spark_gfx_batch[0]),
	 &(Spark_gfx_handles[0]));
	if (!Result)
	{
		Error(ERROR_FILE_LOAD);
	...
	Trans_table_handle = Load_subfile(TRANS_TABLE, Current_palette_nr);
		...
		if (Z_3D >= MINIMUM_Z)
		{
			 ...
			 (Draw_mode == BIG_PIXEL_COMOB_DRAWMODE))
			{
				...
				switch (Draw_mode)
				{
					...
					{
						OPM_SetPixel(&Main_OPM, X_2D, Y_2D, COMOB->Colour);
						break;
					}
					/* Big pixel */
					case BIG_PIXEL_COMOB_DRAWMODE:
					{
						OPM_FillBox(&Main_OPM, X_2D, Y_2D, 2, 2, COMOB->Colour);
						break;
					}
				}
			}
			else
			{
				...
				/* Get frame index */
				Frame_index = COMOB->Frame;

				/* Legal frame index ? */
				if (Frame_index >= Gfx->Nr_frames)
				{
					/* No -> Report error */
					Error(ERROR_ILLEGAL_COMOB_FRAME);

					/* Set frame index to safe value */
					Frame_index = 0;

					/* Store back in COMOB data */
					COMOB->Frame = Frame_index;
				}

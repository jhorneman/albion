...
void Show_participant_targets(struct Combat_participant *Part);
 ...
 * FUNCTION  : Show a participant's targets.
 * FILE      : TACTICAL.C
 * AUTHOR    : Jurie Horneman
...
void
Show_participant_targets(struct Combat_participant *Part)
{
	...
	(
		...
		(NR_TACTICAL_ROWS * TACTICAL_SQUARE_HEIGHT) + 2
	);
	...
	switch (Part->Current_action)
	{
		...
		case MOVE_COMACT:
		{
			...
			Target_square_index = Part->Target.Move_target_data.Target_square_indices[Part->Target.Move_target_data.Nr_moves - 1];
		...
		{
			/* Get target square index */
			Target_square_index = Part->Target.Attack_target_data.Target_square_index;
		...
		{
			/* Show magic targets */
			Show_participant_magic_targets(Part);
			break;
		}
	}

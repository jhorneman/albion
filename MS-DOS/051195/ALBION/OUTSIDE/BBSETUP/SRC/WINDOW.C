...
// Expression size * 1.6 changed to ((size * 16) / 10) - JH
// Expression MENU_BORDER_WIDTH * 1.4 changed to ((MENU_BORDER_WIDTH * 14) / 10) - JH
...
void GeneralWindow (UNCHAR *Text, UNCHAR *Headline, UNCHAR *Button, UNCHAR Color, UNCHAR ColorLight, UNCHAR ColorDark, UNCHAR ColorHeadlineBack, UNCHAR ColorHeadlineText, UNCHAR ColorButtonFrame, UNCHAR ColorButtonHighlight)
{
	...
	for (Size=10; Size<RESOLUTION_Y/2; Size+=WIN_EXPLODE_SPEED)
	{
		...
		if (!PrintCenteredTextInFrame (Headline,
												 ColorHeadlineText,
												 &SetupOpm,
				 	 							 RESOLUTION_X/2 - ((Size * 16) / 10) + (MENU_BORDER_WIDTH + 3),
		...
		//OK-Button machen:
		DrawButton (&SetupOpm,
					 	RESOLUTION_X/2 - Size + ((MENU_BORDER_WIDTH * 14) / 10)+1,
		...
		if (!PrintCenteredTextInFrame (Button,
									  			 BUTTON_COLOR_TEXT,
									  			 &SetupOpm,
					 	 						 RESOLUTION_X/2 - Size + ((MENU_BORDER_WIDTH * 14) / 10) + (1+1),
	...
	GetButtonAnswer (1,
						  0,
					 	  (SILONG)(RESOLUTION_X/2 - Size + ((MENU_BORDER_WIDTH * 14) / 10)+1),
	...
	while (1)
	{
		...
		if (kbhit())
		{
			...
			if (!(Button_state&1))
			{
				...
				{
					// Cast to SILONG added - JH
					for (d=0; d< (SILONG) strlen(pMenu->MenuEntry[c].Shortcut); d++)
					{
 ...
 */
	...
	SizeY+=(FONT_Y_SIZE+8)*3+20; 			/* Button zum Abbrechen einbeziehen	 */
...
BOOL SelectTargetDrive (SILONG NeededKbFree)
{
	...
	SILONG c;
	SILONG Free;
	// Unused variable union REGS Regs removed - JH
	...
	if (Text)
	{
		...
		for (Size=5; Size<RESOLUTION_Y/2; Size+=WIN_EXPLODE_SPEED)
		{
			...
			if (PrintTextInFrame (Text,
					  		  			BUTTON_COLOR_TEXT,
									  	&SetupOpm,
									 	RESOLUTION_X/2 - ((Size * 16) / 10) + MENU_BORDER_WIDTH+2,

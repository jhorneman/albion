...
/* Increase to accelerate spell learning */
#define MAGIC_TALENT_TO_STRENGTH_FACTOR	(4)
...
void Improve_spell_strength(MEM_HANDLE Char_handle, UNSHORT Class_nr,
 UNSHORT Spell_nr);
...
/* Flag needed for testing monster spells */
BOOLEAN Reverse_combat_magic_flag = FALSE;

/* Current use magic data */
struct Use_magic_data Current_use_magic_data;

/* Spells per spell class */
UNSHORT Spells_per_class[MAX_SPELL_CLASSES] = {
	21,
	11,
	10,
	15,
...
static struct Combat_XSpell Combat_spell_exceptions[] = {
	...
	{
		3, 8,
		Select_row_of_tactical_squares
	},
	{
		3, 9,
		Select_a_tactical_square
...
static Spell_handler Class_2_spell_handlers[10] = {
	C2_Spell_1_handler,
	C2_Spell_2_handler,
	C2_Spell_3_handler,
	C2_Spell_4_handler,
...
static Spell_handler Class_4_spell_handlers[4] = {
	C4_Spell_1_handler,
	C4_Spell_2_handler,
	C4_Spell_3_handler,
	C4_Spell_4_handler,
...
Cast_spell(UNSHORT Member_nr, UNSHORT Source_item_slot_index)
{
	...
	else
	{
		...
		{
			Item_data = Get_item_data(&(Char->Body_items[Source_item_slot_index - 1]));
		}
		else
		{
 ...
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - This function sets Current_use_magic_data.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
{
	static struct Event_action Cast_combat_spell_action =
	{
		0,	0, 0,
	...
	Cast_combat_spell_action.Actor_type		= Part->Type;
	Cast_combat_spell_action.Actor_index	= Part->Number;
	Cast_combat_spell_action.Action_value	= Current_use_magic_data.Class_nr;
	Cast_combat_spell_action.Action_extra	= Current_use_magic_data.Spell_nr;
	Cast_combat_spell_action.Action_data	= (void *) Part;

	/* Perform event action */
	Perform_action(&Cast_combat_spell_action);
}
	...
	if (In_Combat)
	{
		...
		switch (Spell_targets)
		{
			case ONE_FRIEND_TARGET:
			{
				...
				{
					/* Yes -> Select one */
					Square_index = Select_tactical_square
					(
						87,
			...
			case ITEM_TARGET:
			{
				...
				{
					/* Yes -> Select item */
					Target_item_slot_index = Select_party_member_item
					(
						Member_index,
						System_text_ptrs[91],
						NULL
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Select_row_of_tactical_square
 * FUNCTION  : Select a (possibly empty) row of tactical squares (combat
 *              spell target selection exception handler).
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 18.09.95 21:25
 * LAST      : 18.09.95 21:25
 * INPUTS    : UNSHORT Class_nr - Spell class index (0...MAX_SPELL_CLASSES).
 *             UNSHORT Spell_nr - Spell index (1...SPELLS_PER_CLASS).
 *             struct Use_magic_data *Use_magic_data_ptr - Pointer to use
...
UNLONG
Select_row_of_tactical_squares(UNSHORT Class_nr, UNSHORT Spell_nr,
 struct Use_magic_data *Use_magic_data_ptr)
{
	UNLONG Combat_mask = 0;
	UNSHORT Square_index;
 ...
 struct Use_magic_data *Use_magic_data_ptr)
{
	...
	(
		428,
		Show_potential_attack_targets,
		COMBAT_PARTY_MASK | COMBAT_MONSTER_MASK,
		0
	);
	...
	if (Target_square_index != 0xFFFF)
	{
		...
		else
		{
			...
			Destination_mask = COMBAT_PARTY_MASK & ~(Get_monster_targets() |
			 Get_party_targets() | Get_occupied_move_targets(Target_part));
		}

		/* Select destination */
		Destination_square_index = Select_tactical_square
		(
			429,
			Show_potential_attack_targets,
			Destination_mask,
		...
		{
			/* Yes -> Build target mask */
			Combat_mask = (1L << Target_square_index);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Do_mega_killer_victims
 * FUNCTION  : Select victims for the mega killer spell (combat spell target
 *              selection exception handler).
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 01.10.95 21:09
 * LAST      : 23.10.95 21:12
...
void
Select_mega_killer_target(UNSHORT Strength)
{
	struct Combat_participant *Part;
	UNLONG Random_bit_list;
	UNSHORT Remaining_monsters;
	UNSHORT Nr_victims;
	UNSHORT Victim_index;
	UNSHORT Bit_nr;
	UNSHORT i, j;
	...
	Remaining_monsters = Count_remaining_monsters();

	/* Determine number of victims */
	Nr_victims = max((Remaining_monsters * Strength) / 100, 1);

	/* Build random bit list */
	Random_bit_list = 0;
	for (i=0;i<Nr_victims;i++)
	{
		...
		{
			/* Select a random bit */
			Bit_nr = rand() % Remaining_monsters;
		}
		/* Until a cleared one is found */
		while (Random_bit_list & (1L << Bit_nr));

		/* Set bit */
		Random_bit_list |= (1L << Bit_nr);
	}
	...
	for (i=0;i<NR_TACTICAL_ROWS;i++)
	{
		for (j=0;j<NR_TACTICAL_COLUMNS;j++)
		{
			...
			if (Combat_matrix[i][j].Part)
			{
				...
				/* Is enemy ? */
				if (Part->Type && (Part->Type == MONSTER_PART_TYPE))
				{
					/* Yes -> Is victim ? */
					if (Random_bit_list & (1L << Victim_index))
					{
						/* Yes -> Store pointer */
						Current_target_parts[Current_nr_target_parts] = Part;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Do_all_magic_combat_targets
 ...
 Spell_per_combat_target_handler Handler)
{
	...
	for (i=0;i<NR_TACTICAL_ROWS;i++)
	{
		for (j=0;j<NR_TACTICAL_COLUMNS;j++)
		{
			...
			if (Combat_target_mask & (1L << (i * NR_TACTICAL_COLUMNS + j)))
			{
				/* Yes -> Get participant data */
				Part = Combat_matrix[i][j].Part;
	...
	if (!Nr_targets)
	{
		...
		(
			Current_use_magic_data.Casting_participant,
			NULL,
			0,
			698
		);
		...
		/* Reset action */
		Current_use_magic_data.Casting_participant->Current_action = NO_COMACT;
	}
}
...
Handle_spell(void)
{
	...
	if (Current_use_magic_data.Source_item_slot_index != 0xFFFF)
	{
		...
		(
			Current_use_magic_data.Casting_handle,
			Slot_index
		);
	}
	else
	{
		/* No -> Get LP & SP required for this spell */
		Required_LP = Calculate_required_LP
		(
			Current_use_magic_data.Casting_handle,
			Current_use_magic_data.Class_nr,
			Current_use_magic_data.Spell_nr
		);
		Required_SP = Get_spell_required_SP
		(
			Current_use_magic_data.Class_nr,
			Current_use_magic_data.Spell_nr
		);
		...
		{
			/* Yes -> Is party member ? */
			Member_nr = Find_char_data_in_party(Current_use_magic_data.Casting_handle);
			if (Member_nr != 0xFFFF)
			{
				/* Yes -> Remove required LP */
				Do_damage(Member_nr, Required_LP);

				/* Tell the player */
				Subject_char_handle = Current_use_magic_data.Casting_handle;
				Set_permanent_message_nr(725);
			}
			else
			{
				/* No -> Just remove LP */
				LP = max(Get_LP(Current_use_magic_data.Casting_handle) -
				 Required_LP, 0);
				Set_LP(Current_use_magic_data.Casting_handle, LP);
			}
		}
		...
		/* Get spell strength */
		Output_strength = (UNSHORT) max(((Get_spell_strength
		(
			Current_use_magic_data.Casting_handle,
			Current_use_magic_data.Class_nr,
			Current_use_magic_data.Spell_nr
		...
		/* Improve spell strength */
		Improve_spell_strength
		(
			Current_use_magic_data.Casting_handle,
			Current_use_magic_data.Class_nr,
			Current_use_magic_data.Spell_nr
 ...
 *             UNSHORT Item_slot_index - Item slot index (1...33).
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
void
Remove_used_magic_item(MEM_HANDLE Char_handle, UNSHORT Item_slot_index)
{
	struct Character_data *Char;
	...
	{
		Packet = &(Char->Backpack_items[Item_slot_index - ITEMS_ON_BODY - 1]);
	}

	/* Anything in packet ? */
	if (!Packet_empty(Packet))
	{
		/* Yes -> Does it have any charges ? */
		if (Packet->Charges)
		{
			/* Yes -> Get item data */
			...
			else
			{
				...
				/* Was this the last charge / destroy after use ? */
				if (!Packet->Charges &&
				 (Item_data->Flags & DESTROY_AFTER_USE))
				{
					/* Yes -> Remove item */
					Remove_item
					(
 ...
 *              (1...100).
 *             UNSHORT Immunity_mask - Mask of character type bits that
 *              give immunity against this spell.
 *             UNSHORT Non_immunity_mask - Mask of character type bits that
 *              give NO immunity against this spell.
 * RESULT    : UNSHORT : Remaining strength of spell (0...100).
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
Handle_magical_defense(struct Combat_participant *Defender_part,
 UNSHORT Incoming_strength, UNSHORT Immunity_mask, UNSHORT Non_immunity_mask)
{
	...
	/* Get character type bits */
	Char_bits = Get_character_type(Defender_part->Char_handle);

	/* Is the victim immune against this spell / not a party member ? */
	if (((Char_bits & Immunity_mask) || !(Char_bits & Non_immunity_mask)) &&
	 (Defender_part->Type != PARTY_PART_TYPE))
	{
		/* Yes -> "No effect" */
		Print_part_message(Defender_part, 774);

		/* Spell is deflected completely */
		Remaining_strength	= 0;
	}
	else
	{
		/* No -> Get magic resistance attribute of defender */
		Magic_resistance = Get_attribute
		(
			Defender_part->Char_handle,
			MAGIC_RESISTANCE
		);
		...
		if (Defender_part->Type == PARTY_PART_TYPE)
		{
			...
			Magic_resistance += (Get_member_temporary_spell_strength
			(
				Defender_part->Number,
		...
		if (Remaining_strength)
		{
			...
			if (Magic_resistance)
			{
				/* Yes -> "Couldn't deflect completely !" */
				Print_part_message(Defender_part, 630);
			}
			else
			{
				/* No -> "Couldn't deflect at all !" */
				Print_part_message(Defender_part, 418);
			}
		}
		else
		{
			/* Yes -> "Deflected!" */
			Print_part_message(Defender_part, 731);

			/* Show deflecting */
			P.Deflect_parms.Strength = Magic_resistance;
			Combat_show(Defender_part, SHOW_DEFLECT, &P);
		}
	}
 ...
 * LAST      : 23.10.95 22:26
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Class_nr - Spell class index (0...MAX_SPELL_CLASSES).
 *             UNSHORT Spell_nr - Spell index (1...SPELLS_PER_CLASS).
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
...
Learn_spell(MEM_HANDLE Char_handle, UNSHORT Class_nr, UNSHORT Spell_nr)
{
	...
	{
		/* Yes -> Is the spell already known ? */
		if (!(Spell_known(Char_handle, Class_nr, Spell_nr)))
		{
			/* No -> Indicate the spell is known */
			Char = (struct Character_data *) MEM_Claim_pointer(Char_handle);

			Char->xKnown_spells[Class_nr] |= (1L << Spell_nr);

			MEM_Free_pointer(Char_handle);

			/* Initialize spell strength */
			Set_spell_strength
			(
				Char_handle,
				Class_nr,
				Spell_nr,
				Get_attribute(Char_handle, MAGIC_TALENT) *
				 MAGIC_TALENT_TO_STRENGTH_FACTOR
			);
		}
	}
}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Improve_spell_strength
 * FUNCTION  : Improve a spell's strength.
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 05.10.95 14:59
 * LAST      : 05.10.95 14:59
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Class_nr - Spell class index (0...MAX_SPELL_CLASSES).
 *             UNSHORT Spell_nr - Spell index (1...SPELLS_PER_CLASS).
...
void
Improve_spell_strength(MEM_HANDLE Char_handle, UNSHORT Class_nr,
 UNSHORT Spell_nr)
{
	...
	if (Spell_exists(Class_nr, Spell_nr))
	{
		...
		Spell_strength = Get_spell_strength
		(
			Char_handle,
			Class_nr,
			Spell_nr
		...
		/* Increase spell strength */
		Spell_strength += (SILONG)(Get_attribute(Char_handle, MAGIC_TALENT) *
		 MAGIC_TALENT_TO_STRENGTH_FACTOR);

		/* Set new spell strength */
		Set_spell_strength
		(
			Char_handle,
			Class_nr,
 ...
 TEMP_SPELL_TYPE_T Temp_spell_index, UNSHORT Duration, UNSHORT Strength)
{
	...
	if (!PARTY_DATA.Temporary_spells[Char_nr - 1][Temp_spell_index].Duration)
	{
		/* No -> Set duration and strength */
		Set_temporary_spell
		(
			&PARTY_DATA.Temporary_spells[Char_nr - 1][Temp_spell_index],
			Duration,
			Strength
		);
	}
...
Select_spell(MEM_HANDLE Char_handle)
{
	...
	struct Spell_object_data Spell_info[MAX_SPELL_CLASSES * SPELLS_PER_CLASS];
					...
					if (Spell_exists(i, j))
					{
						...
						if (Spell_known(Char_handle, i, j))
						{
							...
							Spell_info[Counter].Strength =
							 max(Get_spell_strength(Char_handle, i, j) / 100, 1);
...
Init_Spell_window_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Spell_window->Nr_spells > MAX_SPELLS_IN_WINDOW)
	{
		...
		(
			Object->Self,
			SPELL_WIDTH + 27 + BETWEEN + SCROLL_BAR_WIDTH,
			(MAX_SPELLS_IN_WINDOW * (SPELL_HEIGHT + 1)) + 26
		);
		...
		/* Add object */
		Spell_window->Scroll_bar_object = Add_object
		(
			Object->Self,
			&Scroll_bar_Class,
			(UNBYTE *) &Scroll_bar_OID,
			SPELL_WIDTH + 13 + BETWEEN,
			12,
			SCROLL_BAR_WIDTH,
			(MAX_SPELLS_IN_WINDOW * (SPELL_HEIGHT + 1)) - 1
		);
...
Draw_spell(struct Spell_object *Spell)
{
	...
	if (Spell_window->Scroll_bar_object)
	{
		...
		(
			Spell_window->Scroll_bar_object,
			GET_METHOD,
			NULL
		);
	}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Spell_class_exists
 * FUNCTION  : Check if a spell class exists.
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 01.10.95 16:18
 * LAST      : 01.10.95 16:18
 * INPUTS    : UNSHORT Class_nr - Spell class number (0...MAX_SPELL_CLASSES - 1).
 * RESULT    : BOOLEAN : Spell class exists.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
Spell_class_exists(UNSHORT Class_nr)
{
	Spell_handler *List;
	BOOLEAN Exists = FALSE;

	/* Legal class number ? */
	if (Class_nr < MAX_SPELL_CLASSES)
	{
		/* Any spells in this class ? */
		if (Spells_per_class[Class_nr])
		{
			/* Get address of spell handler list for the selected class */
			List = Spell_handler_lists[Class_nr];

			/* Does this list exist ? */
			if (List)
			{
				/* Yes -> The spell class exists */
				Exists = TRUE;
			}
		}
	}
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Spell_exists
 * FUNCTION  : Check if a spell exists.
 * FILE      : MAGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 01.10.95 16:19
 * LAST      : 01.10.95 16:19
 * INPUTS    : UNSHORT Class_nr - Spell class index (0...MAX_SPELL_CLASSES).
 *             UNSHORT Spell_nr - Spell index (1...SPELLS_PER_CLASS).
 * RESULT    : BOOLEAN : Spell exists.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
Spell_exists(UNSHORT Class_nr, UNSHORT Spell_nr)
{
	Spell_handler *List;
	Spell_handler Handler;
	BOOLEAN Exists = FALSE;
	...
	{
		/* Legal spell number ? */
		if (Spell_nr && (Spell_nr <= SPELLS_PER_CLASS))
		{
			/* Yes -> Does a handler for this spell exist ? */
			List = Spell_handler_lists[Class_nr];
			if (List)
			{
				Handler = List[Spell_nr - 1];
				if (Handler)
				{
					/* Yes -> The spell exists */
					Exists = TRUE;
				}
			}
		}

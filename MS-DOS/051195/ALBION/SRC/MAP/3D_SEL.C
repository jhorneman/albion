		 ...
		 (1 << TAKE_TRIGGER)))
		{
			...
			else
			{
				/* No -> Check the line between the party and the selected map
					square */
				if (Check_line_to_selected_map_square() && !Cheat_mode)
				{
					/* Blocked */
 ...
 *              will fail when the selected map square is itself blocked.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
{
	return Check_if_line_in_3D_map_is_blocked
	(
		Current_map_selection_data.Map_X,
		Current_map_selection_data.Map_Y,
		PARTY_DATA.X,

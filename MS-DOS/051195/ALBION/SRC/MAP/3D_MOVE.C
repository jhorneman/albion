...
#define TURN_3D_SPEED_MOUSE				(60)
#define MOVE_FORWARD_3D_SPEED_MOUSE		(230)
#define MOVE_LATERAL_3D_SPEED_MOUSE		(100)
#define MOVE_BACKWARD_3D_SPEED_MOUSE	(150)

#define TURN_3D_SPEED_KEY					(40)
#define MOVE_FORWARD_3D_SPEED_KEY		(166)
#define MOVE_LATERAL_3D_SPEED_KEY		(67)
#define MOVE_BACKWARD_3D_SPEED_KEY		(100)
...
Get_3D_mouse_state2(SISHORT X, SISHORT Y)
{
	...
	if (Q < 12)
	{
		...
		/* Is the mouse on the left side of the 3D map window ? */
		if (X < CORNER_3D_SIZE)
		{
			...
			{
				/* No -> Is it in the bottom-left corner ? */
		...
		{
			/* Is it in the middle of the 3D map window ? */
			if ((X >= (Width / 2) - (Width / 10)) &&
			 (X < (Width / 2) + (Width / 10)))
			{
				/* Yes -> Is it in the top middle ? */
				if (Y < 2)
				{
					/* Yes */
		 			Q = 256 + 1;
			...
			{
				/* No -> Is the mouse on the right side of the 3D map window ? */
				if (X >= (Width - CORNER_3D_SIZE))
				{
					/* Yes -> Is it in the top-right corner ? */
					if (Y < CORNER_3D_SIZE)
					{
						/* Yes */
		 				Q = 256 + 2;
					...
					{
						/* No -> Is it in the bottom-right corner ? */
						if (Y >= (Height - CORNER_3D_SIZE))
						{
							/* Yes */
		 					Q = 256 + 5;
...
Mouse_move_3D(void)
{
	...
	switch (Q)
	{
		...
		case 0:
		{
			...
			F = (TURN_3D_SPEED_MOUSE * Duration * (Dom - X)) / Dom;

			Rotate_3D(F);

			Dom = ((Height * 60) / MOVE_LATERAL_3D_SPEED_MOUSE);

			F = (MOVE_FORWARD_3D_SPEED_MOUSE * Duration * (Dom - Y)) / Dom;
		...
		case 2:
		{
			...
			F = (TURN_3D_SPEED_MOUSE * Duration * (X - (Width - Dom))) / Dom;
		...
		case 6:
		{
			...
			F = (MOVE_LATERAL_3D_SPEED_MOUSE * Duration * (Dom - X)) / Dom;
		...
		case 8:
		{
			...
			F = (MOVE_LATERAL_3D_SPEED_MOUSE * Duration * (X - (Width - Dom))) / Dom;
		...
		case 9:
		{
			...
			F = (MOVE_BACKWARD_3D_SPEED_MOUSE * Duration * (Y - (Height - Dom))) / Dom;
...
Key_move_3D(void)
{
	...
	if (pressedkeytab[RAW_LEFT])
	{
		...
		{
			/* Yes -> Move left */
			F = MOVE_LATERAL_3D_SPEED_KEY * Duration;
	...
	{
		/* Yes -> Move forward */
		F = MOVE_FORWARD_3D_SPEED_KEY * Duration;
	...
	else
	{
		...
		{
			/* Yes -> Move backward */
			F = MOVE_BACKWARD_3D_SPEED_KEY * Duration;
...
{
	static const struct Collision_3D Collision_table[8] = {
		{ -1,-1, 0x0001 },
		{  0,-1, 0x0007 },
		{  1,-1, 0x0004 },
		{ -1, 0, 0x0049 },
...
//		Edge = f / 4;
		...
		else
		{
			...
			Ptr += (Nr_3D_object_groups * sizeof(struct Object_group_3D)) + 2;
			Ptr += (Nr_3D_floors * sizeof(struct Floor_3D)) + 2;
			...
			if (Result)
			{
				...
				if (NPC_index != 0xFFFF)
				{
					 ...
					 (Group_Z < f))
					{
						...
						Object_group_ptr = &(Object_group_base[VNPCs[NPC_index].Graphics_nr - 1]);
						if (Check_3D_object_group_collision
						(
							Square_X,
							Square_Z,
				...
				if (Result)
				{
					...
					for (i=0;i<NPCS_PER_MAP;i++)
					{
						...
						if ((NPC_present(i)) && (i != NPC_index))
						{
							...
							{
								/* No -> Check for collision */
								Object_group_ptr = &(Object_group_base[VNPCs[i].Graphics_nr - 1]);
								if (Check_3D_object_group_collision
								(
									Square_X,
									Square_Z,
 ...
 UNSHORT Travel_mode)
{
	...
	if (c >= FIRST_WALL)
	{
		...
		/* Blocked ? */
		if (Status & (1L << (BLOCKED_TRAVELMODES_B + Travel_mode)))
		{
			/* Yes */
			Result = FALSE;
		}
 ...
 struct Object_group_3D *Object_group, struct Object_3D *Object_base)
{
	...
	for (i=0;i<OBJECTS_PER_GROUP;i++)
	{
		...
		if (Object_nr)
		{
			...
			/* Blocking ? */
			if (Object_ptr->Flags & (1L << (BLOCKED_TRAVELMODES_B + Travel_mode)))
			{
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Check_if_line_in_3D_map_is_blocked
 * FUNCTION  : Check if a line between two positions in a 3D map is blocked.
 * FILE      : 3D_MOVE.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 27.10.95 14:24
 * LAST      : 27.10.95 14:24
 * INPUT     : UNSHORT Source_X - Source X-coordinate.
 ...
 * BUGS      : No known.
 * NOTES     : - It is essential that the line is traced first, and THEN the
 *              map square is tested. If done the other way around, this test
 *              will fail when the source map square is itself blocked.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
Check_if_line_in_3D_map_is_blocked(UNSHORT Source_X, UNSHORT Source_Y,

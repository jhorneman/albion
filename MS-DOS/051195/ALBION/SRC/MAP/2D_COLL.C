...
{
	static const UNSHORT Target_VDs[8] = {
...
{
	static const SISHORT Big_diagonal_check_offsets[4][2] = {
		{0, 1}, {1, 1}, {2, 1}, {2, 0}
	};
...
{
	static const SISHORT Small_diagonal_check_offsets[3][2] = {
		{0,1}, {1,1}, {1,0}
	};
...
Extra_vertical_movement_check_2D(UNSHORT X, UNSHORT Y, UNSHORT NPC_index)
{
	...
	if (Big_guy)
	{
		...
		if ((StatusL & (1L << (BLOCKED_DIRECTIONS_B + EAST))) ||
		 (StatusR & (1L << (BLOCKED_DIRECTIONS_B + WEST))))
			return FALSE;
	}
...
Do_check_2D(struct Movement_2D *Move, UNSHORT X, UNSHORT Y, SISHORT dX,
 SISHORT dY)
{
	static const UNSHORT Directions[3][3] = {
		{7, 0, 1},
		{6, 8, 2},
		{5, 4, 3}
	};
	static const UNSHORT Direction_pairs[8][2] = {
	...
	if ((Status & (1L << (BLOCKED_DIRECTIONS_B + Direction_pairs[Dir1][0])))
	 && (Status & (1L << (BLOCKED_DIRECTIONS_B + Direction_pairs[Dir1][1]))))
		return FALSE;
	...
	if ((Status & (1L << (BLOCKED_DIRECTIONS_B + Direction_pairs[Dir2][0])))
	 && (Status & (1L << (BLOCKED_DIRECTIONS_B + Direction_pairs[Dir2][1]))))
		return FALSE;

	/* Check if the new location is accessible for the current travelmode */
	if (Status & (1L << (BLOCKED_TRAVELMODES_B + Move->Travel_mode)))
		return FALSE;
	else
		return TRUE;
}

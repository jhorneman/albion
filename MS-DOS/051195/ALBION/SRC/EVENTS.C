...
Map_Exit_event(void)
{
	 ...
	 (MX_type == NORMAL_XY_REL_MX_TYPE))
	{
		...
		New_Y = PARTY_DATA.Y + (SISHORT) ((SIBYTE) Event->Byte_2);
	}
	else
	{
...
Chest_event(void)
{
	...
	/* Enter Chest screen */
	Chest_status = Do_Chest
	(
		(UNSHORT) Event->Byte_1,
		(UNSHORT) Event->Byte_2,
		(UNSHORT) Event->Byte_3,
		(UNSHORT) Event->Byte_4,
		(UNSHORT) Event->Byte_5,
...
Text_event(void)
{
	...
	switch (Event->Byte_1)
	{
		...
		case NORMAL_TEXT_TYPE:
		{
			...
			/* In a dialogue ? */
			if (Current_screen_type(0) == DIALOGUE_SCREEN)
			{
...
Spinner_event(void)
{
	...
	/* Choose a random view direction ? */
	if (Event->Byte_1 == 4)
	{
		/* Yes -> Choose a view direction different from the current one */
		Target_VD = PARTY_DATA.View_direction;
		while (Target_VD == PARTY_DATA.View_direction)
		{
			Target_VD = rand() % 4;
		}
	}
	...
	{
		/* No -> Get view direction from event block */
		Target_VD = (UNSHORT) Event->Byte_1;
	}

	/* 3D map ? */
	if (_3D_map)
	{
		/* 3D -> Calculate target angle */
		Target_angle = (SILONG) ((0 - (Target_VD * (ANGLE_STEPS / 4)))
		 & (ANGLE_STEPS - 1));
		...
		{
			/* Yes -> Turn which way ?
			  (Note that this is the reverse of the Turn_towards_NPC logic.
				This function will always take the longer way.) */
			if (Diff2 > Diff1)
			{
				Angle_direction = 1;
			}
			...
			/* First make two full rotations */
			for (i=0;i<(ANGLE_STEPS * 2) / ROTATE_3D_SPEED;i++)
			{
				/* Rotate */
				Current_angle += ROTATE_3D_SPEED * Angle_direction;
				Current_angle &= ANGLE_STEPS - 1;
			...
			/* Turn towards target view direction */
			dAngle = abs(Current_angle - Target_angle);
			while (dAngle)
			{
				...
				if (dAngle > ROTATE_3D_SPEED)
					...
					Current_angle += dAngle * Angle_direction;
...
Trap_event(void)
{
	...
	/* Can the active member still be active ? */
	if (Get_conditions(Active_char_handle) & ACTIVE_MASK)
	{
...
Do_trap(UNSHORT Member_nr, struct Event_block *Event)
{
	 ...
	 (!(Get_conditions(Char_handle) & DEAD_MASK)))
	{
		...
		if (Probe_attribute(Char_handle, LUCK))
		{
			...
			Do_text_window(System_text_ptrs[720]);
		}
		else
		{
			/* No -> Condition trap ? */
...
Encounter_event(void)
{
	...
	/* Enter combat screen */
	Status = Enter_Combat(Monster_group_nr, Combat_background_nr, 0xFFFF);

	/* Restart / quit / Game over ? */
	if (Restart_game_flag || Quit_program_flag || Game_over_flag)
	{
		/* Yes -> Break current event context */
		Break_current_context();
	}
	...
	{
		/* No -> Did the party win / did combat just end ? */
		if ((Status == COMBAT_PARTY_WON) || (Status == COMBAT_ENDED))
		{
...
Signal_event(void)
{
	...
	if (Context->Source == SET_EVENT_SOURCE)
	{
		...
		switch (Context->Source_data.Set_source.Char_type)
		{
			...
			{
				/* Are we in a dialogue / talking with a party character /
				  talking with this character ? */
				if (In_Dialogue &&
				 (Dialogue_char_type == PARTY_CHAR_TYPE) &&
				 (Dialogue_char_index == Context->Source_data.Set_source.Char_index))
				{
					/* Yes -> Set dialogue actor data */
					Signal_action.Actor_type	= DIALOGUE_ACTOR_TYPE;
					Signal_action.Actor_index	= 0;
				}
				else
				{
					/* No -> Determine party member index */
					Member_nr = Find_character_in_party(Context->Source_data.Set_source.Char_index);

					/* Is in the party ? */
					if (Member_nr != 0xFFFF)
					{
						/* Yes -> Set party member actor data */
						Signal_action.Actor_type	= PARTY_ACTOR_TYPE;
						Signal_action.Actor_index	= Member_nr;
					}
				}
				break;
			}
			...
			{
				/* Actor data cannot be determined at this time */
				break;
			}
		}
	}
		...
		case LOCATED_EFFECT_SOUND_TYPE:
		{
			...
			if (Context->Source == MAP_EVENT_SOURCE)
			{
				...
				else
				{
					...
					SOUND_Remove_located_effects_at(X, Y);
				}
			}
			break;
		}
...
Remove_Member_event(void)
{
	...
	/* Find character in party */
	Char_index = (UNSHORT) Event->Byte_1;
	...
	if (Member_index != 0xFFFF)
	{
		...
		switch (Event->Byte_2)
		{
			...
			case 3:
			{
				/* Set CD bit number to disappear */
		...
		/* Give items back ? */
		if ((Event->Byte_2 == 0) || (Event->Byte_2 == 2))
		{
			/* Yes -> Load character data */
			Char_handle = Load_subfile(PARTY_CHAR, Char_index);

			/* Success ? */
			if (Char_handle)
			{
				/* Yes -> Distribute items */
				Distribute_char_items(Char_handle);
...
Single_Item_Chest_event(void)
{
	...
	if (Context->Source == MAP_EVENT_SOURCE)
	{
		...
		if (!_3D_map)
		{
			...
			Convert_2D_map_to_screen_coordinates
			(
				Context->Source_data.Map_source.X,
				Context->Source_data.Map_source.Y,
				&Source_X,
				&Source_Y
			);
		}

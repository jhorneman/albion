...
Merchant_ModInit(void)
{
	...
	Exit_inventory_button_OID.Help_message_nr	= 420;
...
PUM_Buy_merchant_item(UNLONG Data)
{
	...
	struct Item_data *Item_data;
	struct Item_slot_object *Source_item_slot;
	...
	if (Quantity)
	{
		...
		if (Place_buy_requester(Quantity * Item_price))
		{
			...
			/* Find source item slot */
			Source_item_slot = Find_item_slot_object(Merchant_item_list_object,
			 Descriptor->Item_slot_index);

			/* Find target object */
			Target_object = Get_object_data(Party_member_objects[Inventory_member - 1]);

			/* Show moving items */
			Show_moving_item
			(
				&Bought_packet,
				Source_item_slot->Object.Rect.left + 1,
				Source_item_slot->Object.Rect.top + 1,
			...
			(
				...
				0,
				Bought_packet.Quantity,
				&Bought_packet
			);

			/* Is this an infinite item slot ? */
			if (Packet->Quantity != 255)
			{
				/* No -> Remove items from merchant */

...
/* Word list entry flags */
#define NEW_WORD			(1 << 0)
#define USED_NEW_WORD	(1 << 1)
...
UNLONG Exit_Word_window_object(struct Object *Object, union Method_parms *P);
...
UNCHAR *HLC_Sex_char(UNCHAR *Text);
...
static UNCHAR HLC_String[100];

static UNCHAR Entered_word[WORD_LENGTH] = "";

/* Dictionary variables */
static UNSHORT Merged_dictionary_index_list[MAX_WORDS];
...
static struct Method Word_window_methods[] = {
	...
	{ EXIT_METHOD,			Exit_Word_window_object },
...
static struct Error_message _Gametext_errors[] = {
	...
	{ERROR_TOO_MANY_WORDS,				"Too many words in merged dictionary."},
	{0, NULL}
};
...
Init_game_texts(void)
{
	...
	System_text_handle = Load_file(SYSTEM_TEXTS);
	...
	(
		Ptr,
		MEM_Get_block_size(System_text_handle),
		System_text_ptrs,
		MAX_SYSTEM_TEXTS
	);
...
Init_dictionary(void)
{
	...
	Merged_dictionary_handle = MEM_Allocate_memory(MAX_WORDS * WORD_LENGTH);
...
Exit_dictionary(void)
{
	...
	MEM_Kill_memory(Merged_dictionary_handle);
...
{
	static const UNCHAR *Illegal_word = "ILLEGAL";
	...
	(
		Word_ptr,
		Dictionary_ptr + (Merged_word_index * WORD_LENGTH),
		WORD_LENGTH - 1
	);

	/* Insert EOL just to be sure */
 ...
 UNSHORT Max_words)
{
	...
	Nr_words = min(MAX_WORDS - First_word, Max_words);
...
{
	BOOLEAN Found_word;
	UNSHORT New_word_length;
	UNSHORT i;
	UNCHAR *Word_ptr;

	/* Is the merged dictionary full ? */
	if (Nr_merged_words == MAX_WORDS)
	{
		/* Yes -> Error */
		Gametext_error(ERROR_TOO_MANY_WORDS);
		return;
	}

	/* Get length of new word */
	New_word_length = strlen(New_word_ptr);

	/* Exit if new word is empty */
	if (!New_word_length)
		return;
	...
	for (i=0;i<Nr_merged_words;i++)
	{
		/* Does these words have the same length ? */
		if (strlen(Word_ptr) == New_word_length)
		{
			/* Yes -> Are these words the same ? */
			if (!strncmp(Word_ptr, New_word_ptr, New_word_length))
			{
	...
	{
		/* No -> Add a new word */
		strncpy(Word_ptr, New_word_ptr, WORD_LENGTH - 1);
...
Init_Word_window_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Word_window->Nr_words > MAX_WORDS_IN_WINDOW)
	{
		...
		(
			Object->Self,
			WORD_OBJ_WIDTH + 27 + BETWEEN + SCROLL_BAR_WIDTH,
			(MAX_WORDS_IN_WINDOW * (WORD_OBJ_HEIGHT + 1)) + 42
		);
		...
		/* Add object */
		Word_window->Scroll_bar_object = Add_object
		(
			Object->Self,
			&Scroll_bar_Class,
			(UNBYTE *) &Scroll_bar_OID,
			WORD_OBJ_WIDTH + 13 + BETWEEN,
			12,
			SCROLL_BAR_WIDTH,
			(MAX_WORDS_IN_WINDOW * (WORD_OBJ_HEIGHT + 1)) - 1
		);
	...
	(
		Object->Self,
		&Button_Class,
		(UNBYTE *) &Word_input_button_OID,
		13 + (WORD_OBJ_WIDTH - 70) / 2,
		Y + 5,
		70,
		13
	);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Exit_Word_window_object
 * FUNCTION  : Exit method of Word Window object.
 * FILE      : GAMETEXT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 10.09.95 12:52
 * LAST      : 10.09.95 12:52
 * INPUTS    : struct Object *Object - Pointer to object.
 *             union Method_parms *P - Method parameters.
 * RESULT    : UNLONG : 0.
...
UNLONG
Exit_Word_window_object(struct Object *Object, union Method_parms *P)
{
	/* Destroy word list */
	MEM_Free_memory(Word_list_handle);
...
Build_word_list(void)
{
	...
	Word_list_handle = MEM_Allocate_memory(MAX_WORDS *
	 sizeof(struct Word_list_entry));

	/* Clear word list */
	MEM_Clear_memory(Word_list_handle);
		...
		for (j=0;j<MAX_WORDS;j++)
		{
			...
			if (Merged_dictionary_index_list[j] == i)
			{
				...
				if (Cheat_mode || Read_bit_array(KNOWN_WORDS_BIT_ARRAY, j))
				{
					...
					if (Read_bit_array(USED_NEW_WORDS_BIT_ARRAY, j))
					{
						/* Yes -> Mark */
						Word_list_ptr->Flags |= USED_NEW_WORD;
					}
...
Swap_words(UNLONG A, UNLONG B, UNBYTE *Data)
{
	...
	Word_list_ptr = (struct Word_list_entry *) MEM_Claim_pointer(Word_list_handle);

	memcpy
	(
		(UNBYTE *) &T,
		(UNBYTE *) (Word_list_ptr + A),
		sizeof(struct Word_list_entry)
	);
	memcpy
	(
		(UNBYTE *) (Word_list_ptr + A),
		(UNBYTE *) (Word_list_ptr + B),
		sizeof(struct Word_list_entry)
	);
	memcpy
	(
...
Compare_words(UNLONG A, UNLONG B, UNBYTE *Data)
{
	...
	/* Do both words have the same flags ? */
	if ((Word_list_ptr + A)->Flags == (Word_list_ptr + B)->Flags)
	{
	...
	{
		/* No -> Are both new words ? */
		if (((Word_list_ptr + A)->Flags & NEW_WORD) ==
		 ((Word_list_ptr + B)->Flags & NEW_WORD))
		{
			/* Yes -> Used new word gets lower priority */
			Result = (BOOLEAN) ((Word_list_ptr + A)->Flags & USED_NEW_WORD);
		}
		else
		{
			/* No -> New word gets higher priority */
			Result = (BOOLEAN) !((Word_list_ptr + A)->Flags & NEW_WORD);
		}
	}
...
Word_input(struct Button_object *Button)
{
	...
	/* Enter a word */
	Entered_word[0] = 0;
	Input_string_in_window(WORD_LENGTH - 1, NULL, Entered_word);

	/* Any word entered ? */
	if (Entered_word[0])
	{
		/* Yes -> Search word in merged dictionary */
		Merged_word_index = Search_word_in_dictionary(Entered_word);
...
Draw_word(struct Word_object *Word)
{
	...
	if (Word_list_ptr->Flags & NEW_WORD)
	{
		if (Word_list_ptr->Flags & USED_NEW_WORD)
		{
			Set_ink(SILVER_TEXT);
		}
		else
	...
	(
		...
		Object->X + 2,
		Object->Y + 1,
		Dictionary_ptr + (Word_list_ptr->Merged_word_index * WORD_LENGTH)
	);
...
{
	/* Convert current place price */
	_bprintf
	(
		HLC_String,
		100,
		"%lu.%u",
		Place_total_price / 10,
		(UNSHORT)(Place_total_price % 10)
	);
...
UNCHAR *
HLC_Sex_char(UNCHAR *Text)
{
	static UNCHAR Male_char_string[] = "$";
	static UNCHAR Female_char_string[] = "@";
	...
	switch (Get_sex(HLC_Subject_handle))
	{
		case MALE:
		{
			String = Male_char_string;
			break;
		}
		case FEMALE:
		{
			String = Female_char_string;
			break;
		}
	}

...
/* Event action stack */
struct Event_action Event_action_stack[MAX_EVENT_ACTIONS];
UNSHORT Event_action_stack_index = 0;

/* Event context stack */
struct Event_context Event_context_stack[MAX_EVENT_CONTEXTS];
	...
	for (;;)
	{
		...
		if (Event_set_ptr->Type == 0xFFFF)
		{
			 ...
			 && !(Event_action->Flags & FORBIDDEN))
			{
				...
				{
					/* Yes -> Execute */
					Success = (Event_action->Action_ptr)(Event_action);
...
Handle_map_set(UNSHORT Value)
{
	...
	Context_index = Event_context_stack_index;
	while (Context_index >= 0)
	{
		/* Is this a map context ? */
		Context = &(Event_context_stack[Context_index]);
 ...
 * FUNCTION  : Handle the acting (NOT acTIVE!!!) member's event set.
 * FILE      : EVELOGIC.C
 * AUTHOR    : Jurie Horneman
...
Handle_acting_member_set(UNSHORT Value)
{
	...
	/* Is there an acting member ? */
	if (Event_action->Actor_type == PARTY_ACTOR_TYPE)
	{
		/* Yes -> Get member index */
		Member_nr = Event_action->Actor_index;

		/* Does this member exist ? */
		if (Member_present(Member_nr))
		{
			/* Yes -> Is this member capable of acting ? */
			if (Character_capable(Party_char_handles[Member_nr - 1]))
			{
				/* Yes -> Search event sets of acting member */
				Search_member_sets(Member_nr, ME_ACTOR_FLAG);
			}
		}
	}
}
...
Handle_non_acting_member_set(UNSHORT Value)
{
	...
	if ((Event_action->Actor_type != PARTY_ACTOR_TYPE) ||
	 (Event_action->Actor_index != Value))
	{
...
Handle_acting_part_set(UNSHORT Value)
{
	...
	switch (Event_action->Actor_type)
	{
		...
		case MONSTER_ACTOR_TYPE:
		{
			...
			if (Character_capable(Monster_parts[Index - 1].Char_handle))
			{
				/* Yes -> Search the acting monster's set */
...
Handle_friend_part_set(UNSHORT Value)
{
	...
	switch (Event_action->Actor_type)
	{
		...
		case MONSTER_ACTOR_TYPE:
		{
			...
			if (Monster_parts[Value - 1].Type != EMPTY_PART_TYPE)
			{
				/* Yes -> Is this the acting monster ? */
				if (Value != Index)
				{
					/* No -> Is this monster capable of acting ? */
					if (Character_capable(Monster_parts[Value - 1].Char_handle))
					{
 ...
 *             UNSHORT Chain_nr - Event chain number (0...).
...
Push_event_action(void)
{
	...
	if (Event_action_stack_index < MAX_EVENT_ACTIONS)
	{
...
Execute_event_chain(void)
{
	...
	if (i != 0xFFFF)
	{
		...
		for(;;)
		{
			...
			/* Break event chain / quit / Game over / restart game ? */
...
{
	/* Is there room on the stack ? */
	if (Event_context_stack_index < MAX_EVENT_CONTEXTS)
	{
		/* Yes -> Increase stack index */
 ...
 *             UNSHORT Chain_nr - Chain number (0...).
 * RESULT    : BOOLEAN : Event is saved.
 * BUGS      : No known.
...
Event_is_saved(UNSHORT Map_nr, UNSHORT Chain_nr)
{
	...
	{
		/* Yes -> Build bit number */
		Bit_nr = ((Map_nr - 1) * EVENTS_PER_MAP) + Chain_nr;

		/* Has this event been saved ? */
		Result = Read_bit_array(EVENT_SAVE_BIT_ARRAY, Bit_nr);
	}
 ...
 UNSHORT Value)
{
	...
	/* Does this NPC exist ? */
	if (NPC_present(NPC_index))
	{
		/* Yes -> Does this NPC have an event ? */
		if (First_block_nr != 0xFFFF)
		{
 ...
 UNSHORT Trigger_mode)
{
	...
	for(i=0;i<NPCS_PER_MAP;i++)
	{
		...
		{
			/* Yes -> Right coordinates / right trigger mode / has event /
			 not a monster / hasn't just moved to a new map square ? */
			if ((VNPCs[i].Map_X == X) &&
			 (VNPCs[i].Map_Y == Y) &&
...
Get_NPC_event_triggers(UNSHORT X, UNSHORT Y, UNSHORT NPC_index)
{
	...
	for(i=0;i<NPCS_PER_MAP;i++)
	{
		...
		if ((NPC_present(i)) && (i != NPC_index))
		{
			/* Yes -> Right coordinates / has event / not a monster ? */
			if ((VNPCs[i].Map_X == X) &&
			 (VNPCs[i].Map_Y == Y) &&
			 (VNPCs[i].First_block_nr != 0xFFFF) &&
			 (VNPCs[i].NPC_type != MONSTER_NPC))
...
Execute_chains_in_map(UNSHORT Trigger_mode)
{
	...
	else
	{
		...
		for (i=0;i<NPCS_PER_MAP;i++)
		{
			...
			if (NPC_present(i))
			{
				/* Yes -> Has event / right trigger mode / not a monster ? */
				 ...
				 (VNPCs[i].NPC_type != MONSTER_NPC))
				{
					...
					/* Has this event been saved ? */
					if (!Event_is_saved(Old_map_nr, Chain_nr))
					{
						/* No -> Make a new context */
						Context = Push_event_context();
						...
						Context->Source_data.Map_source.Map_nr 	= Old_map_nr;
						Context->Source_data.Map_source.Trigger 	= Trigger_mode;
						Context->Source_data.Map_source.Value 		= 0;
 ...
 * LAST      : 12.08.95 13:56
 * INPUTS    : UNSHORT Chain_nr - Event chain number (0...).
 ...
 * LAST      : 29.08.95 16:10
 * INPUTS    : UNSHORT First_block_nr - Number of first event block / 0xFFFF.
 * RESULT    : UNSHORT : Event chain number (0...Nr chains) /
 *              0xFFFF = not found).
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
Get_event_chain_nr(UNSHORT First_block_nr)
{
	...
	/* Is the first block number 0xFFFF ? */
	if (First_block_nr != 0xFFFF)
	{
		/* No -> Get pointer to event chain data */
		Ptr = (UNSHORT *)(MEM_Claim_pointer(Map_handle) + Event_chain_offset);
...
{
	static const UNCHAR Trigger_mode_names[MAX_TRIGGER_MODES][20] = {
		"Normal",
		"Examine",
		"Touch",
		"Speak",
	...
	static const UNCHAR Event_type_names[MAX_EVENT_TYPES][20] = {
		"Map exit",
		"Door",
		"Chest",
		"Text",

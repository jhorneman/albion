...
BOOLEAN Cheat_keys_activated = FALSE;
...
Check_low_level_diagnostic_keys(struct BLEV_Event_struct *Event)
{
	...
	/* Exit if cheat keys aren't activated */
...
Init_dprint(SISHORT X, SISHORT Y, UNSHORT Width, UNSHORT Height)
{
	...
	Diag_OPM_handle = MEM_Do_allocate
	(
		Width * Height,
		(UNLONG) &Diag_OPM,
		&ROPM_ftype
...
dprintf(UNCHAR *Format, ...)
{
   ...
   _vbprintf(&String[0], 201, Format, arglist);
   va_end(arglist);
...
MEM_List(void)
{
	...
	for (i=0;i<Workspace->Nr_of_areas;i++)
	{
		...
		LFB_entry = MEM_Find_LFB(Area);

		LFB = LFB_entry->Size;

		dprintf("In use : %lu, free : %lu, potential : %lu, LFB : %lu.",
		 Area->Size - PFM, TFM, PFM, LFB);
...
MEM_Report(FILE *Output_file)
{
	...
	for (i=0;i<Workspace->Nr_of_areas;i++)
	{
		...
		(
			Output_file,
			"In use : %lu, free : %lu, potential : %lu, LFB : %lu.\n",
			Area->Size - PFM,
			TFM,
			PFM,
			LFB
		...
		while (Entry)
		{
			if (Entry->BLOCK_HANDLE)
			{
				...
				if (Handle->Flags & MEM_ALLOCATED)
				{
					...
					(
						Output_file,
						"Allocated : %lu bytes. Handle %lu, C:%u, L:%u",
						Entry->Size,
						(UNLONG) (Handle - &MEM_Handles[0] + 1),
				...
				else
				{
					...
					(
						Output_file,
						"Persistent : %lu bytes. Handle %lu, C:%u, L:%u",
						Entry->Size,
						(UNLONG) (Handle - &MEM_Handles[0] + 1),
 						Handle->Claim_counter,
						Handle->Load_counter
...
{
	static const UNCHAR FORM_chunk[] = "FORM";
	static const UNCHAR PBM_chunk[] = "PBM ";
	static const UNCHAR BMHD_chunk[] = "BMHD";
	static const UNCHAR CMAP_chunk[] = "CMAP";
	static const UNCHAR BODY_chunk[] = "BODY";

	MEM_HANDLE Handle;
	struct OPM OPM;
	UNSHORT Chunk_sp = 0, i;
	UNBYTE *Chunk_stack[4];
	UNBYTE *Start;
	UNBYTE *Ptr;
	UNBYTE *Size_ptr;

...
/* Dialogue background parameters */
#define DIALOGUE_BACKGROUND_X			(Dialogue_PA.Area.left)
#define DIALOGUE_BACKGROUND_Y			(Dialogue_PA.Area.top)
#define DIALOGUE_BACKGROUND_WIDTH	(Dialogue_PA.Area.width)
#define DIALOGUE_BACKGROUND_HEIGHT	(Dialogue_PA.Area.height)
...
#define NR_NORMAL_MC_ENTRIES		(FIRST_NORMAL_MC_ENTRY + 20)
...
#define DIALOGUE_ERR_ILLEGAL_CHAR_TYPE 			(4)

#define DIALOGUE_ERR_MAX	(4)
...
void Draw_dialogue_screen(void);
...
/* Dialogue log support functions */
...
UNSHORT Dialogue_active_member;

static UNSHORT Dialogue_event_set_indices[2];

static MEM_HANDLE Word_bit_arrays_handle;

UNBYTE *Known_words_bit_array_ptr;
UNBYTE *Used_new_words_bit_array_ptr;
UNBYTE *New_words_bit_array_ptr;

static struct Dialogue_node Current_dialogue_node;
static struct Dialogue_node Default_dialogue_node;

static MEM_HANDLE Standard_MC_entry_text_handle;
static UNSHORT Standard_MC_entry_text_block_nr;
...
/* Dialogue log list file type */
static UNCHAR Dialogue_log_list_fname[] = "Dialogue log list";

static struct File_type Dialogue_log_list_ftype = {
	MEM_Relocate,
	0xFF,
	0,
	MEM_KILL_ALWAYS,
	Dialogue_log_list_fname
};
...
Dialogue_ModInit(void)
{
	...
	/* Get active member */
	Dialogue_active_member			= PARTY_DATA.Active_member;

	/* Destroy current and default dialogue nodes */
	BASEMEM_FillMemByte
	(
		(UNBYTE *) &Current_dialogue_node,
		sizeof(struct Dialogue_node),
		0
	);
	BASEMEM_FillMemByte
	(
		(UNBYTE *) &Default_dialogue_node,
		sizeof(struct Dialogue_node),
		0
	);

	/* Clear standard MC entry data */
	Standard_MC_entry_text_handle		= NULL;
	Standard_MC_entry_text_block_nr	= 0;

	/* Are we talking with a party character ? */
	if (Dialogue_char_type == PARTY_CHAR_TYPE)
	{
		/* Yes -> Determine party member index */
		Dialogue_party_member_index =	Find_character_in_party(Dialogue_char_index);

		/* Is in the party ? */
		if (Dialogue_party_member_index != 0xFFFF)
		{
			/* Yes */
	...
	Word_bit_arrays_handle = MEM_Allocate_memory(3 * ((MAX_WORDS + 7) / 8));

	Known_words_bit_array_ptr = MEM_Claim_pointer(Word_bit_arrays_handle);
	New_words_bit_array_ptr = Known_words_bit_array_ptr + (MAX_WORDS + 7) / 8;
	Used_new_words_bit_array_ptr = New_words_bit_array_ptr + (MAX_WORDS + 7) / 8;

	/* Clear word arrays */
	Clear_bit_array(KNOWN_WORDS_BIT_ARRAY);
	Clear_bit_array(NEW_WORDS_BIT_ARRAY);
	Clear_bit_array(USED_NEW_WORDS_BIT_ARRAY);
	...
	Dialogue_background_handle = MEM_Do_allocate
	(
		DIALOGUE_BACKGROUND_WIDTH * DIALOGUE_BACKGROUND_HEIGHT,
		0,
	...
	OPM_New
	(
		DIALOGUE_BACKGROUND_WIDTH,
		DIALOGUE_BACKGROUND_HEIGHT,
		1,
		&Dialogue_background_OPM,
		Ptr
	);
	...
	(
		&Main_OPM,
		&Dialogue_background_OPM,
		DIALOGUE_BACKGROUND_X,
		DIALOGUE_BACKGROUND_Y,
		DIALOGUE_BACKGROUND_WIDTH,
		DIALOGUE_BACKGROUND_HEIGHT,
		0,
		0
	...
	/* Start of dialogue */
	{
		...
		else
		{
			...
			{
				/* Yes */
				Process_and_display_text(&Main_OPM, System_text_ptrs[506], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
			}
			else
			{
				/* No */
				Process_and_display_text(&Main_OPM, System_text_ptrs[505], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
			}
		}
	}
}
...
{
	/* This is to make sure that when the dialogue module is not popped by
	 Dialogue_MainLoop, the main loop function will not pop the module again
	 because the flag is still set.
	 This situation could occur when an End Dialogue event precedes a Map Exit
	 event. */
	...
	Used_new_words_bit_array_ptr	= NULL;
...
Dialogue_DisInit(void)
{
	...
	/* Push new object tree */
	Push_root(&Main_OPM);

	/* Add OPM for updating */
	Add_update_OPM(&Status_area_OPM);

	/* Add dialogue object */
	Dialogue_object = Add_object
	(
		0,
		&Dialogue_Class,
		NULL,
...
Dialogue_MainLoop(void)
{
	...
	UNSHORT Event_set_index;
	UNCHAR *Text_ptr;

	/* New active member ? */
	if (Dialogue_active_member != PARTY_DATA.Active_member)
	{
		/* Yes -> Re-draw the dialogue screen */
		Draw_dialogue_screen();
	...
	if (Current_dialogue_node.Text_handle)
	{
		...
		Process_and_display_text(&Main_OPM, Text_ptr, WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
	...
	/* At this point, it is possible that the dialogue has been ended during
	 an event chain and that a different module has already been installed. */
	if (!In_Dialogue)
		return;
	...
	/* Has the dialogue character's first event set changed ? */
	if (Dialogue_event_set_indices[0] !=
	 Get_event_set_index(Dialogue_char_handle, 0))
	{
		/* Yes -> Get new event set index */
		Event_set_index = Get_event_set_index(Dialogue_char_handle, 0);

		/* Destroy old event set and texts */
		MEM_Free_memory(Dialogue_event_set_handles[0]);
		MEM_Free_memory(Dialogue_event_text_handles[0]);

		/* Is there a new first event set ? */
		if (Event_set_index)
		{
		...
		/* Set event set index */
		Dialogue_event_set_indices[0] = Event_set_index;
	}

	/* Has the dialogue character's first event set changed ? */
	if (Dialogue_event_set_indices[1] !=
	 Get_event_set_index(Dialogue_char_handle, 1))
	{
		/* Yes -> Get new event set index */
		Event_set_index = Get_event_set_index(Dialogue_char_handle, 1);

		/* Destroy old event set and texts */
		MEM_Free_memory(Dialogue_event_set_handles[1]);
		MEM_Free_memory(Dialogue_event_text_handles[1]);

		/* Is there a new second event set ? */
		if (Event_set_index)
		{
			...
			if (!Dialogue_event_text_handles[1])
			{
				Dialogue_error(DIALOGUE_ERR_FILE_LOAD);
		...
		/* Set event set index */
		Dialogue_event_set_indices[1] = Event_set_index;
	}
}
...
Init_dialogue_data(void)
{
	...
	else
	{
		...
		switch (Dialogue_char_type)
		{
			...
			default:
			{
				Dialogue_error(DIALOGUE_ERR_ILLEGAL_CHAR_TYPE);
	...
	/* Set event set indices */
	Dialogue_event_set_indices[0] = Get_event_set_index(Dialogue_char_handle, 0);
	Dialogue_event_set_indices[1] = Get_event_set_index(Dialogue_char_handle, 1);
...
Exit_dialogue_data(void)
{
	...
	{
		/* No -> Free data */
		MEM_Kill_memory(Dialogue_char_handle);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Draw_dialogue_screen
 * FUNCTION  : Draw dialogue screen.
 * FILE      : DIALOGUE.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 21.10.95 16:55
 * LAST      : 21.10.95 16:55
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - This function is also called by Dialogue_MainLoop when the
...
Draw_dialogue_screen(void)
{
	...
	/* Current screen is 3D map ? */
	if (Current_screen_type(1) == MAP_3D_SCREEN)
	{
		/* Yes -> Prepare screen */
		Render_3D_map();
	}
	...
	(
		&Main_OPM,
		DIALOGUE_WINDOW_X + 10,
		DIALOGUE_WINDOW_Y + DIALOGUE_WINDOW_HEIGHT - 5,
		DIALOGUE_WINDOW_WIDTH - 10,
		5,
		&(Recolour_tables[0][0])
	);
	Put_recoloured_box
	(
		&Main_OPM,
		DIALOGUE_WINDOW_X + DIALOGUE_WINDOW_WIDTH - 5,
		DIALOGUE_WINDOW_Y + 10,
		5,
		DIALOGUE_WINDOW_HEIGHT - 15,
		&(Recolour_tables[0][0])
	);
	...
	(
		...
		DIALOGUE_WINDOW_X + 7,
		DIALOGUE_WINDOW_Y + 7,
		DIALOGUE_WINDOW_WIDTH - 14,
		DIALOGUE_WINDOW_HEIGHT - 14,
		&(Recolour_tables[0][0])
	);
	Draw_window_border
	(
		...
		DIALOGUE_WINDOW_X,
		DIALOGUE_WINDOW_Y,
		DIALOGUE_WINDOW_WIDTH,
		DIALOGUE_WINDOW_HEIGHT
	);
	...
	Ptr = MEM_Claim_pointer(Small_portrait_handles[PARTY_DATA.Active_member - 1]);
	...
	(
		&Main_OPM,
		360 - 36 - 8 - 2 - Get_line_width(Name),
		6,
		Name
	);
}
...
Handle_normal_MC_entry(UNSHORT Text_block_nr, UNSHORT MC_index)
{
	...
	{
		/* Yes -> Make a new entry in the dialogue log */
		Add_dialogue_log_entry
		(
			SELECT_ANSWER_ACTION,
			Text_block_nr * 256 + MC_index
		);
	}
}
		...
		if (Merged_word_index == 0xFFFE)
		{
			...
			else
			{
				...
				if (Dialogue_partner_in_party)
				{
					...
					Process_and_display_text(&Main_OPM, System_text_ptrs[510], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
				}
				else
				{
					/* No -> Default reaction */
					Process_and_display_text(&Main_OPM, System_text_ptrs[509], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
				}
			}
		}
		else
		{
			...
			if (!Result)
			{
				...
				else
				{
					...
					{
						/* Yes -> Default reaction */
						Process_and_display_text(&Main_OPM, System_text_ptrs[498], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
					}
					else
					{
						/* No -> Default reaction */
						Process_and_display_text(&Main_OPM, System_text_ptrs[497], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
					}
				}
			}
		}
...
Handle_give_item_default_MC_entry(void)
{
	...
	/* Select an item */
	Item_slot_index = Select_party_member_item
	(
		PARTY_DATA.Active_member,
		System_text_ptrs[496],
		NULL
	);
	...
	if (Item_slot_index != 0xFFFF)
	{
		...
		else
		{
			...
			Process_and_display_text(&Main_OPM, System_text_ptrs[499], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
		}
	}
}
...
Do_give_item(struct Event_action *Action)
{
	...
	Remove_item
	(
		Give_item_data->Char_handle,
		Give_item_data->Item_slot_index,
		1
	);
...
Handle_join_party_default_MC_entry(void)
{
	...
	{
		/* Yes -> "No room!" */
		Process_and_display_text(&Main_OPM, System_text_ptrs[508], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
	}
	...
	{
		/* No -> Do default reaction */
		Process_and_display_text(&Main_OPM, System_text_ptrs[501], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
	}
}
...
Handle_leave_party_default_MC_entry(void)
{
	...
	{
		/* No -> Do default reaction */
		Process_and_display_text(&Main_OPM, System_text_ptrs[502], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
...
Handle_end_dialogue_default_MC_entry(void)
{
	...
	else
	{
		...
		{
			/* Yes */
			Process_and_display_text(&Main_OPM, System_text_ptrs[504], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
		}
		else
		{
			/* No */
			Process_and_display_text(&Main_OPM, System_text_ptrs[503], WAIT_TEXT_DISPLAY_FLAG | DELAY_TEXT_DISPLAY_FLAG);
		}
	}
}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Set_standard_MC_entry_text_block
 * FUNCTION  : Set standard MC entry text block.
 * FILE      : DIALOGUE.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 28.07.95 12:48
 * LAST      : 28.07.95 12:48
 * INPUTS    : MEM_HANDLE Text_handle - Text file handle.
...
void
Set_standard_MC_entry_text_block(MEM_HANDLE Text_handle,
 UNSHORT Text_block_nr)
{
	/* Set standard MC entry text block data */
	Standard_MC_entry_text_handle		= Text_handle;
	Standard_MC_entry_text_block_nr	= Text_block_nr;
}
...
{
	BOOLEAN Replace_entry_texts = FALSE;
	UNLONG FAQ_reply_offsets[NR_FAQ_MC_ENTRIES];
	UNLONG Entry_text_offsets[NR_NORMAL_MC_ENTRIES];
	UNSHORT i;
	UNCHAR *Text_ptr;

	/* Clear subblock offsets */
	for (i=0;i<NR_NORMAL_MC_ENTRIES;i++)
		Entry_text_offsets[i] = 0;
	...
	/* Any replacing text block given ? */
	if (Standard_MC_entry_text_handle)
	{
		/* Yes -> Get text file address */
		Text_ptr = (UNCHAR *) MEM_Claim_pointer(Standard_MC_entry_text_handle);

		/* Find text block */
		Text_ptr = Find_text_block(Text_ptr, Standard_MC_entry_text_block_nr);
		...
		(
			Text_ptr,
			Entry_text_offsets,
			NR_NORMAL_MC_ENTRIES
		);

		MEM_Free_pointer(Standard_MC_entry_text_handle);

		/* Set flag */
		Replace_entry_texts = TRUE;
	}

	/* Check all FAQ multiple-choice entries */
	for (i=0;i<NR_FAQ_MC_ENTRIES;i++)
	{
		/* Is this FAQ reply present ? */
		if (FAQ_reply_offsets[i])
		{
			/* Yes -> Replace entry texts ? */
			if (Replace_entry_texts)
			{
				/* Yes -> Is this entry present in the replacing text block ? */
				if (Entry_text_offsets[NR_DEFAULT_MC_ENTRIES + i])
				{
					/* Yes -> Write MC entry data */
					MC_list[Nr_MC_entries].Type				= FAQ_MC_ENTRY_TYPE;
					MC_list[Nr_MC_entries].Index				= i;
					MC_list[Nr_MC_entries].Text_handle		= Standard_MC_entry_text_handle;
					MC_list[Nr_MC_entries].Text_block_nr	= Standard_MC_entry_text_block_nr;
					MC_list[Nr_MC_entries].Text_offset		= Entry_text_offsets[NR_DEFAULT_MC_ENTRIES + i];
...
//			continue;
		...
		if (Replace_entry_texts)
		{
			...
			if (Entry_text_offsets[i])
			{
				...
				MC_list[Nr_MC_entries].Text_handle		= Standard_MC_entry_text_handle;
				MC_list[Nr_MC_entries].Text_block_nr	= Standard_MC_entry_text_block_nr;
				MC_list[Nr_MC_entries].Text_offset		= Entry_text_offsets[i];
...
Init_MC_window_object(struct Object *Object, union Method_parms *P)
{
	...
	for (i=0;i<OID->Nr_entries;i++)
	{
		...
		if (OID->Entry_list[i].Type != EMPTY_MC_ENTRY_TYPE)
		{
			...
			memcpy
			(
				(UNBYTE *) &(Entry_OID.Data),
				(UNBYTE *) &(OID->Entry_list[i]),
...
Init_dialogue_log(UNBYTE *Data)
{
	...
	(
		sizeof(struct Dialogue_log_list),
		0,
		&Dialogue_log_list_ftype
	);
	...
	if (Nr_dialogue_log_entries % LOG_ENTRIES_PER_LIST)
	{
		...
		(
			&(List->Entries[0]),
			Data,
			(Nr_dialogue_log_entries % LOG_ENTRIES_PER_LIST) *
...
Add_dialogue_log_entry(UNSHORT Action_type, UNSHORT Action_value)
{
	...
	{
		Index = i % LOG_ENTRIES_PER_LIST;

		/* Is this the same entry as the one we're adding ? */
		if ((List->Entries[Index].Char_type	== Dialogue_char_type) &&
		 (List->Entries[Index].Char_index	== Dialogue_char_index) &&
		 (List->Entries[Index].Action_type	== Action_type) &&
		 (List->Entries[Index].Action_value	== Action_value))
		{
			/* Yes -> Break */
			Exit = TRUE;
			break;
		...
		/* End of this list ? */
		if (Index == LOG_ENTRIES_PER_LIST - 1)
		{
	...
	if (!Exit)
	{
		...
		Index = Nr_dialogue_log_entries % LOG_ENTRIES_PER_LIST;

		List->Entries[Index].Char_type		= Dialogue_char_type;
		List->Entries[Index].Char_index		= Dialogue_char_index;
		List->Entries[Index].Action_type		= Action_type;
		List->Entries[Index].Action_value	= Action_value;
		...
		if (!(Nr_dialogue_log_entries % LOG_ENTRIES_PER_LIST))
		{
			...
			(
				sizeof(struct Dialogue_log_list),
				(UNLONG) (Nr_dialogue_log_entries / LOG_ENTRIES_PER_LIST),
				&Dialogue_log_list_ftype
			);
 ...
 UNSHORT Action_type, UNSHORT Action_value)
{
	...
	for (i=0;i<Nr_dialogue_log_entries;i++)
	{
		...
		if ((List->Entries[Index].Char_type	== Char_type) &&
		 (List->Entries[Index].Char_index	== Char_index) &&
		 (List->Entries[Index].Action_type	== Action_type) &&
		 (List->Entries[Index].Action_value	== Action_value))
		{
			/* Yes -> Found */

...
#define OPEN_CHEST_EMPTY_PICTURE		(11)
#define OPEN_CHEST_FULL_PICTURE		(12)
#define CLOSED_CHEST_PICTURE			(13)
#define JUNKPILE_PICTURE				(14)
...
UNLONG Restore_Closed_chest_object(struct Object *Object, union Method_parms *P);
...
UNLONG Restore_Open_chest_object(struct Object *Object, union Method_parms *P);

/* Chest item slot methods (replacing normal item slot methods) */
UNLONG Left_Chest_item_slot_object(struct Object *Object, union Method_parms *P);
UNLONG DLeft_Chest_item_slot_object(struct Object *Object, union Method_parms *P);
UNLONG Drop_Chest_item_slot_object(struct Object *Object, union Method_parms *P);
...
static BOOLEAN Chest_is_empty_flag;
...
static MEM_HANDLE Chest_data_handle;
...
static struct Method Closed_chest_methods[] = {
	...
	{ RESTORE_METHOD,	Restore_Closed_chest_object },
...
static struct Method Open_chest_methods[] = {
	...
	{ RESTORE_METHOD,	Restore_Open_chest_object },
	{ TOUCHED_METHOD,	Dehighlight },
	{ 0, NULL}
};
...
Closed_chest_ModInit(void)
{
	...
	/* Load small picture */
	Load_small_picture(CLOSED_CHEST_PICTURE);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Restore_Closed_chest_object
 * FUNCTION  : Restore method of Closed chest object.
 * FILE      : CHEST.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 26.09.95 20:32
 * LAST      : 26.09.95 20:32
 * INPUTS    : struct Object *Object - Pointer to object.
 *             union Method_parms *P - Method parameters.
 * RESULT    : UNLONG : 0.
...
UNLONG
Restore_Closed_chest_object(struct Object *Object, union Method_parms *P)
{
	struct BBRECT *Rect;
	struct BBRECT Old;
...
Open_chest_ModInit(void)
{
	...
	if (Chest_flags & CHEST_OR_JUNKPILE)
	{
		/* Junkpile -> Load small picture */
		Load_small_picture(JUNKPILE_PICTURE);
	}
	else
	{
		/* Chest -> Check if chest is empty */
		Chest_is_empty_flag = Inventory_empty
		(
			&(Chest->Items[0]),
			ITEMS_PER_CHEST
		);
		...
		if (Chest->Chest_gold || Chest->Chest_food)
			Chest_is_empty_flag = FALSE;

		/* Empty ? */
		if (Chest_is_empty_flag)
		{
			/* Yes -> Load small picture */
			Load_small_picture(OPEN_CHEST_EMPTY_PICTURE);
		}
		else
		{
			/* No -> Load small picture */
			Load_small_picture(OPEN_CHEST_FULL_PICTURE);
		}
	}
	...
	{
		/* Anything there ? */
		if (!Packet_empty(&(Chest->Items[i])))
		{
			/* Yes -> Too many items ? */
			if (Chest->Items[i].Quantity > ITEMS_PER_PACKET)
			{
				/* Yes -> Clip */
				Chest->Items[i].Quantity = ITEMS_PER_PACKET;

				/* Report error */
				Error(ERROR_TOO_MANY_ITEMS_IN_PACKET);
			}
		}
	}
...
Open_chest_MainLoop(void)
{
	...
	if (Chest_flags & CHEST_OR_JUNKPILE)
	{
		/* Junkpile -> Not in drag & drop mode / chest is empty ? */
		if (!Drag_drop_mode && Chest_is_empty_flag)
		{
			/* Yes -> Set chest status */
	...
	{
		/* Chest -> Is there a change ? */
		if (Chest_is_empty_flag != Old_flag)
		{
			/* Yes -> Empty ? */
			if (Chest_is_empty_flag)
			{
...
Init_Open_chest_object(struct Object *Object, union Method_parms *P)
{
	...
	Chest_item_list_object = Add_object
	(
		Object->Self,
		&Item_list_Class,
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Restore_Open_chest_object
 * FUNCTION  : Restore method of Open chest object.
 * FILE      : CHEST.C
 * AUTHOR    : Jurie Horneman
...
UNLONG
Restore_Open_chest_object(struct Object *Object, union Method_parms *P)
{
	struct BBRECT *Rect;
	struct BBRECT Old;
 ...
 * LAST      : 05.07.95 16:52
 * INPUTS    : UNSHORT Slot_index - Slot index (1...33) / 0 = auto-move.

...
/**** Read located sound effect table ****/
	...
	Length = SOUND_Get_located_effect_table_length();
	...
	SOUND_Load_located_effects(Ptr);
...
EXIT_LOAD_GAME_STATE:
	...
	if (File_handle != -1)
	{
		...
		if (Success)
		{
			...
			_bprintf(Number, 4, "%u", Saved_game_nr);
			Write_INI_variable
			(
				"ALBION",
				"SAVED_GAME_NR",
...
Save_game_state(UNSHORT Saved_game_nr, UNCHAR *Saved_game_name)
{
	...
	BOOLEAN Failure;
	BOOLEAN Success = FALSE;
	BOOLEAN Out_of_disk_space = FALSE;
	...
	/* Save current monster move delay */
	PARTY_DATA.Monster_move_delay = Monster_move_delay;
...
/**** Write located sound effect table ****/
	...
	Ptr = SOUND_Get_located_effect_table_address();
...
EXIT_SAVE_GAME_STATE:
	...
	if (File_handle != - 1)
	{
		...
		else
		{
			...
			remove(Save_game_fname);

			/* Out of disk space ? */
			if (Out_of_disk_space)
			{
				/* Yes -> Inform the player */
				Do_text_window(System_text_ptrs[775]);
			}
		}
	}

...
static struct NPC_sound_set NPC_sound_sets[MAX_NPC_SOUND_SETS] = {
	...
	/* 9 Wandernde Steins„ule */
	{
		{
			{   0, 100, 100, 100, 0 },	/* Permanent	*/
			{  23, 100,  50, 100, 0 },	/* Moving		*/
			{   0, 100, 100,   0, 0 },	/* Discover		*/
			{   0, 100, 100, 100, 0 }	/* Hunting		*/
		}
	},
	/* 10 Skrinn (nicht implementiert) */
	{
		{
			{   0, 100, 100, 100, 0 },	/* Permanent	*/
			{   0, 100, 100, 100, 0 },	/* Moving		*/
	...
	for (i=0;i<NPCS_PER_MAP;i++)
	{
		...
		if (NPC_present(i))
		{
			/* Yes -> Add permanent effect
			  (the NPC sound logic will make sure the effect won't be added
				twice. This is the best way of making sure the effect is there.) */
			...
			for (j=0;j<MAX_NPC_EFFECT_TYPES;j++)
			{
				...
				if (j != 0xFFFF)
				{
					...
					SOUND_Set_effect_position(Effect_index, X, Y);
				}
			}
		}
	}
	...
	if (VNPCs[NPC_index].Effect_indices[NPC_effect_type] == 0xFFFF)
	{
		...
		if (Sound_set_index < MAX_NPC_SOUND_SETS)
		{
			...
			if (Effect->Effect_nr)
			{
				...
				/* Install it */
				Effect_index = SOUND_Add_located_effect
				(
					Effect->Effect_nr,
					Effect->Priority,
					Effect->Volume,
				...
				{
					/* No -> Store index */
					VNPCs[NPC_index].Effect_indices[NPC_effect_type] = Effect_index;
				}
			}
		}
	}
...
Remove_NPC_sound_effect(UNSHORT NPC_index, UNSHORT NPC_effect_type)
{
	...
	{
		/* Yes -> Remove it */
		SOUND_Remove_located_effect(VNPCs[NPC_index].Effect_indices[NPC_effect_type]);

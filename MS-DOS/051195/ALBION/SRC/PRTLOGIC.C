...
#define LEVEL_MAKE_FACTOR		(125)

#define GAME_OVER_SONG_NR		(43)
...
Calculate_game_tables(void)
{
	...
	{
		Level_factors[i] = max(1, (LEVEL_MAKE_FACTOR * (i * i)) / 100 + i - 14);
	}
...
Game_over(void)
{
	...
	SOUND_Play_song(GAME_OVER_SONG_NR);
	...
	/* Set screen type to avoid re-drawing of 2D map */
	...
	/* Tell the player */
	Do_text_window(System_text_ptrs[417]);
...
Activate_party_member(UNSHORT Member_nr)
{
	...
	/* Are we in dialogue and NOT in combat ? */
	if (In_Dialogue && !In_Combat)
	{
		/* Yes -> With a party member ? */
...
Get_attribute(MEM_HANDLE Char_handle, UNSHORT Attribute_nr)
{
	...
	if (Attribute_nr < MAX_ATTRS)
	{
		...
		Value = Char->xAttributes[Attribute_nr].Normal;

		/* Add attribute bonuses from body items */
		for (i=0;i<ITEMS_ON_BODY;i++)
		{
			...
			if (!Packet_empty(&(Char->Body_items[i])))
			{
				...
				{
					/* Yes -> Of the given attribute ? */
					if (Attribute_nr == (UNSHORT) Item_data->Attribute)
					{
						/* Yes -> Get bonus value */
						Bonus = (SISHORT) Item_data->Attribute_normal;

						/* Cursed ? */
						if (Char->Body_items[i].Flags & CURSED)
						{
							/* Yes -> Subtract item bonus */
							Value -= Bonus;
						}
						else
						{
 ...
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_real_attribute
 * FUNCTION  : Get a character's attribute, without including the
 *              effects of any equipped items.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.10.95 22:11
 * LAST      : 22.10.95 22:11
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Attribute_nr - Attribute index (0...).
 * RESULT    : SISHORT : Attribute value.
...
SISHORT
Get_real_attribute(MEM_HANDLE Char_handle, UNSHORT Attribute_nr)
{
	struct Character_data *Char;
	SISHORT Value = 0;
...
Set_attribute(MEM_HANDLE Char_handle, UNSHORT Attribute_nr, SISHORT Value)
{
	...
	if (Attribute_nr < MAX_ATTRS)
	{
		...
		/* Set attribute value */
		Char->xAttributes[Attribute_nr].Normal = max(min(Value, 32767), 0);
 ...
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_max_attribute
 * FUNCTION  : Get a character's maximum attribute.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 22:31
 * LAST      : 16.10.95 22:31
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Attribute_nr - Attribute index (0...).
 * RESULT    : SISHORT : Attribute value.
...
SISHORT
Get_max_attribute(MEM_HANDLE Char_handle, UNSHORT Attribute_nr)
{
	struct Character_data *Char;
	SISHORT Value = 0;
	...
	if (Attribute_nr < MAX_ATTRS)
	{
		...
		Value = Char->xAttributes[Attribute_nr].Maximum;
 ...
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Set_max_attribute
 * FUNCTION  : Set a character's maximum attribute.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 22:32
 * LAST      : 16.10.95 22:32
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Attribute_nr - Attribute index (0...).
 *             SISHORT Value - New maximum value.
 * RESULT    : None.
 * BUGS      : No known.
 * #FUNCTION HEADER END#
 */
...
void
Set_max_attribute(MEM_HANDLE Char_handle, UNSHORT Attribute_nr, SISHORT Value)
{
	struct Character_data *Char;
	...
	if (Attribute_nr < MAX_ATTRS)
	{
		...
		/* Set maximum attribute value */
		Char->xAttributes[Attribute_nr].Maximum = max(min(Value, 32767), 0);
...
Get_skill(MEM_HANDLE Char_handle, UNSHORT Skill_nr)
{
	...
	if (Skill_nr < MAX_SKILLS)
	{
		...
		Value = Char->xSkills[Skill_nr].Normal;

		/* Add skill bonuses from body items */
		for (i=0;i<ITEMS_ON_BODY;i++)
		{
			/* Anything in body slot ? */
			if (!Packet_empty(&(Char->Body_items[i])))
			{
				...
				{
					/* Any skill tax ? */
					if (Item_data->Malus[j])
					{
						/* Yes -> Of the given skill ? */
						if (Skill_nr == (UNSHORT) Item_data->Item_skills[j])
						{
							/* Yes -> Tax */
							Value -= (SISHORT) Item_data->Malus[j];
						}
					}
				}
				...
				{
					/* Yes -> Of the given skill ? */
					if (Skill_nr == (UNSHORT) Item_data->Skill)
					{
						/* Yes -> Get bonus value */
						Bonus = (SISHORT) Item_data->Skill_normal;
		...
		/* Attack skill / is this character blind ? */
		if (((Skill_nr == CLOSE_RANGE_ATTACK) || (Skill_nr == LONG_RANGE_ATTACK)) &&
		 (Get_conditions(Char_handle) & (1 << BLIND)))
		{
			/* Yes -> Halve skill */
			Value /= 2;
		}
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_real_skill
 * FUNCTION  : Get a character's skill, without including the
 *              effects of any equipped items.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.10.95 22:10
 * LAST      : 22.10.95 22:10
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Skill_nr - Skill index (0...).
 * RESULT    : SISHORT : Skill value.
...
SISHORT
Get_real_skill(MEM_HANDLE Char_handle, UNSHORT Skill_nr)
{
	struct Character_data *Char;
	SISHORT Value = 0;
...
Set_skill(MEM_HANDLE Char_handle, UNSHORT Skill_nr, SISHORT Value)
{
	...
	if (Skill_nr < MAX_SKILLS)
	{
		...
		/* Set skill value */
		Char->xSkills[Skill_nr].Normal = max(min(Value, 32767), 0);
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_max_skill
 * FUNCTION  : Get a character's maximum skill.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 22:30
 * LAST      : 16.10.95 22:30
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 *             UNSHORT Skill_nr - Skill index (0...).
 * RESULT    : SISHORT : Maximum skill value.
 * BUGS      : No known.
 * #FUNCTION HEADER END#
 */
...
SISHORT
Get_max_skill(MEM_HANDLE Char_handle, UNSHORT Skill_nr)
{
	struct Character_data *Char;
	SISHORT Value = 0;
	...
	if (Skill_nr < MAX_SKILLS)
	{
		...
		/* Get maximum skill value */
		Value = Char->xSkills[Skill_nr].Maximum;
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Set_max_skill
 * FUNCTION  : Set a character's maximum skill.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 16.10.95 22:31
 * LAST      : 16.10.95 22:31
...
void
Set_max_skill(MEM_HANDLE Char_handle, UNSHORT Skill_nr, SISHORT Value)
{
	struct Character_data *Char;
	...
	if (Skill_nr < MAX_SKILLS)
	{
		...
		/* Set maximum skill value */
		Char->xSkills[Skill_nr].Maximum = max(min(Value, 32767), 0);
...
Set_condition(MEM_HANDLE Char_handle, UNSHORT Condition_nr)
{
	...
	if (Condition_nr  < MAX_CONDITIONS)
	{
		...
		/* Get conditions */
		Old_conditions = Char->xBody_conditions;

		/* Set condition */
		Char->xBody_conditions |= (1 << Condition_nr);

		/* Is this a party member ? */
		if (Char->Type == PARTY_CHAR_TYPE)
		{
			/* Yes -> Was this a change ? */
			if (!(Old_conditions & (1 << Condition_nr)))
			{
				/* Yes -> Tell the player */
				Subject_char_handle = Char_handle;

				Set_permanent_message_nr(762 + Condition_nr);
			}
		}
		MEM_Free_pointer(Char_handle);
	}
...
Get_max_LP(MEM_HANDLE Char_handle)
{
	...
	/* Get initial maximum life points */
	Value = Char->xLife_points_maximum;

	/* Add LP max bonuses from body items */
	for (i=0;i<ITEMS_ON_BODY;i++)
	{
		/* Anything in body slot ? */
		if (!Packet_empty(&(Char->Body_items[i])))
		{
			...
			{
				/* Yes -> Subtract item bonus */
				Value -= (SISHORT) Item_data->LP_max;
			}
			else
			{
				/* No -> Add item bonus */
 ...
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_real_max_LP
 * FUNCTION  : Get a character's maximum life points, without including the
 *              effects of any equipped items.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.10.95 22:00
 * LAST      : 22.10.95 22:00
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 * RESULT    : SISHORT : Maximum life points.
 * BUGS      : No known.
...
SISHORT
Get_real_max_LP(MEM_HANDLE Char_handle)
{
	struct Character_data *Char;
	SISHORT Value;
...
Get_max_SP(MEM_HANDLE Char_handle)
{
	...
	/* Get initial maximum spell points */
	Value = Char->xSpell_points_maximum;

	/* Add SP max bonuses from body items */
	for (i=0;i<ITEMS_ON_BODY;i++)
	{
		/* Anything in body slot ? */
		if (!Packet_empty(&(Char->Body_items[i])))
		{
			...
			{
				/* Yes -> Subtract item bonus */
				Value -= (SISHORT) Item_data->SP_max;
			}
			else
			{
				/* No -> Add item bonus */
 ...
 *****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Get_real_max_SP
 * FUNCTION  : Get a character's maximum spell points, without including the
 *              effects of any equipped items.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.10.95 22:01
 * LAST      : 22.10.95 22:01
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 * RESULT    : SISHORT : Maximum spell points.
 * BUGS      : No known.
...
SISHORT
Get_real_max_SP(MEM_HANDLE Char_handle)
{
	struct Character_data *Char;
	SISHORT Value;
...
Get_nr_occupied_hands(MEM_HANDLE Char_handle)
{
	...
	/* Iskai ? */
	if (Get_race(Char_handle) == ISKAI_RACE)
	{
		/* Yes -> Anything in the tail ? */
		if (!Packet_empty(&(Char->Body_items[TAIL - 1])))
		{
			/* Yes -> Add number of hands to counter */
			Item_data = Get_item_data(&(Char->Body_items[TAIL - 1]));
...
Spell_class_known(MEM_HANDLE Char_handle, UNSHORT Class_nr)
{
	...
	if (Spell_class_exists(Class_nr))
	{
		...
		/* Spell class known ? */
		Result = (BOOLEAN)(Char->xKnown_spell_classes & (1L << Class_nr));
...
Spell_known(MEM_HANDLE Char_handle, UNSHORT Class_nr, UNSHORT Spell_nr)
{
	...
	if (Spell_exists(Class_nr, Spell_nr))
	{
		...
		{
			/* No -> Spell known ? */
			if (Char->xKnown_spells[Class_nr] & (1L << Spell_nr))
			{
				/* Yes */
				Result = TRUE;
			}
...
Kill_party_member(UNSHORT Member_nr)
{
	...
	if (Member_present(Member_nr))
	{
		...
		/* Was this the active party member ? */
 ...
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - The revival check is a bit more secure now.
 * #FUNCTION HEADER END#
 */
...
Do_healing(UNSHORT Member_nr, SISHORT Healing)
{
	...
	if (Member_present(Member_nr))
	{
		...
		if (Healing > 0)
		{
			...
			/* "Alive" now ? */
			if ((Get_conditions(Party_char_handles[Member_nr - 1]) & (1 << UNCONSCIOUS)) &&
			 New_LP)
			{
...
De_exhaust_party_member(UNSHORT Member_nr)
{
	...
	if (Member_present(Member_nr))
	{
		...
		if (Get_conditions(Char_handle) & (1 <<EXHAUSTED))
		{
			...
			{
				if (Char->xAttributes[i].Normal < Char->xAttributes[i].Backup)
					Char->xAttributes[i].Normal = Char->xAttributes[i].Backup;
			}

			/* Restore skills */
			for (i=0;i<MAX_SKILLS;i++)
			{
				if (Char->xSkills[i].Normal < Char->xSkills[i].Backup)
					Char->xSkills[i].Normal = Char->xSkills[i].Backup;
			}
...
//	Play_jingle(46);
	...
	if (Char->Special[0])
	{
		...
		New_APR = (SISHORT) min(max(1, (Level / Char->Special[0])), MAX_APR);
	}
	else
	{
		/* Yes -> Default is one */
	...
	if (Character_has_magical_abilities(Char_handle))
	{
		/* Yes -> Calculate new SP */
		New_SP = (Char->Special[2] +
		 (Get_attribute(Char_handle, INTELLIGENCE) / 30)) * Level;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Healable_member_evaluator
 * FUNCTION  : Check if a party member can be healed (member select window
 *              evaluator).
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 19.10.95 23:09
 * LAST      : 19.10.95 23:09
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 * RESULT    : UNSHORT : Message nr / 0 = member is OK
 *              / 0xFFFF = member is absent.
...
UNSHORT
Healable_member_evaluator(MEM_HANDLE Char_handle)
{
	/* Is this party member alive OR not in combat ? */
	if (Character_alive(Char_handle) || !In_Combat)
	{
		/* Yes -> Member is OK */
		return 0;
	}
	...
	{
		/* No -> "Member cannot be healed in combat!" */
		return 759;
	}
}
...
Remove_party_member(UNSHORT Member_index)
{
	...
	/* Get character index */
	Char_index = PARTY_DATA.Member_nrs[Member_index - 1];
	...
	/* Were we talking to this party member ? */
	if (In_Dialogue &&
	 (Dialogue_char_type == PARTY_CHAR_TYPE) &&
 ...
 * LAST      : 20.10.95 15:39
 * INPUTS    : None.
 * RESULT    : BOOLEAN : Something happened (i.e. a warning was given or the
 *              party has been exhausted).
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
Do_camp_logic(void)
{
	...
	/* Exit if in cheat mode or in a script */
	if (Cheat_mode || In_Script)
		return FALSE;
			...
			if (Member_present(i + 1))
			{
				...
				if (Character_alive(Party_char_handles[i]))
				{
					...
					if (Get_conditions(Party_char_handles[i]) & (1 << EXHAUSTED))
					{
						...
						(
							i + 1,
							(Get_LP(Party_char_handles[i]) * 10) / 100
						);
					}
					else
					{
						...
						Char->xAttributes[0].Backup = Char->xAttributes[0].Normal;
						Char->xAttributes[0].Normal = (Char->xAttributes[0].Normal * 3) / 4;

						/* Decrease other attributes */
						for (j=1;j<MAX_ATTRS;j++)
						{
							Char->xAttributes[j].Backup = Char->xAttributes[j].Normal;
							Char->xAttributes[j].Normal /= 2;
						}

						/* Decrease skills */
						for (j=0;j<MAX_SKILLS;j++)
						{
							Char->xSkills[j].Backup = Char->xSkills[j].Normal;
							Char->xSkills[j].Normal /= 2;
						}
			...
			if (Character_alive(Party_char_handles[i]))
			{
				...
				if (Get_conditions(Party_char_handles[i]) & (1 << DISEASED))
				{
					...
					(
						Party_char_handles[i],
						Attribute,
						max(0, Get_real_attribute(Party_char_handles[i], Attribute) - 1)
					);

					/* Tell the player */
					Subject_char_handle = Party_char_handles[i];
					Do_text_window(System_text_ptrs[761]);
				}
			}
		}
	}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Character_has_magical_items
 * FUNCTION  : Search a character for magical items.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 13.10.95 22:09
 * LAST      : 13.10.95 22:09
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 * RESULT    : BOOLEAN : Character has magical items.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
Character_has_magical_items(MEM_HANDLE Char_handle)
{
	struct Character_data *Char;
	struct Item_data *Item_data;
	BOOLEAN Has_magical_items = FALSE;
	UNSHORT i;
	...
	for (i=0;i<ITEMS_ON_BODY;i++)
	{
		...
		if (!Packet_empty(&(Char->Body_items[i])))
		{
			...
			{
				/* Yes -> Yay! */
				Has_magical_items = TRUE;
			}

			Free_item_data();

			if (Has_magical_items)
				break;
		}
	}
 ...
 * #FUNCTION HEADER BEGIN#
 * NAME      : Member_present
 * FUNCTION  : Check if a party member is present.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 15.10.95 18:08
 * LAST      : 15.10.95 18:08
 * INPUTS    : UNSHORT Member_index - Party member index (1...6).
 * RESULT    : BOOLEAN : Member is present.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
BOOLEAN
Member_present(UNSHORT Member_index)
{
	BOOLEAN Result = FALSE;

	/* Legal member index ? */
	if ((Member_index > 0) && (Member_index <= 6))
	{
		/* Yes -> Is this party member present ? */
		if (PARTY_DATA.Member_nrs[Member_index - 1])
		{
			/* Yes */
			Result = TRUE;
		}
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Customkeys_party
 * FUNCTION  : "Custom keys method" for party short-cuts.
 * FILE      : PRTLOGIC.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 19.10.95 18:27
 * LAST      : 19.10.95 19:16
...
UNLONG
Customkeys_party(struct Object *Object, union Method_parms *P)
{
	BOOLEAN Result = TRUE;
	UNSHORT Key_code;
	...
	switch (Key_code)
	{
		...
		case '1':
		{
			/* Does this party member exist ? */
			if (Member_present(1))
			{
				/* Yes -> Is this already the active party member ? */
				if (PARTY_DATA.Active_member != 1)
				{
					/* No -> Can this character be active ? */
					if (Get_conditions(Party_char_handles[1 - 1]) & ACTIVE_MASK)
					{
						/* Yes -> Activate */
						Activate_party_member(1);
					}
				}
			}
			break;
		...
		case '2':
		{
			...
			{
				/* Yes -> Is this already the active party member ? */
				if (PARTY_DATA.Active_member != 2)
				{
					/* No -> Can this character be active ? */
					if (Get_conditions(Party_char_handles[2 - 1]) & ACTIVE_MASK)
					{
						/* Yes -> Activate */
						Activate_party_member(2);
					}
				}
			}
			break;
		...
		case '3':
		{
			...
			{
				/* Yes -> Is this already the active party member ? */
				if (PARTY_DATA.Active_member != 3)
				{
					/* No -> Can this character be active ? */
					if (Get_conditions(Party_char_handles[3 - 1]) & ACTIVE_MASK)
					{
						/* Yes -> Activate */
						Activate_party_member(3);
					}
				}
			}
			break;
		...
		case '4':
		{
			...
			{
				/* Yes -> Is this already the active party member ? */
				if (PARTY_DATA.Active_member != 4)
				{
					/* No -> Can this character be active ? */
					if (Get_conditions(Party_char_handles[4 - 1]) & ACTIVE_MASK)
					{
						/* Yes -> Activate */
						Activate_party_member(4);
					}
				}
			}
			break;
		...
		case '5':
		{
			...
			{
				/* Yes -> Is this already the active party member ? */
				if (PARTY_DATA.Active_member != 5)
				{
					/* No -> Can this character be active ? */
					if (Get_conditions(Party_char_handles[5 - 1]) & ACTIVE_MASK)
					{
						/* Yes -> Activate */
						Activate_party_member(5);
					}
				}
			}
			break;
		...
		case '6':
		{
			...
			{
				/* Yes -> Is this already the active party member ? */
				if (PARTY_DATA.Active_member != 6)
				{
					/* No -> Can this character be active ? */
					if (Get_conditions(Party_char_handles[6 - 1]) & ACTIVE_MASK)
					{
						/* Yes -> Activate */
						Activate_party_member(6);
					}
				}
			}
			break;
		...
		case BLEV_F1:
		{
			...
			if (Member_present(1))
			{
				/* Yes -> Are we already in this member's inventory screen ? */
				if (!In_Inventory || (Inventory_member != 1))
				{
					/* No -> Can this member's inventory be accessed ? */
					if (Get_conditions(Party_char_handles[1 - 1]) & INVENTORY_MASK)
					{
						/* No -> Cannot access inventory */
						Subject_char_handle = Party_char_handles[1 - 1];
						Set_permanent_message_nr(758);
					}
					else
					{
						/* Yes -> Enter inventory */
		...
		case BLEV_F2:
		{
			...
			{
				/* Yes -> Are we already in this member's inventory screen ? */
				if (!In_Inventory || (Inventory_member != 2))
				{
					/* No -> Can this member's inventory be accessed ? */
					if (Get_conditions(Party_char_handles[2 - 1]) & INVENTORY_MASK)
					{
						/* No -> Cannot access inventory */
						Subject_char_handle = Party_char_handles[2 - 1];
						Set_permanent_message_nr(758);
					}
					else
					{
		...
		case BLEV_F3:
		{
			...
			{
				/* Yes -> Are we already in this member's inventory screen ? */
				if (!In_Inventory || (Inventory_member != 3))
				{
					/* No -> Can this member's inventory be accessed ? */
					if (Get_conditions(Party_char_handles[3 - 1]) & INVENTORY_MASK)
					{
						/* No -> Cannot access inventory */
						Subject_char_handle = Party_char_handles[3 - 1];
						Set_permanent_message_nr(758);
					}
					else
					{
		...
		case BLEV_F4:
		{
			...
			{
				/* Yes -> Are we already in this member's inventory screen ? */
				if (!In_Inventory || (Inventory_member != 4))
				{
					/* No -> Can this member's inventory be accessed ? */
					if (Get_conditions(Party_char_handles[4 - 1]) & INVENTORY_MASK)
					{
						/* No -> Cannot access inventory */
						Subject_char_handle = Party_char_handles[4 - 1];
						Set_permanent_message_nr(758);
					}
					else
					{
		...
		case BLEV_F5:
		{
			...
			{
				/* Yes -> Are we already in this member's inventory screen ? */
				if (!In_Inventory || (Inventory_member != 5))
				{
					/* No -> Can this member's inventory be accessed ? */
					if (Get_conditions(Party_char_handles[5 - 1]) & INVENTORY_MASK)
					{
						/* No -> Cannot access inventory */
						Subject_char_handle = Party_char_handles[5 - 1];
						Set_permanent_message_nr(758);
					}
					else
					{
		...
		case BLEV_F6:
		{
			...
			{
				/* Yes -> Are we already in this member's inventory screen ? */
				if (!In_Inventory || (Inventory_member != 6))
				{
					/* No -> Can this member's inventory be accessed ? */
					if (Get_conditions(Party_char_handles[6 - 1]) & INVENTORY_MASK)
					{
						/* No -> Cannot access inventory */
						Subject_char_handle = Party_char_handles[6 - 1];
						Set_permanent_message_nr(758);
					}
					else
					{

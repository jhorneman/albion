...
void Print_broken_number(UNCHAR *String, SILONG Value);
...
struct Object_class Number_bar_Class = {
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Input_broken_number
 * FUNCTION  : Display and handle a number input window for a broken number.
 * FILE      : INPUTNR.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 05.10.95 21:35
 * LAST      : 05.10.95 21:35
 * INPUTS    : SILONG Value - Start value.
 *             SILONG Minimum - Minimum value.
 *             SILONG Maximum - Maximum value.
...
SILONG
Input_broken_number(SILONG Value, SILONG Minimum, SILONG Maximum, UNCHAR *Text)
{
	struct InputNr_OID OID;
	SILONG It;
	UNSHORT Obj;
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Input_broken_number_with_symbol
 * FUNCTION  : Display and handle a number input window with a symbol for
 *              a broken number.
 * FILE      : INPUTNR.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 05.10.95 21:37
 * LAST      : 05.10.95 21:37
 * INPUTS    : SILONG Value - Start value.
 *             SILONG Minimum - Minimum value.
 *             SILONG Maximum - Maximum value.
...
SILONG
Input_broken_number_with_symbol(SILONG Value, SILONG Minimum, SILONG Maximum,
 UNCHAR *Text, UNSHORT Width, UNSHORT Height, MEM_HANDLE Graphics_handle,
 UNLONG Graphics_offset)
{
	struct InputNr_OID OID;
...
Init_InputNr_object(struct Object *Object, union Method_parms *P)
{
	...
	if (OID->Broken_number_flag)
		Number_bar_OID.Print = Print_broken_number;
	else
...
void
Print_broken_number(UNCHAR *String, SILONG Value)
{
	_bprintf(String, 10, "%ld.%u", Value / 10, abs(Value % 10));
}
...
Init_Number_bar_object(struct Object *Object, union Method_parms *P)
{
	...
	static union Button_data Number_bar_up_button_data;
	...
	(
		Object->Self,
		&Button_Class,
		(UNBYTE *) &Number_bar_down_button_OID,
		0,
		0,
		10,
		NUMBER_BAR_HEIGHT
	...
	(
		Object->Self,
		&Button_Class,
		(UNBYTE *) &Number_bar_up_button_OID,
		Object->Rect.width - 10,
		0,
		10,
		NUMBER_BAR_HEIGHT
	);
...
Do_draw_number_bar(struct Number_bar_object *Number_bar)
{
	...
	if (Number_bar->Print)
	{
		(Number_bar->Print)(String, *(Number_bar->Value_ptr));
	}
	else
	{
		_bprintf(String, 10, "%ld", *(Number_bar->Value_ptr));
	}

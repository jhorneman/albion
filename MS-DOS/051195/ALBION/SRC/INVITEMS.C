			...
			if (Drag_packet.Quantity == 1)
			{
				...
				))
				{
					...
					))
					{
						...
						/* Set new drag source */
						New_drag_source_item_slot = Find_item_slot_object
						(
	...
	if (!Packet_empty(&Packet))
	{
		...
		if (Normal_clicked(Object))
		{
			...
			if (Quantity)
			{
				...
				(
					Inventory_char_handle,
					Item_slot->Number + ITEMS_ON_BODY,
					Quantity,
					&Drag_packet
				))
				{
...
Drop_Backpack_item_slot_object(struct Object *Object, union Method_parms *P)
{
	...
	{
		/* Yes -> Drop */
		Drop_on_backpack_item
		(
			(struct Item_slot_object *) Object,
			P->Drag_drop_data_ptr
		);
	}
...
Build_item_graphics(void)
{
	...
	(
		...
		0,
		ITEM_SLOT_INNER_WIDTH,
		Nr_frames * ITEM_SLOT_INNER_HEIGHT,
		0
	);
...
Init_drag(UNSHORT Width, UNSHORT Height, UNSHORT Max_frames)
{
	...
	Drag_OPM_handle = MEM_Do_allocate
	(
	...
	switch (Item_data->Type)
	{
		...
		case TEXT_SCROLL_IT:
		{
			...
			if (In_Combat)
			{
				...
				PUMES[5].Blocked_message_nr = 760;
			}
			else
			{
		...
		case SPECIAL_IT:
		{
			...
			if (In_Combat)
			{
				...
				PUMES[6].Blocked_message_nr = 760;
			}
			else
			{
				/* No -> Is the item broken & not cheating ? */
				...
				{
					/* No -> Does the party already have this special item ? */
		...
		default:
		{
			...
			if (Chain_nr != 0xFFFF)
			{
				...
				if (In_Combat)
				{
					...
					PUMES[8].Blocked_message_nr = 760;
				}
			}
		}
	}
...
PUM_Learn_spell_inventory_item(UNLONG Data)
{
	...
	for (i=0;i<6;i++)
	{
		...
		if (Member_present(i + 1))
		{
			...
			if (Character_alive(Party_char_handles[i]))
			{
				...
				if (Spell_class_known(Party_char_handles[i], Item_data->Spell_class))
				{
	...
	if (Counter && (Selected_party_member != 0xFFFF))
	{
		...
		/* Does the selected member already know this spell ? */
		if (Spell_known
		(
			Subject_char_handle,
		...
		{
			/* Yes -> Insult the player */
			Do_text_window(System_text_ptrs[188]);
		}
		else
		{
			/* No -> Is the spell's level too high ? */
			if (Get_spell_level(Item_data->Spell_class, Item_data->Spell_nr) >
			 Get_level(Subject_char_handle))
			{
				/* Yes -> Insult the player */
			...
			else
			{
				...
				/* Set data */
				Used_item_char_handle	= Subject_char_handle;
	...
	else
	{
		...
		Do_text_window(System_text_ptrs[187]);
	}
...
PUM_Drink_inventory_item(UNLONG Data)
{
	...
	Selected_party_member = Select_party_member
	(
		System_text_ptrs[719],
		Healable_member_evaluator,
		0
	);
	...
	if (Max_quantity)
	{
		...
		if (Quantity)
		{
			...
			if (Merchant_sell_requester(Quantity, Item_price))
			{
				...
				{
					/* Find target object */
					Target_object = Get_object_data(Merchant_item_list_object);

					/* Show moving items */
					Show_moving_item
					(
						&Sold_packet,
						Item_slot->Object.Rect.left + 1,
						Item_slot->Object.Rect.top + 1,
						Target_object->Rect.left + (Target_object->Rect.width - 16) / 2,
						Target_object->Rect.top + (Target_object->Rect.height - 16) / 2
					);

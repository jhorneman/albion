...
/* defines */
#define MAX_DROPPING_GOLD_FOOD	(5)
...
void Show_dropping_gold_food(struct Gold_food_object *Gold_food,
 UNSHORT Quantity);
void Build_gold_food_graphics(BOOLEAN Is_gold, UNSHORT Quantity);
...
Update_Gold_food_object(struct Object *Object, union Method_parms *P)
{
	...
	Delta = New_value - Gold_food->Value;
	if (Delta)
	{
		/* Yes -> Adjust value */
		if (abs(Delta) > 1000)
		{
			Gold_food->Value += 1000 * sgn(Delta);
		}
		else
		{
			if (abs(Delta) > 100)
			{
				Gold_food->Value += 100 * sgn(Delta);
			}
			else
			{
				if (abs(Delta) > 10)
				{
					Gold_food->Value += 10 * sgn(Delta);
				}
				else
				{
					Gold_food->Value += sgn(Delta);
				}
			}
		}
...
Help_Gold_food_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Gold_food->Is_gold)
	{
		...
		_bprintf(String, 100, System_text_ptrs[48], Value / 10, Value % 10);
	}
	else
	{
		/* Food -> Make help line string */
		_bprintf(String, 100, System_text_ptrs[49], Value);
	}
		...
		if (Normal_clicked(Object))
		{
			...
			if (Total_value > 1)
			{
				...
				if (Gold_food->Is_gold)
				{
					...
					Quantity = (UNSHORT) Input_broken_number_with_symbol
					(
						1,
						0,
						(SILONG) Total_value,
				...
				else
				{
					...
					(
						...
						0,
						(SILONG) Total_value,
						System_text_ptrs[515],
						20,
						10,
						NULL,
						(UNLONG) &(Food_symbol[0])
					);
				}
			}
...
Drag_gold_food(struct Gold_food_object *Gold_food, UNSHORT Quantity)
{
	...
	if (Quantity)
	{
		...
		/* Build graphics */
		Build_gold_food_graphics(Gold_food->Is_gold, Quantity);
 ...
 struct Drag_drop_data *Drag_drop_data_ptr)
{
	...
	if (Added_quantity)
	{
		...
		if (Drag_quantity)
		{
			...
			Build_gold_food_graphics(Gold_food->Is_gold, Drag_quantity);
...
Gold_food_drag_abort_handler(struct Drag_drop_data *Drag_drop_data_ptr)
{
	...
	switch (Drag_drop_data_ptr->Data_ID)
	{
		...
		case FOOD_DD_DATA_ID:
		{
			...
			{
				/* Yes -> Drop the gold or food back on the source slot */
				Drop_on_gold_food
				(
					(struct Gold_food_object *) Drag_drop_data_ptr->Source_object,
					Drag_drop_data_ptr
				);
			}
			else
...
Draw_gold_food(struct Gold_food_object *Gold_food)
{
	...
	else
	{
		...
		(
			Current_OPM,
			Object->X + (Object->Rect.width - 20) / 2,
			Object->Y + 1,
			20,
			10,
			&(Food_symbol[0])
		...
		/* Build value string */
		_bprintf(String, 20, "%u", Gold_food->Value);
	}
...
Gold_food_PUM_evaluator(struct PUM *PUM)
{
	...
	Gold_food = (struct Gold_food_object *) Get_object_data(PUM_source_object_handle);
...
PUM_Drop_gold_food(UNLONG Data)
{
	...
	if (Gold_food->Is_gold)
	{
		/* Gold -> Value larger than 1 ? */
		if (Total_value > 1)
		{
			/* Yes -> Ask the player how much gold should be dropped */
			Quantity = (UNSHORT) Input_broken_number_with_symbol
			(
				1,
				0,
				(SILONG) Total_value,
				System_text_ptrs[727],
				12,
				10,
				NULL,
				(UNLONG) &(Gold_symbol[0])
		...
		{
			/* No -> Are you sure ? */
			Result = Boolean_requester(System_text_ptrs[728]);
			if (!Result)
			{
				/* No -> Clear quantity */
				Quantity = 0;
	...
	else
	{
		...
		{
			/* No -> Are you sure ? */
			Result = Boolean_requester(System_text_ptrs[730]);
			if (!Result)
			{
				/* No -> Clear quantity */
				Quantity = 0;
	...
	if (Quantity)
	{
		/* No -> Remove gold or food */
		Total_value = Get_gold_food_value(Gold_food);
		Set_gold_food_value(Gold_food, Total_value - Quantity);

		/* Show dropping */
		Show_dropping_gold_food(Gold_food, Quantity);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Show_dropping_gold_food
 * FUNCTION  : Show dropping of gold / food.
 * FILE      : GOLDFOOD.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 05.10.95 22:00
 * LAST      : 06.10.95 13:47
 * INPUTS    : struct Gold_food_object *Gold_food - Pointer to gold/food
 *              object.
 *             UNSHORT Quantity - Quantity that is dropped.
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
void
Show_dropping_gold_food(struct Gold_food_object *Gold_food,
 UNSHORT Quantity)
{
	struct HDOB Drop_HDOBs[MAX_DROPPING_GOLD_FOOD];
	struct Object *Object;
	BOOLEAN HDOBs_are_ready;
	UNSHORT HDOB_nrs[MAX_DROPPING_GOLD_FOOD];
	UNSHORT Nr_HDOBs;
	UNSHORT i;
	...
	Nr_HDOBs = min(max(Quantity, 1), MAX_DROPPING_GOLD_FOOD);
	...
	for (i=0;i<Nr_HDOBs;i++)
	{
		...
		if (Gold_food->Is_gold)
		{
			/* Gold -> Set special HDOB data */
			Drop_HDOBs[i].Width				= 12;
			Drop_HDOBs[i].Height				= 10;
			Drop_HDOBs[i].Graphics_offset	= (UNLONG) &(Gold_symbol[0]);
		}
		else
		{
			/* Food -> Set special HDOB data */
			Drop_HDOBs[i].Width				= 20;
			Drop_HDOBs[i].Height				= 10;
			Drop_HDOBs[i].Graphics_offset	= (UNLONG) &(Food_symbol[0]);
		}

		/* Set common HDOB data */
		Drop_HDOBs[i].Draw_mode			= HDOB_MASK;
		Drop_HDOBs[i].Flags				= 0;
		Drop_HDOBs[i].X					= Object->Rect.left + (Object->Rect.width -
												 Drop_HDOBs[i].Width) / 2;
		Drop_HDOBs[i].Y					= Object->Rect.top + 1;
		Drop_HDOBs[i].Nr_frames			= 1;
		Drop_HDOBs[i].Draw_data			= 0;
		Drop_HDOBs[i].Graphics_handle	= NULL;
 ...
 * LAST      : 25.08.95 13:27
 * INPUTS    : BOOLEAN Is_gold - Is gold ?
...
void
Build_gold_food_graphics(BOOLEAN Is_gold, UNSHORT Quantity)
{
	UNCHAR String[20];
	...
	if (Is_gold)
	{
		...
		_bprintf(String, 20, "%u.%u", Quantity / 10, Quantity % 10);
	}
	else
	{
		/* Food -> Draw food symbol */

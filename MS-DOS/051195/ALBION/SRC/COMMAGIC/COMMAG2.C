	...
	if (Strength)
	{
		...
		if (Victim_part->Type == MONSTER_PART_TYPE)
		{
			...
			for(t=left;t<right;t+=rndmm(80,160)){
				...
				behave->Data.Animcontrol_data.Duration= (UNSHORT) rndmm(20,40);
...
//	struct COMOB_behaviour *Behaviour_data;
	...
	if (Fireball_COMOB && Firering_COMOB)
	{
		...
		Fireball_COMOB->Display_width = 75 - (15 * 4);
		Fireball_COMOB->Display_height = 75 - (15 * 4);
		...
		Firering_COMOB->Display_width = 75 + (15 * 20);
		Firering_COMOB->Display_height = 75 + (15 * 20);
		...
		for(;;){
			...
			if(Fireball_COMOB->Display_width>75){
				Fireball_COMOB->Display_width=75;
			}
			Fireball_COMOB->Display_height += 2*Nr_combat_updates;
			if(Fireball_COMOB->Display_height>75){
				Fireball_COMOB->Display_height=75;
			}

			/* Make firering smaller */
			Firering_COMOB->Display_width -= 10*Nr_combat_updates;
			if(Firering_COMOB->Display_width<75||Firering_COMOB->Display_width>32760){
				Firering_COMOB->Display_width=75;
			}
			Firering_COMOB->Display_height -= 10*Nr_combat_updates;
			if(Firering_COMOB->Display_height<75||Firering_COMOB->Display_height>32760){
				Firering_COMOB->Display_height=75;
			}
...
//	UNBYTE *Recolour_table_ptr[C2_Spell_8_ANZHALOCOMOBS];
	...
	struct COMOB *scare_COMOB = NULL;
	struct COMOB **halo_COMOB = NULL;
	struct COMOB_behaviour *behave;
	SILONG x,y,z;
	SILONG left,right,top,bottom;
	SILONG mleft,mright,mtop,mbottom;
	SILONG tleft,tright,ttop,tbottom;
	SILONG zoommode,time,max_width,max_height;
...
// 				}
		...
		else
		{
			...
			Courage = (SISHORT) Char->Courage;
			Courage -= 10 + (25 * (SISHORT) Strength) / 100;
			Char->Courage = max(Courage, 0);

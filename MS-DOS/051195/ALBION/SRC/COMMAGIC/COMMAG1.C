...
C1_Spell_4_handler(UNSHORT Strength)
{
	...
	for (i=0;i<C1_Spell_4_MAX_MAGIC;i++)
	{
		magic_COMOB[i] = NULL;
	}
...
//	struct COMOB *dummy_COMOB = NULL;
	...
	for (i=0;i<6;i++)
	{
		...
		{
			/* Yes -> Set fleeing condition */
			Set_condition(Party_parts[i].Char_handle, FLEEING);

			/* Remove participant */
			Remove_participant(&Party_parts[i]);
		}
	}
}
...
//	struct COMOB_behaviour *behave_y;
	...
	struct COMOB *riss_COMOB = NULL;
//	struct COMOB *trans_COMOB = NULL;
	...
	SILONG time;
//	SILONG hw[3];
	SILONG i,s,t,u,x,y,z,rx,ry,rz;
	SILONG steps,maxsteps,nextball;

	/* Clear counter */
	Current_nr_target_parts = 0;

	/* Select targets of mega killer spell */
...
//				trans_COMOB->Z_3D -= 5;
				...
				/* Halos verschiwnden */
				for(t=0;t<3;t++){
					halo_COMOB[t]->Lifespan= (UNSHORT) rndmm(1,3);
				}
...
//			COMOB->Frame=rndmm(0,COMOB->Nr_frames-1);
			...
			behave_x->Data.Oscillate_data.Value = (UNSHORT) rndmm(0,C1_Spell_10_CIRCLEDIVIDE)*C1_Spell_10_CIRCLERAD/C1_Spell_10_CIRCLEDIVIDE;

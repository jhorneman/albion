...
void Set_script_background_type(SCRIPT_BG_TYPE_T New_type);
...
BOOLEAN Party_awaken_script_command_handler(UNCHAR *Script_ptr);
BOOLEAN Party_activate_script_command_handler(UNCHAR *Script_ptr);
BOOLEAN Sound_effects_off_script_command_handler(UNCHAR *Script_ptr);
...
static Script_command_handler Script_command_handlers[MAX_SCRIPT_COMMANDS] = {
	...
	Sound_effects_off_script_command_handler,
	...
	Party_awaken_script_command_handler,
	Party_activate_script_command_handler,
...
static UNCHAR *Script_commands[MAX_SCRIPT_COMMANDS] = {
	...
	"do_event_chain",
	"show_pic",
	/* Must be here to override sound command */
	"sound_fx_off",
	"song",
	"ambient",
	"set_quest_bit",
...
/* Script screen method list */
static struct Method Script_methods[] = {
	{ UPDATE_METHOD, Update_Status_area },
	{ 0, NULL}
};

/* Script screen class description */
static struct Object_class Script_Class = {
...
Script_ModInit(void)
{
	...
	/* Analyse script */
	Nr_lines_in_script = Analyse_script
	(
		SCRIPT_Script_handle,
	...
	{
		/* Yes -> Switch to map background */
		Set_script_background_type(MAP_2D_SCRIPT_BG);
	}
	...
	/* Is the current background the map ? */
...
Analyse_script(MEM_HANDLE Script_handle, MEM_HANDLE Script_line_data_handle)
{
	...
	while (Script_ptr < Script_end_ptr)
	{
		...
		Exit_flag = FALSE;
		while (!Exit_flag && (Script_ptr < Script_end_ptr))
		{
			...
			switch (Char)
			{
				/* Space or tab (must be leading) */
				case ' ':
				case '\t':
				{
					/* Increase offset to current line */
					Script_line_data_ptr[Nr_lines].Offset++;
				...
				case '\r':
				{
					...
					*(Script_ptr - 1) = '\0';
					...
					/* Is the current character a LF ? */
					if (*Script_ptr == '\n')
					{
						/* Yes -> Replace it with a 0 */
						*Script_ptr++ = '\0';
					}
				...
				case ';':
				{
					...
					Script_line_data_ptr[Nr_lines].Line_type = COMMENT_SCRIPT_LINE;

					/* Count up */
					Nr_lines++;

					/* Skip to end of line */
					while ((Char != '\r') && (Script_ptr < Script_end_ptr))
					{
						Char = *Script_ptr++;
					}
				...
				default:
				{
					...
					Script_line_data_ptr[Nr_lines].Line_type = COMMAND_SCRIPT_LINE;
	...
	if (Nr_lines)
	{
		/* Yes -> Shrink script line data buffer */
		MEM_Resize_memory
		(
			Script_line_data_handle,
			Nr_lines * sizeof(struct Script_line_data)
		);
	}
 ...
 ****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Set_script_background_type
 * FUNCTION  : Set the script background type.
 * FILE      : SCRIPT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 22.10.95 19:57
 * LAST      : 23.10.95 22:41
 * INPUTS    : SCRIPT_BG_TYPE_T New_type - New background type.
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
void
Set_script_background_type(SCRIPT_BG_TYPE_T New_type)
{
	UNSHORT i;

	/* Act depending on new background type */
	switch (New_type)
	{
		/* 2D map */
		case MAP_2D_SCRIPT_BG:
		{
			/* Is the old background type 2D map ? */
			if (Script_background_type != MAP_2D_SCRIPT_BG)
			{
				/* No -> Add OPMs to update list */
				for (i=0;i<4;i++)
				{
			...
			/* Set screen type to 2D map */
			Set_screen_type(MAP_2D_SCREEN);
			break;
		}
		/* Others */
		case NO_SCRIPT_BG:
		case PICTURE_SCRIPT_BG:
		case FLIC_SCRIPT_BG:
		{
			/* Is the old background type 2D map ? */
			if (Script_background_type == MAP_2D_SCRIPT_BG)
			...
			/* Set screen type to no screen */
			Set_screen_type(NO_SCREEN);
			break;
		}
	}

	/* Set new script background type */
	Script_background_type = New_type;
}
...
Show_pic_script_command_handler(UNCHAR *Script_ptr)
{
	...
	/* Get picture size */
	Get_LBM_dimensions(Handle, &Pic_width, &Pic_height);

	/* Hide status area */
	Hide_status_area = TRUE;
	...
	Set_script_background_type(PICTURE_SCRIPT_BG);
...
Play_anim_script_command_handler(UNCHAR *Script_ptr)
{
	...
	Set_script_background_type(FLIC_SCRIPT_BG);
...
Fill_screen_script_command_handler(UNCHAR *Script_ptr)
{
	...
	/* Is the current background a picture or a flic ? */
	if ((Script_background_type == PICTURE_SCRIPT_BG) ||
	...
	Set_script_background_type(NO_SCRIPT_BG);
 ...
 ****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Party_awaken_script_command_handler
 * FUNCTION  : Party awaken script command handler.
 * FILE      : SCRIPT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 08.10.95 15:16
 * LAST      : 08.10.95 15:16
 * INPUTS    : UNCHAR *Script_ptr - Pointer to current line in script, after
 *              command.
 * RESULT    : BOOLEAN : End script flag.
...
BOOLEAN
Party_awaken_script_command_handler(UNCHAR *Script_ptr)
{
	SILONG Char_nr;
	UNSHORT Member_index;
	...
	if (Member_index != 0xFFFF)
	{
		/* Yes -> Does this character have any LP ? */
		if (!Get_LP(Party_char_handles[Member_index - 1]))
		{
			/* No -> Give one LP */
			Do_healing(Member_index, 1);
		}
	}
 ...
 ****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Party_activate_script_command_handler
 * FUNCTION  : Party activate script command handler.
 * FILE      : SCRIPT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 08.10.95 15:16
 * LAST      : 08.10.95 15:16
...
BOOLEAN
Party_activate_script_command_handler(UNCHAR *Script_ptr)
{
	SILONG Char_nr;
	UNSHORT Member_index;
 ...
 ****************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Sound_effects_off_script_command_handler
 * FUNCTION  : Sound effects off script command handler.
 * FILE      : SCRIPT.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 27.10.95 20:53
 * LAST      : 27.10.95 20:53
 * INPUTS    : UNCHAR *Script_ptr - Pointer to current line in script, after
 *              command.
 * RESULT    : BOOLEAN : End script flag.
...
BOOLEAN
Sound_effects_off_script_command_handler(UNCHAR *Script_ptr)
{
...
Script_error(UNSHORT Error_code)
{
	...
	(
		Script_print_error,
		Script_library_name,
		sizeof(struct Script_error_data),
		(UNBYTE *) &Error_data
	);
}
...
Script_print_error(UNCHAR *buffer, UNBYTE *data)
{
	...
	(
		...
		Script_error_strings[Error_code],
		Error_data->Line_nr,
		Error_data->Script_nr
	);
}
...
Play_animation_frames(UNSHORT Nr_frames)
{
	...
	for (i=0;i<Nr_frames;i++)
	{
		...
		{
			/* Yes -> Current palette is unknown to system */
		...
		/* Is this the Game over flic ? */
		if (ANIM_Flic_nr == GAME_OVER_FLIC_NR)
		{
			/* Yes -> Exit if the user acted */
			if (Check_if_user_acted())
			{
				break;
			}
		}

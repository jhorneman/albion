...
//	struct BBPALETTE Palette;
	...
	add_x_3d=(right-left)/(Source_width/pixelxsize);
	add_y_3d=(bottom-top)/(Source_height/pixelysize);
	...
	p=Ptr;
	for(y_3d=0,y=0;y<Source_height;y+=pixelysize,y_3d+=add_y_3d){
		for(x_3d=0,x=0;x<Source_width;x+=pixelxsize,x_3d+=add_x_3d){
			...
			col = Find_closest_colour(red/anz,green/anz,blue/anz);

			/* maximale Anzahl COMOBS erreicht dann beenden */
			...
			/* COMOB in Liste eintragen */
			COMOBLIST[aktcomob]=COMOB;
			...
			/* Set display parameters */
			COMOB->Draw_mode = SILHOUETTE_COMOB_DRAWMODE;
			COMOB->Colour = col;
			...
			COMOB->Graphics_handle = Spark_gfx_handles[2];
...
void
FairyII_handler(struct COMOB_behaviour *Behaviour)
{
	struct COMOB_behaviour *Behaviour_data;
	...
	for (i=0;i<2;i++)
	{
		...
		/* Add dust behaviour */
		Behaviour_data = Add_COMOB_behaviour(Dust_COMOB, NULL, DustII_handler);
		if (!Behaviour_data)
		{
		...
		/* Set lifespan */
		Dust_COMOB->Lifespan = rndmm(40,60);
...
void
DustII_handler(struct COMOB_behaviour *Behaviour)
{
	struct COMOB_behaviour *Behaviour_data;
	struct COMOB *Dust_COMOB;
...
/*					New_COMOB->dY_3D = max(Dust_COMOB->dY_3D + COMOB_DEC_FACTOR,
					...
					/* Set lifespan */
					New_COMOB->Lifespan = rndmm(20,30);

...
Init_InvRight_object(struct Object *Object, union Method_parms *P)
{
	...
	if (In_Merchant)
	{
		...
		Add_object(Object->Self, &Party_gold_display_Class, NULL,
		 145, 141, 70, 20);
	}
	else
	{
...
Init_Damage_display_object(struct Object *Object, union Method_parms *P)
{
	...
	Damage_display->Value = (UNLONG) Get_damage(Inventory_char_handle);
...
Update_Damage_display_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Get damage */
	Damage = (UNLONG) Get_damage(Inventory_char_handle);
...
Init_Protection_display_object(struct Object *Object, union Method_parms *P)
{
	...
	Protection_display->Value = (UNLONG) Get_protection(Inventory_char_handle);
...
Update_Protection_display_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Get protection */
	Protection = (UNLONG) Get_protection(Inventory_char_handle);
...
Help_Protection_display_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Make help line string */
	sprintf(String, System_text_ptrs[678], Get_protection(Inventory_char_handle));
...
Update_Weight_display_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Delta)
	{
		...
		if (abs(Delta) > 100)
		{
			Weight_display->Value += 100 * sgn(Delta);
		}
		else
		{
			if (abs(Delta) > 10)
			{
				Weight_display->Value += 10 * sgn(Delta);
			}
			else
			{
				Weight_display->Value += sgn(Delta);
			}
		}
...
Help_Weight_display_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Make help line string */
	Weight = Get_carried_weight(Inventory_char_handle);
	sprintf(String, System_text_ptrs[680], Weight);
...
Draw_Party_gold_display_object(struct Object *Object, union Method_parms *P)
{
	...
	Print_centered_string(Current_OPM, Object->X + 1, Object->Y + 1,
	 Object->Rect.width - 2, System_text_ptrs[585]);

	/* Print party gold */
	sprintf(String, "%lu.%u", Party_gold_display->Value / 10,
	 (UNSHORT) (Party_gold_display->Value % 10));
...
Help_Party_gold_display_object(struct Object *Object, union Method_parms *P)
{
	...
	/* Make help line string */
	sprintf(String, System_text_ptrs[586], Party_gold / 10,

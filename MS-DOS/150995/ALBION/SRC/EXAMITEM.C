...
Draw_ExamItem1_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Packet->Flags & CURSED)
	{
		...
		(
			...
			43,
			"%d",
			0 - (UNSHORT) Item_data->Damage_pts
		);
		...
		(
			...
			53,
			"%d",
			0 - (UNSHORT) Item_data->Protection_pts
		);
	}
	else
	{
		...
		(
			...
			53,
			"%d",
			(UNSHORT) Item_data->Protection_pts
		);
	}
...
Draw_ExamItem2_object(struct Object *Object, union Method_parms *P)
{
	...
	for (i=0;i<2;i++)
	{
		...
		/* Any skill tax ? */
		Number = Item_data->Item_skills[i];
		if (Number)
		{
			...
			(
				...
				11 + (i * 10),
				"%s %d",
				System_text_ptrs[643 + Number - 1],
				0 - (UNSHORT) Item_data->Malus[i]
			);
		}
		else
		{
	...
	else
	{
		...
		if (Number)
		{
			...
			(
				...
				51,
				"%s +%d",
				System_text_ptrs[635 + Number - 1],
				(UNSHORT) Item_data->Attribute_normal
			);
		}
		else
		{
		...
		if (Number)
		{
			...
			(
				...
				"%s +%d",
				System_text_ptrs[643 + Number - 1],
				(UNSHORT) Item_data->Skill_normal
			);
		}
		else
		{

...
UNSHORT Learn_spell_member_evaluator(MEM_HANDLE Char_handle);
...
Heal_LP_place_handler(void)
{
	...
	if (Selected_party_member)
	{
		...
		if (Selected_party_member != 0xFFFF)
		{
			...
			if (Place_price)
			{
				Max_delta_LP = min((UNSHORT) Max_delta_LP, (Party_gold / Place_price));
			}
	...
	if (Selected_party_member)
	{
		...
		if (Selected_party_member != 0xFFFF)
		{
			...
			if (Place_value != 0xFF)
			{
				...
				Selected_condition = Do_select_window(Text_ptr,
				 Heal_condition_evaluator, Draw_condition, MAX_CONDITIONS, 120, 12,
			...
			else
			{
				...
				Selected_condition = Do_select_window(System_text_ptrs[608],
				 Heal_condition_evaluator, Draw_condition, MAX_CONDITIONS, 120, 12,
				 (UNBYTE *) &Place_victim_handle);
			}
...
Heal_condition_evaluator(UNSHORT Index, UNBYTE *Data)
{
	...
	/* Get handle of victim character data */
...
Buy_food_place_handler(void)
{
	...
	if (Max_delta_food)
	{
		...
		if (Place_selector_text_block_nr != 0xFF)
		{
			...
			Delta_food = (UNSHORT) Input_number(1, 0, (SILONG) Max_delta_food,
			 Text_ptr);
		...
		else
		{
			...
			Delta_food = (UNSHORT) Input_number(1, 0,
			 (SILONG) Max_delta_food, System_text_ptrs[588]);
		}
...
Learn_spell_place_handler(void)
{
	...
	if (Counter)
	{
		/* Yes -> More than one (unlikely) ? */
		if (Counter > 1)
		{
			/* Yes -> Which party member ? */
			Selected_party_member = Select_party_member(System_text_ptrs[627],
			 Learn_spell_member_evaluator, 0);
		}
		...
		if (Selected_party_member != 0xFFFF)
		{
			...
			/* Learn which spell ? */
			Selected_spell = Do_select_window(System_text_ptrs[631],
			 Learn_spell_evaluator, Draw_learn_spell,
			 Spells_per_class[Place_value], 120, 12,
			 (UNBYTE *) &Place_victim_handle);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Learn_spell_member_evaluator
 * FUNCTION  : Check if a party member can learn spells (member select window evaluator).
 * FILE      : GAME.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 03.09.95 16:32
 * LAST      : 03.09.95 16:47
 * INPUTS    : MEM_HANDLE Char_handle - Handle of character data.
 * RESULT    : UNSHORT : Message nr / 0 = member is OK
 *              / 0xFFFF = member is absent.
...
UNSHORT
Learn_spell_member_evaluator(MEM_HANDLE Char_handle)
{
	UNSHORT Message_nr;
	...
	{
		/* Yes -> Knows this spell class ? */
		if (Spell_class_known(Char_handle, Place_value))
		{
			/* Yes -> Member is OK */
			Message_nr = 0;
		}
 ...
 UNBYTE *Data, BOOLEAN Blocked)
{
	...
	/* Build string */
	sprintf(String,"%s (%u)", Get_spell_name(Place_value, Index + 1),
...
Repair_item_place_handler(void)
{
	...
	if (Place_selector_text_block_nr != 0xFF)
	{
		...
		/* Repair which item ? */
		Selected_item_index = Select_character_item(Place_victim_handle,
		 Text_ptr, Repair_item_evaluator);
	...
	else
	{
		...
		Selected_item_index = Select_character_item(Place_victim_handle,
		 System_text_ptrs[591], Repair_item_evaluator);
	}
	...
	if (Selected_item_index != 0xFFFF)
	{
		...
		if (Place_buy_requester(Total_price))
		{
			...
			if (Selected_item_index < ITEMS_ON_BODY)
			{
				Char->Body_items[Selected_item_index].Flags &= ~BROKEN_ITEM;
			}
			else
			{
				Char->Backpack_items[Selected_item_index - ITEMS_ON_BODY].Flags &= ~BROKEN_ITEM;
			}
...
Place_buy_requester(UNLONG Price)
{
	...
	if (Price)
	{
		...
		else
		{
			...
			sprintf(String, System_text_ptrs[557], Price / 10, Price % 10);

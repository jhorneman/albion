...
Dialogue_ModInit(void)
{
	...
	if (Dialogue_char_type == PARTY_CHAR_TYPE)
	{
		...
		Dialogue_partner_in_party =
		 (Find_character_in_party(Dialogue_char_index) != 0xFFFF);

		if (Dialogue_partner_in_party)
		{
			/* In party -> Determine party member index */
			Dialogue_party_member_index =
			 Find_character_in_party(Dialogue_char_index);
		}
	}
...
Add_FAQ_MC_entries(struct MC_entry *MC_list, UNSHORT Nr_MC_entries)
{
	...
	/* Build sub-block catalogue to see if FAQ reply sub-blocks are present */
	Build_subblock_catalogue(Text_ptr, FAQ_reply_offsets, NR_FAQ_MC_ENTRIES);
	...
	if (Standard_MC_entry_text_handle)
	{
		...
		/* Build sub-block catalogue */
		Build_subblock_catalogue(Text_ptr, Entry_text_offsets,

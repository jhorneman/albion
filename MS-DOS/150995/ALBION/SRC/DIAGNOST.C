	...
	if (Event->ul_pressed_keys & BLEV_ALT)
	{
		...
		switch (Event->sl_key_code)
		{
			...
			case BLEV_F5:
			{
				...
				Buffer[0] = 0;
				Input_string_in_window(3, "Load which saved game?", Buffer);
...
//				Open_subfile(FLIC, 11, &File_handle, &File_base_offset);
//				File_handle = DOS_Open("G:\\ALBION\\XLDLIBS\\FLIC011.FLC",
//				 BBDOSFILESTAT_READ);
				File_handle = DOS_Open("G:\\ALBION\\OUTSIDE\\INTRO\\INTRO.FLC",
				 BBDOSFILESTAT_READ);
				File_base_offset = 0;

				Buffer_handle = MEM_Allocate_memory(100000);

				Ptr = MEM_Claim_pointer(Buffer_handle);

				Data_ptr = Main_OPM.data;

				Main_OPM.data = (UNBYTE *) 0xA0000;

				/* Prepare playback */
				BASEMEM_FillMemByte((UNBYTE *) &FLC, sizeof(struct Flic_playback), 0);
				...
				FLC.Trans_colour_index	= 0xFFFF;

				FLC.Palette					= &Palette;
				FLC.First_colour  		= 0;
				FLC.Max_colours	  		= 256;

				FLC.Input_buffer_length = 100000;
				FLC.Input_file_handle	= File_handle;
				FLC.File_base_offset		= File_base_offset;

				FLC.Flags |= FLC_STREAM;

				/* Start playback */
				FASTFLC_Start_flic_playback(&FLC);

				/* Calculate delta time */
				dT = max(1, (FLC.Flic.speed * 6) / 100);

				/* Play each frame */
//				for (i=0;i<FLC.Nr_frames;i++)
				for (i=0;i<200;i++)
				{
					/* Get time */
					T = SYSTEM_GetTicks();
...
//					Switch_screens();
					...
					/* Wait */
					while (SYSTEM_GetTicks() < (UNLONG)(T + dT))
					{
					};
				}
...
//				Close_subfile(File_handle);
			...
			case BLEV_F9:
			{
				...
				Buffer[0] = 0;
				Input_string_in_window(3, "Save under which number?", Buffer);
				...
				if (Buffer[0])
				{
					...
					Buffer[0] = 0;
					Input_string_in_window(40, "Enter saved game name", Buffer);

					if (Buffer[0])
					{
						Save_game_state(Number, Buffer);
					}
				}

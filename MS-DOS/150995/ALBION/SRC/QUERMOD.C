...
BOOLEAN Is_modify_possible(UNLONG Amount, UNLONG Maximum, UNSHORT Modify_mode,
 UNLONG Number);

UNLONG Modify_amount(UNLONG Amount, UNLONG Maximum, UNSHORT Modify_mode,
 UNLONG Number);

UNLONG Modify_amount_from_maximum(UNLONG Amount, UNLONG Maximum,
 UNSHORT Modify_mode, UNLONG Number);
...
Change_Used_Item_event(void)
{
	...
	/* Act depending on context source type */
	switch (Context->Source)
	{
		...
		case MAP_EVENT_SOURCE:
		{
			...
			if (Context->Source_data.Map_source.Trigger == USE_ITEM_TRIGGER)
			{
				...
				Result = Modify_carried_items(Used_item_char_handle,
				 Context->Source_data.Map_source.Value, 1, DEC_MODIFY_MODE);
				...
				if (Result)
				{
					...
					Result = Modify_carried_items(Used_item_char_handle,
					 Event->Word_6, 1, INC_MODIFY_MODE);
		...
		{
			/* No -> Get current event action data */
			...
			if (Event_action->Action_type == USE_ITEM_ACTION)
			{
				/* Yes -> Remove one item of the type that was used */
				Result = Modify_carried_items(Used_item_char_handle,
				 Event_action->Action_value, 1, DEC_MODIFY_MODE);
...
Datachange_event(void)
{
	...
	switch (Event->Byte_3)
	{
		...
		{
			Do_datachange(Active_char_handle, PARTY_DATA.Active_member, Event);
			break;
		}
		/* Entire party */
		case ENTIRE_PARTY_DC:
		{
			...
			for (i=0;i<6;i++)
			{
				...
				if (Member_present(i + 1))
				{
					...
					Do_datachange(Party_char_handles[i], i + 1, Event);
				}
			}
		...
		case SINGLE_MEMBER_DC:
		{
			...
			{
				/* Yes -> Change party member's data */
				Do_datachange(Party_char_handles[Member_index - 1],
				 Member_index, Event);
			}
			else
			{
				...
				{
					Do_datachange(Char_handle, Member_index, Event);
				}
		...
		case NPC_DC:
		{
			...
			{
				Do_datachange(Char_handle, 0xFFFF, Event);
			}
		...
		{
			Do_datachange(Inventory_char_handle, Inventory_member, Event);
			break;
		}
	}
}
 ...
 struct Event_block *Event)
{
	...
	switch (Event->Byte_1)
	{
		...
		case ATTRIBUTE_DC_TYPE:
		{
			...
			/* Already too high ? */
			if (Attr->Normal < Attr->Maximum)
			{
				/* No -> Modify */
				Attr->Normal = (UNSHORT) Modify_amount_from_maximum
				(
					(UNLONG) Attr->Normal,
					(UNLONG) Attr->Maximum,
					Modify_mode,
		...
		case SKILL_DC_TYPE:
		{
			...
			/* Already too high ? */
			if (Skill->Normal < Skill->Maximum)
			{
				/* No -> Modify */
				Skill->Normal = (UNSHORT) Modify_amount_from_maximum
				(
					(UNLONG) Skill->Normal,
					(UNLONG) Skill->Maximum,
					Modify_mode,
					(UNLONG) Number
				);
			}
		...
		case LP_DC_TYPE:
		{
			...
			/* Calculate new LP */
			New_LP = (UNSHORT) Modify_amount_from_maximum
			(
				(UNLONG) Old_LP,
				(UNLONG) Get_max_LP(Char_handle),
				Modify_mode,
				(UNLONG) Number
			);
			...
			else
			{
				...
				{
					/* No -> Show healing */
					Show_party_member_healing(Member_nr, New_LP - Old_LP);
				}
			}
		...
		case SP_DC_TYPE:
		{
			...
			(
				Char_handle,
				(UNSHORT) Modify_amount_from_maximum
				(
					(UNLONG) Get_SP(Char_handle),
					(UNLONG) Get_max_SP(Char_handle),
					Modify_mode,
					(UNLONG) Number
				)
			);
		...
		case TP_DC_TYPE:
		{
			...
			/* Calculate new TP */
			TP = Modify_amount((UNLONG) Get_TP(Char_handle), 0xFFFF,
		...
		case ITEMS_DC_TYPE:
		{
			Result = Modify_carried_items(Char_handle, Index, Number,
			 Modify_mode);
		...
		case GOLD_DC_TYPE:
		{
			...
			if (Is_modify_possible((UNLONG) Char->Char_gold,
			 32767, Modify_mode, Number))
			{
				/* Yes -> Modify */
				Char->Char_gold = (UNSHORT) Modify_amount((UNLONG) Char->Char_gold,
				 32767, Modify_mode, Number);
			}
			else
			{
		...
		{
			/* Is this modification possible ? */
			if (Is_modify_possible((UNLONG) Char->Char_food,
			 32767, Modify_mode, Number))
			{
				/* Yes -> Modify */
				Char->Char_food = (UNSHORT) Modify_amount((UNLONG) Char->Char_food,
				 32767, Modify_mode, Number);
			}
			else
			{
	...
	switch (Event->Byte_1)
	{
		...
		case MEMBER_CONSCIOUS_QM_TYPE:
		{
			...
			if (Member_index != 0xFFFF)
			{
				...
				if (!(Get_conditions(Party_char_handles[Member_index - 1]) &
				 DEAD_MASK))
				{
					Result = TRUE;
				}
...
Modify_event(void)
{
	...
	switch (Event->Byte_1)
	{
		...
		{
			/* Add a party member */
			Add_party_member(Number, 0xFFFF);
		...
		case GOLD_CARRIED_QM_TYPE:
		{
			...
			/* Is this modification possible ? */
			if (Is_modify_possible(Party_gold, 6 * 32767, Modify_mode,
			 (UNLONG) Number))
			{
				/* Yes -> Get party gold */
				...
				/* Modify this amount */
				Party_gold = Modify_amount(Party_gold, 6 * 32767, Modify_mode,
				 (UNLONG) Number);
		...
		case FOOD_CARRIED_QM_TYPE:
		{
			...
			/* Is this modification possible ? */
			if (Is_modify_possible(Party_food, 6 * 32767, Modify_mode,
			 (UNLONG) Number))
			{
				/* Get party food */
				...
				/* Modify this amount */
				Party_food = Modify_amount(Party_food, 6 * 32767, Modify_mode,
				 (UNLONG) Number);
		...
		case DAY_QM_TYPE:
		{
			...
			switch (Modify_mode)
			{
				...
				case INC_MODIFY_MODE:
				{
					...
					for (i=0;i<Number;i++)
					{
						for (j=0;j<Current_time_frame->Hours_per_day *
						 Current_time_frame->Steps_per_hour;j++)
						{
							Next_step();
						}
					}
...
BOOLEAN
Is_modify_possible(UNLONG Amount, UNLONG Maximum, UNSHORT Modify_mode,
 UNLONG Number)
{
	BOOLEAN Result = TRUE;
...
UNLONG
Modify_amount_from_maximum(UNLONG Amount, UNLONG Maximum, UNSHORT Modify_mode,
 UNLONG Number)
{
	/* Which modify mode ? */
	switch (Modify_mode)
 ...
 UNSHORT Number, UNSHORT Modify_mode)
{
	...
	/* Is this modification possible ? */
	if (!Is_modify_possible((UNLONG) Old_amount, 0xFFFFFFFF,
	 Modify_mode, (UNLONG) Number))
	{
		/* No -> Exit */
		return FALSE;
	...
	else
	{
		...
		while (Counter)
		{
			...
			/* Build packet */
			Build_item_packet(&Add_packet, Item_index, Attempted_amount);
			...
			{
				/* Yes -> Is this exactly the attempted amount ? */
				if ((UNSHORT) Add_packet.Quantity == Attempted_amount)
				{
					/* Yes -> Error */
					break;
				}
			...
			if (Member_present(i + 1))
			{
				...
				if (Subtract)
				{
					...
					for (j=0;j<ITEMS_PER_CHAR;j++)
					{
						...
						{
							/* Yes -> Remove it */
							Remove_item(Party_char_handles[i], ITEMS_ON_BODY +
							 j + 1, 1);

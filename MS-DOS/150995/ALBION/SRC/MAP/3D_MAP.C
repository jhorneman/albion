...
static UNSHORT Max_shade_factor;
...
M3_ModInit(void)
{
	...
	if (Loading_game)
	{
		...
		I3DM.Window_3D_width		= PARTY_DATA.Window_3D_width;
		I3DM.Window_3D_height	= PARTY_DATA.Window_3D_height;
	}
	else
	{
		/* No -> Set dimensions in interface */
...
Render_3D_map(void)
{
	...
	if (Cheat_mode)
	{
		I3DM.Shade_factor = MAX_SHADE_FACTOR - Max_shade_factor;
	}
	else
	{
		I3DM.Shade_factor = MAX_SHADE_FACTOR -
...
Init_shade_factor(void)
{
	...
	Max_shade_factor = MAX_SHADE_FACTOR - (UNSHORT) Lab_ptr->Shade_factor;
...
Get_target_shade_factor(void)
{
	...
	switch (Current_map_light_state)
	{
		...
		{
			/* Get light strength */
			Light_strength = max(Get_light_spell_strength(),
			 Calculate_party_light());

			/* Is the active member an Iskai ? */
			if (Get_race(Active_char_handle) == ISKAI_RACE)
			{
				/* Yes -> Add 25% */
				Light_strength = min(100, Light_strength + 25);
			}

			/* Calculate target shade factor */
			Target_shade_factor = (Max_shade_factor * Light_strength) / 100;
			break;
		}
		/* Changing light */
		case CHANGING_LIGHT:

...
#define MUSICERR_NO_MDI_INI	(3)
#define MUSICERR_NO_DIG_INI	(4)

#define MUSICERR_MAX				(4)
...
Play_song(UNSHORT Song_nr)
{
	 ...
	 (Sound_state & MIDI_ON))
	{
		...
		if (Song_nr)
		{
			...
			AIL_set_sequence_volume(Song_song.Sequence_handle,
...
Play_jingle(UNSHORT Song_nr)
{
	 ...
	 (Sound_state & MIDI_ON))
	{
		...
		if (Song_nr)
		{
			...
			/* Set the sequence volume */
			AIL_set_sequence_volume(Jingle_song.Sequence_handle,
			 (LONG) SONG_VOLUME, (LONG) 0);
		}
	}
}

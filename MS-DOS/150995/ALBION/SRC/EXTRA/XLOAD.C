...
//static struct Batch_load_data Batch_data_buffer[SUBFILES_MAX];
...
XLD_Close_subfile(SISHORT File_handle)
{
	...
	if (XLD_Library_open)
	{
		/* Yes -> Was this the currently opened library we've closed ? */
		if (XLD_Library_ID == File_handle)
		{
			/* Yes -> The library is no longer open */

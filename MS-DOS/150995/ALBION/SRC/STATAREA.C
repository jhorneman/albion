...
Update_Party_member_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Member_present(Party_member->Number))
	{
		...
		if (LP)
		{
			...
			LP_bar_size = max((LP * 20) / Get_max_LP(Char_handle), 1);
		}
		else
		{
			/* No -> No bar */
		...
		if (SP)
		{
			...
			SP_bar_size = max((SP * 20) / Get_max_SP(Char_handle), 1);
		}
		else
		{
			/* No -> No bar */
...
Help_Party_member_object(struct Object *Object, union Method_parms *P)
{
	...
	if (Member_present(Party_member->Number))
	{
			...
			switch (Current_drag_drop_data.Data_ID)
			{
				...
				case APRES_ITEM_DD_DATA_ID:
				{
					...
					if (Char_inventory_full(Char_handle))
					{
						...
						sprintf(String, System_text_ptrs[596], Char_name);
					}
					else
					{
						...
						sprintf(String, System_text_ptrs[593], Item_name, Char_name);
					}
					break;
				}
				/* Gold */
				case GOLD_DD_DATA_ID:
				{
					...
					if (Char->Char_gold < 32767)
					{
						...
						sprintf(String, System_text_ptrs[594], Char_name);
					}
					else
					{
						/* No -> "Cannot carry any more gold!" */
						sprintf(String, System_text_ptrs[597], Char_name);
					}
				...
				case FOOD_DD_DATA_ID:
				{
					...
					{
						/* Yes -> Make help line string */
						sprintf(String, System_text_ptrs[595], Char_name);
					}
					else
					{
						/* No -> "Cannot carry any more food!" */
						sprintf(String, System_text_ptrs[598], Char_name);
					}
		...
		else
		{
			...
			sprintf(String, System_text_ptrs[0], Char_name, Get_LP(Char_handle),
			 Get_SP(Char_handle));
		}
	...
	switch (Data_ID)
	{
		...
		case APRES_ITEM_DD_DATA_ID:
		{
			...
			if (!Char_inventory_full(Char_handle))
			{
				...
				Put_item_in_character(Char_handle, 0,
				 (UNSHORT) Drag_packet.Quantity, &Drag_packet);
...
Update_Permanent_text_object(struct Object *Object, union Method_parms *P)
{
	...
	if (i)
	{
		...
		/* Is it a new message or is it time to erase ? */
		if ((i > PERMANENT_TEXT_DURATION - BLINK_DURATION) || (i == 1))
		{
			/* Yes -> Print message */
			Execute_method(Object->Self, DRAW_METHOD, NULL);
		}

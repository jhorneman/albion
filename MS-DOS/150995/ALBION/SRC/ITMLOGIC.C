	...
	if (!Packet_empty(Packet))
	{
		...
		if (Slot_index <= ITEMS_ON_BODY)
		{
			...
			for (i=0;i<2;i++)
			{
				...
				{
					/* Yes -> Tax */
					Char->Skills[Skill_nr - 1].Normal -= (UNSHORT) Item_data->Malus[i];
				}
			}
...
Positive_item_boni(MEM_HANDLE Char_handle, struct Item_data *Item_data)
{
	...
	(
		Char_handle,
		Get_max_LP(Char_handle) + (UNSHORT) Item_data->LP_max
	);
	...
	(
		Char_handle,
		Get_max_SP(Char_handle) + (UNSHORT) Item_data->SP_max
	);
...
//	 * ITEM_NAME_LENGTH;
	Item_names += (((Packet->Index - 1) * LANGUAGES_MAX)) * ITEM_NAME_LENGTH;

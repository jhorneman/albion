...
Init_3D_data(void)
{
	...
	Number = Map_ptr->LAB_DATA_NR;
	MEM_Free_pointer(Map_handle);

	/* Load LAByrinth data */
	Lab_data_handle = Load_subfile(LAB_DATA, Number);
	...
	Number = Lab_ptr->Background_graphics_nr;
	if (Number)
		Background_gfx_handle = Load_subfile(BACKGROUND_GFX, Number);
	else
...
Put_unanimated_overlays_on_unanimated_walls(void)
{
	...
	MEM_Claim_pointer(Lab_data_handle);
}
 ...
 UNLONG Size)
{
	...
	BASEMEM_CopyMem(Source_ptr, Target_ptr, 64 * 256);
}
 ...
 * BUGS      : No known.
 * NOTES     : - The handle is claimed, so it must be freed afterwards.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
 */
...
{
	return (UNBYTE *) (((UNLONG) MEM_Claim_pointer(Shade_table_handle) +
	 256) & 0xffffff00);
}

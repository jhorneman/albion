...
static struct Rectangle_2D Rectangle_list[MAX_2D_OBJECTS];

UNSHORT Mapbuf_width, Mapbuf_height;
UNSHORT Map_2D_width, Map_2D_height;

UNSHORT Mapbuf_X, Mapbuf_Y;
UNLONG Camera_2D_X, Camera_2D_Y;

static UNSHORT Map_buffer[MAPBUF_HEIGHT_MAX+3][MAPBUF_WIDTH_MAX][MAPBUF_DEPTH][2];
static UNBYTE Update_buffer[MAPBUF_HEIGHT_MAX+3][MAPBUF_WIDTH_MAX];
...
{
	/* Clear map buffers */
	BASEMEM_FillMemLong((UNBYTE *) &(Map_buffer[0][0][0][0]),
	 (MAPBUF_HEIGHT_MAX + 3) * MAPBUF_WIDTH_MAX * MAPBUF_DEPTH * 4, 0L);

	/* Clear update buffer */
	BASEMEM_FillMemByte(&(Update_buffer[0][0]), (MAPBUF_HEIGHT_MAX + 3) *
	 MAPBUF_WIDTH_MAX, 0);
}
...
Set_2D_camera(void)
{
	...
	Calculate_2D_camera_position(&Target_X, &Target_Y);

	/* Calculate vector from current to target camera coordinates */
	dX = Target_X - Camera_2D_X;
	dY = Target_Y - Camera_2D_Y;

	/* Determine camera speed */
	X_speed = ((abs(dX) + 7) / 8) * sgn(dX);
	Y_speed = ((abs(dY) + 7) / 8) * sgn(dY);
 ...
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Initialize_2D_camera
 * FUNCTION  : Initialize the position of the camera in a 2D map.
 * FILE      : 2D_MAP.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 26.04.95 10:15
 * LAST      : 26.04.95 10:15
 * INPUTS    : None.
 * RESULT    : None.
 * BUGS      : No known.
...
Initialize_2D_camera(void)
{
	...
	/* Set camera */
	Camera_2D_X = Target_X;
	Camera_2D_Y = Target_Y;
}

/*
 ******************************************************************************
 * #FUNCTION HEADER BEGIN#
 * NAME      : Calculate_2D_camera_position
 * FUNCTION  : Calculate the desired position of the camera in a 2D map.
 * FILE      : 2D_MAP.C
 * AUTHOR    : Jurie Horneman
 * FIRST     : 26.04.95 10:16
 * LAST      : 26.04.95 10:16
 * INPUTS    : UNLONG *X_ptr - Pointer to output X-coordinate.
 *             UNLONG *Y_ptr - Pointer to output Y-coordinate.
 * RESULT    : None.
 * BUGS      : No known.
 * SEE ALSO  :
 * #FUNCTION HEADER END#
...
void
Calculate_2D_camera_position(UNLONG *X_ptr, UNLONG *Y_ptr)
{
	SILONG Target_X, Target_Y;

	/* Calculate target camera coordinates */
	Target_X = Party_object_X - (Map_2D_width / 2) + 16;
	Target_Y = Party_object_Y - (Map_2D_height / 2) - 8;

/*	if (Preview_direction != 0xFFFF)
	{
		Target_X += 32 * Offsets8[Preview_direction][0];
		Target_Y += 32 * Offsets8[Preview_direction][1];
	} */
	...
	/* Write coordinates */
	*X_ptr = (UNLONG) Target_X;
	*Y_ptr = (UNLONG) Target_Y;
}
	...
	if (Underlay_nr > 1)
	{
		...
		if (!(Data->Flags & ICON_INVISIBLE))
		{
			...
			if (Data->Nr_frames == 1)
			{
				...
				Icon = Icon_graphics[0] + 256 * Data->First_frame;
 ...
 * LAST      : 28.07.94 18:54
 * INPUTS    : UNSHORT Display_X - X-coordinate.
 *             UNSHORT Display_Y - Y-coordinate.
 * RESULT    : None.
 * BUGS      : No known.
 * NOTES     : - Several pointers (for instance to icon data and graphics)
 *              must have been initialized.
...
void
Draw_2D_map(UNSHORT Display_X, UNSHORT Display_Y)
{
	...
	{
		/* No -> Arrange for the removal of previous objects */
								...
								for (n=0;n<MAPBUF_DEPTH-1;n++)
								{
									 ...
									 (Y + l >= 0) && (Y + l < Mapbuf_height+3))
									{
										Display_extra_icons(Map_buffer[Y + l][X + m][n][0],
										 Map_buffer[Y + l][X + m][n][1], X + m, Y + l - n);
									}
								}
							}
						}
	...
	Draw_2D_scroll_buffer(Display_X, Display_Y);
}
...
void
Draw_2D_scroll_buffer(UNSHORT Display_X, UNSHORT Display_Y)
{
	/* OPM given ? */
	if (Current_2D_OPM)
	{
		/* Yes -> Display 2D map scroll buffer in OPM */
		Scroll_display_OPM(&Scroll_2D, Current_2D_OPM, Display_X, Display_Y);
	}
	else
	{
		/* No -> Display 2D map scroll buffer directly on the screen */
		Scroll_display(&Scroll_2D, Display_X, Display_Y);
	}
}
	...
	if (Underlay_nr)
	{
		...
		if (!(Data->Flags & ICON_INVISIBLE))
		{
			...
			if (Data->Flags & ADD_NEXT_ICON)
			{
				...
				Data = &((Icon_data[0])[Underlay_nr-1]);
...
/*			Put_recoloured_box(&(Scroll_2D.Base_OPM), nX, nY, 16, 8,
			 &(Recolour_tables[rand() % 8][0])); */
		}
...
/*			Put_recoloured_box(&(Scroll_2D.Base_OPM), nX, nY + 8, 16, 8,
			 &(Recolour_tables[rand() % 8][0])); */
		}
...
Set_update_rectangle(SISHORT X, SISHORT Y, UNSHORT Width, UNSHORT Height)
{
	...
	if (Nr_2D_rectangles == Nr_2D_objects)
		return;
 ...
 UNLONG Flags)
{
	...
	if (Flags & (1 << (BLOCKED_TRAVELMODES_B + PARTY_DATA.Travel_mode)))
		OPM_FillBox(OPM, X + 2, Y + 2, 12, 12, WHITE);

	if (Flags & (1 << BLOCKED_DIRECTIONS_B))
		OPM_HorLine(OPM, X, Y, 16, WHITE);
	if (Flags & (1 << (BLOCKED_DIRECTIONS_B + 1)))
		OPM_VerLine(OPM, X + 15, Y, 16, WHITE);
	if (Flags & (1 << (BLOCKED_DIRECTIONS_B + 2)))
		OPM_HorLine(OPM, X, Y + 15, 16, WHITE);
	if (Flags & (1 << (BLOCKED_DIRECTIONS_B + 3)))
...
{
	if (Get_event_triggers(Map_X, Map_Y, 0xFFFF))
	{

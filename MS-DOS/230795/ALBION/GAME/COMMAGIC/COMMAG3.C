...
//			light_COMOB[aktlightcomob]->Special_handle = Transparency_table_handle;
		...
		if(nextball<0){
			...
			for(t=0;t<balls;t++){
				...
				/* Zufallsbeginn */
				sx+=rndmm2(-5500,-4000,4000,5500);
				sy+=rndmm2(-5500,-4000,4000,5500);
				sz+=rndmm2(-5500,-4000,4000,5500);
...
//				energie_COMOB->Special_handle = Transparency_table_handle;
				...
				behave->Data.Just_data.Data[4] = -8000;
				behave->Data.Just_data.Data[5] = -8000;
	...
	if (Strength)
	{
		...
		if (Victim_part->Type == MONSTER_PART_TYPE)
		{
			...
			/* Dissolve Monster */
			dissolve_balls=Dissolve(Victim_part,4,4,&ball_COMOB[0],C3_Spell_14_ANZDISSOLVECOMOBS);
...
//			Delete_COMOB(flash_COMOB);
			...
			/* set coordinates */
			blitz_COMOB->X_3D = orgx;
			blitz_COMOB->Y_3D = orgy;
			blitz_COMOB->Z_3D = orgz-(5*COMOB_DEC_FACTOR);
			...
			/* Set display parameters */
			blitz_COMOB->Draw_mode = TRANSPARENT_COMOB_DRAWMODE;
			blitz_COMOB->Special_handle = Transluminance_table_handle;

			blitz_COMOB->Hotspot_X_offset = 50;
			blitz_COMOB->Hotspot_Y_offset = 50;

			/* Set random size */
			blitz_COMOB->Display_width = 25;
			blitz_COMOB->Display_height = blitz_COMOB->Display_width;
			...
			do{
				Update_screen();
				blitz_COMOB->Display_width = t;
				blitz_COMOB->Display_height = t;
				t+=Nr_combat_updates*15;
			}while(t<75);
			blitz_COMOB->Display_width = 75;
			blitz_COMOB->Display_height = 75;

			/* und explodieren dann */
			for(t=0;t<dissolve_balls;t++){

				/* nach oben schleudern */
				ball_COMOB[t]->dX_3D=rndmm(-300,300);
				ball_COMOB[t]->dY_3D=rndmm( 300,600);
				ball_COMOB[t]->dZ_3D=rndmm(-300,300);

				/* Lifespan setzten */
//				ball_COMOB[t]->Lifespan=rndmm(32,360);
				...
				behave->Data.Just_data.Data[2] = 75;
				behave->Data.Just_data.Data[3] = 75;
				behave->Data.Just_data.Data[4] = -8000;
				behave->Data.Just_data.Data[5] = -8000;
...
//				behave->Data.Just_data.Data[5] = -800;
			...
			blitz_COMOB->Display_width = 5;
			blitz_COMOB->Display_height = 5;

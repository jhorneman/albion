...
void Swap_colours(UNLONG A, UNLONG B, UNBYTE *Data);
BOOLEAN Compare_colours(UNLONG A, UNLONG B, UNBYTE *Data);
...
{
	UNLONG Diff, Right_diff, Left_diff, Green_diff;
	SISHORT Middle, Left, Right;
	SISHORT dR, dG, dB;
	...
	if (Diff)
	{
		...
		for (;;)
		{
			/* Are we still searching to the left ? */
			if (Left > 0)
			{
...
#ifdef URGLE
{
	UNLONG Max1 = 0xFFFFFFFF, Max2 = 0xFFFFFFFF, d;
	UNSHORT i;
	SISHORT R,G,B;
	UNBYTE Target = 0;

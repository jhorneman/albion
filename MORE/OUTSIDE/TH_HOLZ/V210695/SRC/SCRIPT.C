...
void SKRIPT_Init (SCRIPT *Script, char *ScriptFilename)
{
	...
	for (i=0; i<Script->AnzLines; i++)
	{
		...
		for (j=0; j<l; j++)
		{
			...
			{
				(Script->Line[i])[j]='\n';
				strcpy (Script->Line[i]+j+1, Script->Line[i]+j+2);
				l--; j--;
			}
		}
...
SILONG SCRIPT_ProcessScriptLine (SCRIPT *Script, SILONG Line, SILONG *AnzOpenIf)
{
	...
	switch (Token)
	{
		...
		case TOKEN_COPY:
			...
			Copy (Parameter[1], Parameter[2], AnzParameter==4);
			break;
		...
		case TOKEN_INSTALL:
			...
			Install (Parameter[1], Parameter[2], AnzParameter==4);
			break;
		...
		case TOKEN_SELECT_TARGET_DRIVE:
			if (AnzParameter != 2)	InternalError (ERR_NUM_PARAMETERS, Line+1, Script->Line[Line]);
			SelectTargetDrive (SCRIPT_GetIntParameter (Parameter[1], Line));
			break;

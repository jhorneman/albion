...
//#define D32RealSeg(P)   ((((unsigned long) (P)) >> 4) & 0xF000)
//#define D32RealOff(P)   (((unsigned long) (P)) & 0xFFFF)

//VESA-Struktur:
struct VgaInfoBlock
{
	char   VESA_Signature[4];    //Sollte immer 'VESA' sein
	short  VESA_Version;
	char  *OEM_String;
	long   Capabilities;
	short *SupportList;
	short  MemoryBlocks;         //Anzahl der 64k Bl”cke auf der Karte
	char   Dummy[236];
} *MyVesaInfo = NULL;
...
void *D32DosMemAlloc(long size)
{
	...
	Regs.x.ebx = (size + 15) >> 4; //Number of paragraphs requested
...
BOOL VESA_UpdateIni (void)
{
   ...
   int386x (0x31, &Regs, &Regs, &SRegs);
	...
	if (IfExistsFile (VesaIniFilename))
	{
		IniSet (VesaIniFilename, "OEM", MyVesaInfo->OEM_String);

		sprintf (Buffer, "%li", (SILONG)(MyVesaInfo->VESA_Version/256));
		IniSet (VesaIniFilename, "VESA_VERSION_NUMBER", Buffer);

		sprintf (Buffer, "%li", (SILONG)(MyVesaInfo->VESA_Version&255));
		IniSet (VesaIniFilename, "VESA_VERSION_SUBNUMBER", Buffer);

		sprintf (Buffer, "%li", MyVesaInfo->MemoryBlocks*64*1024);
		IniSet (VesaIniFilename, "MEMORY", Buffer);

		//256 Farben:
		if (VESA_SupportsMode (0x100, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_640x400x256", "Y");
		else	IniSet (VesaIniFilename, "MODE_640x400x256", "N");

		if (VESA_SupportsMode (0x101, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_640x480x256", "Y");
		else	IniSet (VesaIniFilename, "MODE_640x480x256", "N");

		if (VESA_SupportsMode (0x103, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_800x600x256", "Y");
		else	IniSet (VesaIniFilename, "MODE_800x600x256", "N");

		if (VESA_SupportsMode (0x105, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_1024x768x256", "Y");
		else	IniSet (VesaIniFilename, "MODE_1024x768x256", "N");

		//32000 Farben:
		if (VESA_SupportsMode (0x110, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_640x480x32k", "Y");
		else	IniSet (VesaIniFilename, "MODE_640x480x32k", "N");

		if (VESA_SupportsMode (0x113, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_800x600x32k", "Y");
		else	IniSet (VesaIniFilename, "MODE_800x600x32k", "N");

		if (VESA_SupportsMode (0x116, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_1024x768x32k", "Y");
		else	IniSet (VesaIniFilename, "MODE_1024x768x32k", "N");

		//64000 Farben:
		if (VESA_SupportsMode (0x111, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_640x480x64k", "Y");
		else	IniSet (VesaIniFilename, "MODE_640x480x64k", "N");

		if (VESA_SupportsMode (0x114, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_800x600x64k", "Y");
		else	IniSet (VesaIniFilename, "MODE_800x600x64k", "N");

		if (VESA_SupportsMode (0x117, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_1024x768x64k", "Y");
		else	IniSet (VesaIniFilename, "MODE_1024x768x64k", "N");

		//16.2Mio Farben:
		if (VESA_SupportsMode (0x112, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_640x480x16.2M", "Y");
		else	IniSet (VesaIniFilename, "MODE_640x480x16.2M", "N");

		if (VESA_SupportsMode (0x115, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_800x600x16.2M", "Y");
		else	IniSet (VesaIniFilename, "MODE_800x600x16.2M", "N");

		if (VESA_SupportsMode (0x118, MyVesaInfo->SupportList))
				IniSet (VesaIniFilename, "MODE_1024x768x16.2M", "Y");
		else	IniSet (VesaIniFilename, "MODE_1024x768x16.2M", "N");
	}

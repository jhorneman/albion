...
.Change:	move.w	d0,Camera_height_old	; Yes -> Recalculate
	...
	add.w	d0,d0			; Convert from cm to 3D
	move.w	d0,d1			; Save
...
;-- sichtbar !
	move.w	d6,d2			; Calculate distance
	add.w	d7,d2
	asr.w	#1,d2
;	cmp.w	#Max_wall_Z,d2
;	bpl	.blocko
...
;	add	d7,d6
;	asr	#1,d6
	move.w	d2,(a3)+			; entfernung speichern
	clr.w	(a3)+
	addq.l	#4,a3
...
; Changed registers : d2,d5,d6,d7,a3,a4
; Note :
;  - This subroutine is a part of [ Get_faces ].
;*****************************************************************************
Create_floor_polygons:
	movem.l	d0/d3/d4/a5/a6,-(sp)
	lea.l	Floor_ptrs,a6
; ---------- Create floor polygon -----------------
	...
	bpl.s	.bp_visible
	tst.w	d7
	bmi	.No_floor			;-> ganz im negativen Z-bereich !
.bp_visible:
	move.w	d5,d1			; Calculate distance
	add.w	d7,d1
	asr.w	#1,d1
	sub.w	#patt_size/2,d1
;	cmp.w	#Max_wall_Z,d1		; Out of range ?
;	bpl	.No_floor
...
;	sub.w	#patt_size/2,d5
	move.w	d1,(a3)+			;entfernung speichern
	clr.w	(a3)+
	addq.l	#4,a3
...
;	bpl	.Exit
	...
	move.l	a4,4(a3)			;adresse speichern
	addq.w	#1,Visible_floors
	cmp.b	#City_map_type,Current_map_type	; City ?
	bne.s	.Dungeon
	move.w	#"BC",(a4)+		; Yes -> City ceiling
	bra.s	.Go_on
.Dungeon:	move.w	#"BP",(a4)+		; No -> Normal ceiling
.Go_on:	move.w	upper_y,d2
...
;	sub.w	#patt_size/2,d5
	...
	clr.w	(a3)+
	addq.l	#4,a3
.Exit:	movem.l	(sp)+,d0/d3/d4/a5/a6
	rts
...
.Loop1:	movem.l	d7/a0,-(sp)
	...
	move.w	(a0),d0
	cmp.w	#"BP",d0			; Is floor ?
	beq.s	.Floor
	cmp.w	#$face,d0			; No -> Is wall ?
	beq.s	.Wall1
	cmp.w	#$abed,d0			; No -> Is object ?
	beq.s	.Object1
; ---------- Clip city ceiling --------------------
	bsr	preclip_tmpoly
	tst.w	d3
	...
	bra	.Next1
; ---------- Clip object --------------------------
.Object1:	movem.w	2(a0),d0-d4		; No -> Is object
...
; ---------- Draw city ceiling --------------------
	...
	blt	.Skip2			;-> clip_out !
	bsr	draw_newTM
.Skip2:	movem.l	(sp)+,d7/a1/a4
	bra	.Next2
; ---------- Draw object --------------------------
.Object2:	movem.l	d7/a1,-(sp)
...
.do_zoomclip:
	...
	move.w	d7,-(sp)
	move.w	Object_dungeon_height(a0),d7	; Get object height in cm
	add.w	d7,d7			; Convert to 3D units
	add.w	d3,d7			; Add Y-coordinate
	sub.w	lower_y,d7		; Translate
	cmp.w	Wall_height_3D,d7		; Larger than wall ?
	bpl.s	.Above
	move.w	(sp)+,d7
...
.Above:	move.w	(sp)+,d7
	...
	bmi.s	.Out			;  just 3D window clipping
...
.Loop:	move.w	Object_nr(a0),d4		; Anything there ?
	...
	tst.w	d2			; Behind camera ?
	bmi.s	.Next			; Yes -> Skip
;	cmp.w	#Max_wall_Z,d2		; Out of range ?
;	bmi.s	.No
;	move.l	Labdata_ptr,a1		; Yes -> Get object data
;	adda.l	Object_data_offset,a1
;	move.w	d4,d3
;	mulu.w	#Object3D_data_size,d3
;	add.l	d3,a1
;	move.w	Object_Z(a0),d3		; Large object ?
;	add.w	Object_dungeon_height(a1),d3
;	add.w	d3,d3
;	cmp.w	Wall_height_3D,d3
;	bmi.s	.Next
.No:	move.w	d2,(a3)+			; Save data in sort list
	move.w	d7,(a3)+
	move.l	a6,(a3)+
	addq.w	#1,Visible_objects		; Count up !!!
	move.w	#$abed,(a6)+		; Save object data
...
Get_wall_address:
	...
	bra.s	.Do
.Trans:	move.b	d2,Transparent_wall_flag	; No -> Transparent
.Do:	and.w	#$00ff,d0

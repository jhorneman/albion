...
PUME_colour	EQU
Blocked_PUME_colour	EQU
PUME_background_colour	EQU
...
.Loop:	btst	#PUME_absent,PUME_flags(a0)	; Absent ?
	bne.s	.Next
	move.w	#PUME_background_colour,d1	; No -> Display
	jsr	Display_PUME
	addq.w	#1,d0
...
;************************************************************************************
; [ Display a Pop-Up Menu Entry ]
;   IN : d0 - PUME index {1...} (.w)
;        d1 - Background colour (.w)
;        a0 - Pointer to PUME data (.l)
; All registers are restored
;************************************************************************************
Display_PUME:
	movem.l	d0-d4/a0/a1,-(sp)
	move.w	d1,d4			; Save colour
	move.l	PUM_Sp,a1			; Get current PUM data
	move.l	(a1),a1
	...
	move.w	PUM_X(a1),d0
	addq.w	#1,d0
	move.w	d0,d2			; Get bottom-right coordinates
	move.w	d1,d3
	add.w	PUM_width,d2
	subq.w	#2+1,d2
	add.w	#PUME_height-2-1,d3
	jsr	Draw_box			; Draw background
